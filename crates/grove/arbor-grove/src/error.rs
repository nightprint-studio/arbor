...
//!
//! Each grove crate has its own `Result<T, …Error>` alias; glob-merging all four
//! preludes would collide on the bare `Result` name. Rather than push that
//! collision onto consumers, the facade exposes a single [`GroveError`] that
...
//! single grove error to convert into `AppError` at the IPC boundary.

use thiserror::Error;

use arbor_grove_audio::prelude::AudioError;
use arbor_grove_engine::prelude::EngineError;
use arbor_grove_lang::prelude::LangError;
use arbor_grove_pattern::prelude::PatternError;

/// Any failure surfaced through the grove facade.
///
...
/// `GroveError` automatically under `?`.
#[derive(Debug, Error)]
pub enum GroveError {
    ...
    /// `AudioError`; a bare `AudioError` maps to [`GroveError::Audio`] instead.
