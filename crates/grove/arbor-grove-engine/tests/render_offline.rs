...
use arbor_grove_engine::prelude::{render_offline, RenderConfig};
use arbor_grove_pattern::prelude::{pure, seq, track, tracks, ControlMap};
...
fn renders_a_drum_pattern_to_audible_wav() {
    ...
    let out = temp("grove_render_drums.wav");
...
fn renders_with_delay_send_through_the_full_chain() {
    ...
    let out = temp("grove_render_delay.wav");
...
fn renders_multiple_tracks_into_one_mix() {
    ...
    let out = temp("grove_render_multitrack.wav");
    let res = render_offline(&t, 1.0, 1, &fast_cfg(), &out);
