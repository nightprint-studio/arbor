...
/// source it came from.
///
/// Shape follows Tidal (`whole` / `part` / `value`) plus grove's `span`:
///
...
/// - `value` — the payload (generic; the real grove value is
///   [`crate::control::ControlMap`]).
/// - `span` — byte range in the `.grove` source for live highlight; `None` for
