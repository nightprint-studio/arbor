//! The typed AST for a `.grove` program.
//!
...
//! [`Pattern`](arbor_grove_pattern::prelude::Pattern), and the emitter prints it
//! back to source (`design/grove/editing-model.md` — keep the AST clean and
//! regular so emission is deterministic). It mirrors the grammar in
//! `design/grove/grammar.md` one-to-one.
//!
...
/// A whole `.grove` source file: a sequence of top-level items.
#[derive(Clone, Debug, PartialEq)]
pub struct Program {
    pub items: Vec<Item>,
}
