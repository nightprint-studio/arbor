//! # merula
//!
//! The **facade** of merula (Arbor's live-coding music engine). merula is split
//! into four library crates with one-way dependencies:
//!
//! ```text
//!         merula-pattern        pure pattern algebra (zero deps)
//!          ↑               ↑
...
//! single `use merula::prelude::*;` instead of importing from four crates.
//!
...
//! offers a single unified [`MerulaError`](error::MerulaError) +
...
//! `EngineError`) and convert into `MerulaError` with `?`.
//!
//! ## Entry point
//!
//! Reach the whole merula API through [`prelude`] (workspace convention). There
