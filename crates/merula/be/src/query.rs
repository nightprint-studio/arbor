//! `query` domain — off-thread arrangement query for the timeline view.
//!
//! `active_haps` (the live editor highlight) only reports what sounds *now*; the
//! arrangement view needs the whole timeline. `merula_query` queries the
//! last-evaluated `Tracks` over `[0, cycles)` — on a dispatcher worker, never the
//! audio thread — and returns every hap + every named section + the detected loop
//! period.
//!
//! W1 ports the **pure compute core** — the DTOs, [`collect_haps`], and the
//! loop-period detector ([`detect_loop_cycles`]) with their tests. The handler
//! reads the last-good evaluation from [`MerulaState::latest`]; until the W3 eval
//! domain stages a typed `Latest` into it (the W0 slot is raw JSON, which a
//! `Tracks<ControlMap>` — full of closures — cannot round-trip through), there is
//! nothing staged, so `merula_query` returns the empty result. [`collect_haps`] is
//! `pub(crate)` so the W3 eval / snippet domains call the same core.
...
/// The `merula_query` result: every hap + every named section over the window.
#[derive(Debug, Clone, Serialize)]
pub struct QueryHaps {
    ...
    /// `tempo(...)` point, else the script's `cps(...)`. `None` when the script set
    /// neither.
    pub cps: Option<f64>,
}
...
/// still bounding the query of a non-looping snippet to its content end. Mirrors
/// the shell's value verbatim.
pub const SNIPPET_WINDOW: u32 = 64;

/// The `merula_eval_snippet` result: a snippet evaluated in isolation. Errors come
/// back inline in `diagnostics` (never on the `merula:diagnostics` channel — that
/// belongs to the main editor); on success `diagnostics` is empty.
#[derive(Debug, Clone, Serialize)]
pub struct SnippetEval {
    /// Parse/eval/validation errors (empty on success).
...
///    which the hap set is invariant under a shift of `P`.
/// 3. **Fallback** — no repetition detected: use the content end (max hap end,
///    rounded up). Never `0` when haps exist.
fn detect_loop_cycles(haps: &[QueryHap], sections: &[QuerySection], window: u32) -> u32 {
    ...
    if !sections.is_empty() {
        let period = sections.iter().map(|s| s.end.ceil() as u32).max().unwrap_or(0);
        if period > 0 {
...
/// `merula_query` (over the staged arrangement) and the snippet evaluator (over a
/// freshly-evaluated `EvalOutput`). Pure — no lock, no state.
pub(crate) fn collect_haps(
...
/// it's safe to call while playing.
#[arbor_rpc::handler]
fn merula_query(_ctx: &MerulaState, cycles: u32) -> Result<QueryHaps, String> {
    let queried = session::with_latest(|l| {
        let (haps, sections, loop_cycles) = collect_haps(&l.tracks, cycles);
        QueryHaps { haps, sections, loop_cycles, cps: l.cps }
    });
    Ok(queried.unwrap_or(QueryHaps {
        haps: Vec::new(),
        sections: Vec::new(),
        loop_cycles: 0,
        cps: None,
