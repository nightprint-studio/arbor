...
The **timing runtime** of [merula](../../../design/merula), Arbor's live-coding music engine. This is
**Fase 3**: it queries the pure patterns and fires sample-accurate triggers at the audio backend, in
time. It depends on `merula-pattern` + `merula-audio` — and deliberately **not** on
`merula-lang`: the shell evaluates `.merula` source → `Tracks<ControlMap>` and hands those in;
