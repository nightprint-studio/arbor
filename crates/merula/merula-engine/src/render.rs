//! Offline render: drive the audio [`Renderer`](merula_audio::prelude::Renderer)
...
/// ([`Renderer::track_peaks`](merula_audio::prelude::Renderer::track_peaks))
...
mod tests {
    ...
    fn region_render_offset_succeeds_and_writes_file() {
        use merula_pattern::prelude::{fastcat, pure, track, tracks};
        use merula_pattern::prelude::ControlMap;
        ...
        let out = std::env::temp_dir().join("merula_render_region.wav");
    ...
    fn analyze_levels_flags_a_hot_track() {
        use merula_pattern::prelude::{pure, track, tracks, ControlMap};
    ...
    fn render_with_missing_file_source_falls_back_and_succeeds() {
        use merula_pattern::prelude::{sample, track, tracks};
        ...
        let out = std::env::temp_dir().join("merula_render_missing_source.wav");
