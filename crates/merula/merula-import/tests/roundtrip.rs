...
//! assert the emitted `.merula` re-parses and carries the expected idioms.

use merula_import::prelude::{smf_to_merula, ImportOptions};
...
#[test]
fn full_pipeline_emits_parseable_idiomatic_merula() {
    let src = smf_to_merula(&demo(), &ImportOptions::default()).unwrap();
