//! # merula-import
//!
//! The **deterministic** half of merula's audio/MIDI import: turn a MIDI file
//! into idiomatic `.merula` source, with **no AI** anywhere in the path. (The
//! AI half — WAV → MIDI transcription — is [`merula-transcribe`]; this
...
//!              │
//!              ▼ emit (build merula-lang AST, print canonically)
//!   idiomatic .merula source
//! ```
//!
...
//!   as a handful of named phrases), printed through `merula-lang`'s emitter.
//!
//! The one-call entry point is [`midi_to_merula`](convert::midi_to_merula).
//!
//! ## Entry point
//!
//! Reach the public API through [`prelude`] (workspace convention).
//!
//! [`merula-transcribe`]: https://docs.rs/merula-transcribe
