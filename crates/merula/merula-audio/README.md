...
The real-time / DSP layer of [merula](../../../design/merula), Arbor's live-coding music engine — the
only merula crate that touches the audio hardware. This is **Fase 2**: it turns the engine's
...
**The contract** (`seam.rs`) — frozen so `merula-engine` is developed against it in parallel:
...
- `synth.rs` — the **default synth** (saw/square/sine/triangle oscillator + ADSR), merula's fallback
...
`merula-pattern` for `ControlMap` / `SourceKind`.
...
Part of the merula crate stack: `merula-pattern` → `merula-audio` → `merula-engine`.
