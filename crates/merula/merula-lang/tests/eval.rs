...
//!
//! Mirrors Fase 1's exit criterion: a `.merula` AST → the right haps.

use std::rc::Rc;
use std::sync::Arc;

use merula_lang::ast::*;
use merula_lang::prelude::{evaluate, EvalConfig, NoImports, SilentLog};
use merula_pattern::prelude::{ControlMap, Hap, Pattern, TempoMap, Time, TimeSpan, Tracks};

// ── AST builders ──────────────────────────────────────────────────────────────

fn sp() -> merula_pattern::prelude::SourceSpan {
    merula_pattern::prelude::SourceSpan::new(0, 0)
}
...
fn run(items: Vec<Item>) -> merula_lang::prelude::Result<Tracks<ControlMap>> {
    evaluate(
        &Program { items },
        Rc::new(NoImports),
        Arc::new(SilentLog),
