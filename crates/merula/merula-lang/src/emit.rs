//! AST → `.merula` source: a deterministic pretty-printer.
//!
//! The inverse direction of the evaluator. It walks the [`ast`](crate::ast) and
//! prints **canonical** `.merula` text that re-parses to the same tree — a
...
//! expected, `design/merula/editing-model.md`). On already-canonical input the
...
// Precedence ladder, loosest → tightest (mirrors `design/merula/grammar.md §4`).
const P_LAMBDA: u8 = 0;
const P_RANGE: u8 = 1;
const P_ADD: u8 = 2;
const P_MUL: u8 = 3;
...
/// Print a whole program to canonical `.merula` source (trailing newline).
