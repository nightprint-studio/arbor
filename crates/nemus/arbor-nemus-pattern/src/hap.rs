...
/// source it came from.
///
/// Shape follows Tidal (`whole` / `part` / `value`) plus nemus's `span`:
///
...
/// - `value` — the payload (generic; the real nemus value is
///   [`crate::control::ControlMap`]).
/// - `span` — byte range in the `.nemus` source for live highlight; `None` for
