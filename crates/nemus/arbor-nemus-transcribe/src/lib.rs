...
//!
//! WAV → MIDI transcription for nemus, behind a swappable [`Transcriber`] trait
...
//!                  ┌────────────┴───────────┐
//!                  ▼                         ▼
//!   in memory → arbor-nemus-import      written to disk → .mid
//!   (transient WAV → .nemus)            ("Convert WAV to MIDI")
//! ```
//!
//! ## Backends
//!
...
//!   models download on demand (reusing nemus's sample-pack download path). The
//!   trait is the seam; nothing else changes when they land.
//!
//! Pick a backend through [`transcriber_for`]. The interchange is always a
//! [`midly::Smf`] — the deterministic `.nemus` converter (`arbor-nemus-import`)
