...
| `render` | `render_offline(tracks, cps, cycles, &RenderConfig, path)` — non-real-time driver reusing the same scheduling + `Renderer`, writing WAV (24-bit/48k default) via `hound`. Pre-scans the arrangement and preloads every `sample`/`audio` file source (best-effort) so file voices decode instead of falling back to synth, and configures each track's delay bus as it changes. Length is an explicit `cycles` count + tail. Fine under Arbor's job system. `render_offline_with_progress(…, on_progress, should_cancel)` is the same driver with a `FnMut(RenderProgress)` callback fired per block (frames written / total) and a `Fn() -> bool` polled before each block — when it returns true the bounce stops early and returns `RenderOutcome::Cancelled` (partial file finalized), so the shell can surface a live percentage **and** a Stop button. |
