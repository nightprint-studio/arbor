//! # arbor-nemus-engine
//!
//! The **timing runtime** of nemus (Arbor's live-coding music engine): it turns
//! patterns into sound *at the right moment*. It sits between the pure algebra
//! and the audio backend, depending on both — but **not** on `arbor-nemus-lang`
//! (the shell evaluates source → [`Tracks`](arbor_nemus_pattern::prelude::Tracks)
...
//!   over an [`AudioSink`](arbor_nemus_audio::prelude::AudioSink), with quantized
//!   tempo / re-eval swaps.
...
//! `arbor_nemus_audio::prelude::RecordingSink` and assert on the emitted events
