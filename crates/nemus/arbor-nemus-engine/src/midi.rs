...
//! Standard MIDI File (one MIDI track per nemus track). Pure note data — no
...
/// `out_path`. One MIDI track per nemus track that yields any note; the first
...
/// Map a nemus drum sound name to a General-MIDI percussion key, or `None` for a
...
mod tests {
    use super::*;
    use arbor_nemus_pattern::prelude::{fastcat, pure, sample, track, tracks};
    ...
    fn exports_pitched_notes_to_a_reparseable_file() {
        ...
        let out = std::env::temp_dir().join("nemus_export_pitched.mid");
    ...
    fn empty_arrangement_writes_valid_file() {
        ...
        let out = std::env::temp_dir().join("nemus_export_empty.mid");
