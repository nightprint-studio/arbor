//! Assemble idiomatic `.nemus` from a [`Song`].
//!
//! The output is built as an `arbor-nemus-lang` AST and printed through that
...
//!
//! This mirrors the algorithm in `arbor-nemus-lang`'s `materialize` (single-cycle
...
/// Render a whole [`Song`] to idiomatic `.nemus` source (with trailing newline).
pub fn song_to_nemus(song: &Song, opts: &ImportOptions) -> String {
...
/// MIDI semitone → nemus note name (`60 → c4`, `63 → ef4`); octave always
...
mod tests {
    ...
    fn render(tracks: Vec<NoteTrack>, len: u32) -> String {
        ...
        song_to_nemus(&song, &ImportOptions::default())
    }
