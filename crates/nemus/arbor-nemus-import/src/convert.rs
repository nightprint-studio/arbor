...
/// Convert raw `.mid` bytes (on-disk or transient) to idiomatic `.nemus` source.
pub fn midi_to_nemus(bytes: &[u8], opts: &ImportOptions) -> Result<String> {
    Ok(emit::song_to_nemus(&midi_to_song(bytes, opts)?, opts))
}

/// Convert an already-parsed [`Smf`] to idiomatic `.nemus` source.
pub fn smf_to_nemus(smf: &Smf, opts: &ImportOptions) -> Result<String> {
    let mut song = transcode::from_smf(smf, opts)?;
    quantize::quantize_song(&mut song, opts.grid);
    Ok(emit::song_to_nemus(&song, opts))
}
