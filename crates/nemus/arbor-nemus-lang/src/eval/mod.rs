...
fn eval_var(ctx: &Rc<Ctx>, env: &Env, name: &str, span: arbor_nemus_pattern::prelude::SourceSpan) -> Result<Value> {
...
fn eval_call(
    ...
    name: &str,
    args: Vec<Value>,
    span: arbor_nemus_pattern::prelude::SourceSpan,
) -> Result<Value> {
    if let Some(v) = resolve(ctx, env, name) {
        return match v {
...
fn outputs_to_tracks(outputs: Vec<Value>) -> Result<Tracks<ControlMap>> {
    use arbor_nemus_pattern::prelude::{track, track_with_sections, tracks};
