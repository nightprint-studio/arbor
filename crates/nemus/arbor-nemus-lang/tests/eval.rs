...
//!
//! Mirrors Fase 1's exit criterion: a `.nemus` AST → the right haps.

use std::rc::Rc;
use std::sync::Arc;

use arbor_nemus_lang::ast::*;
use arbor_nemus_lang::prelude::{evaluate, EvalConfig, NoImports, SilentLog};
use arbor_nemus_pattern::prelude::{ControlMap, Hap, Pattern, TempoMap, Time, TimeSpan, Tracks};
...
fn run(items: Vec<Item>) -> arbor_nemus_lang::prelude::Result<Tracks<ControlMap>> {
    evaluate(
        &Program { items },
        Rc::new(NoImports),
        Arc::new(SilentLog),
