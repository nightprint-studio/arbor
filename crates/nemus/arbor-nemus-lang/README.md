...
The `.nemus` **language** layer of [nemus](../../../design/nemus), Arbor's live-coding music engine.
It turns source text into the pure [`Pattern`](../arbor-nemus-pattern) algebra of **Fase 0** and back.
This is **Fase 1**: no audio, no scheduler — pure and unit-testable.

```text
source (.nemus)  ──parse──▶  CST  ──walk──▶  AST  ──eval──▶  Pattern
                                              AST  ──emit──▶  source
```

## Layers (decoupled by design)

The layers never reach across each other (`design/nemus/editing-model.md`):
...
- **Front end** — a [Tree-sitter](https://tree-sitter.github.io) grammar (`grammar.js`) plus a
  ...
  maximal munch, and `[]`/`<>` balancing (`design/nemus/grammar.md §5`). Native byte ranges give every
  node a source span for free.
- **AST** (`ast.rs`) — the typed tree, the contract every other layer targets. Mirrors
  `design/nemus/grammar.md` one-to-one; every node carries a `SourceSpan`.
...
```sh
cd crates/nemus/arbor-nemus-lang
...
`tree-sitter parse some.nemus` (this one *does* compile the parser, so it needs the C toolchain).
...
## Consumes from `arbor-nemus-pattern`
...
Reach the API through the prelude (workspace convention): `use arbor_nemus_lang::prelude::*;`.

Part of the nemus crate stack: `arbor-nemus-pattern` → **`arbor-nemus-lang`** / `arbor-nemus-audio` →
`arbor-nemus-engine`. See [`design/nemus/architecture.md`](../../../design/nemus/architecture.md).
