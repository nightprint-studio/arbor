...
The real-time / DSP layer of [nemus](../../../design/nemus), Arbor's live-coding music engine — the
only nemus crate that touches the audio hardware. This is **Fase 2**: it turns the engine's
...
**The contract** (`seam.rs`) — frozen so `arbor-nemus-engine` is developed against it in parallel:
...
`arbor-nemus-pattern` for `ControlMap` / `SourceKind`.
