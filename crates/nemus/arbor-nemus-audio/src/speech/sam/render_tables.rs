//! Renderer ROM tables — a faithful 1:1 port of the SAM reconstruction
//! (`discordier/sam-js`, `src/renderer/tables.es6`). The numeric values are the
//! original Software Automatic Mouth data; do not "tidy" them.
//!
//! Where the JS combines three byte-fields with `a | b | c` (each occupying a
//! distinct byte position), the expressions are kept verbatim so a value can be
//! checked against the source at a glance.

/// Values substituted for zero bits in unvoiced consonant samples. (`tab48426`)
pub const SAMPLED_CONSONANT_VALUES0: [u8; 5] = [0x18, 0x1A, 0x17, 0x17, 0x17];

/// Per-stress pitch offset. (`tab47492`)
pub const STRESS_PITCH: [i32; 10] = [0x00, 0xE0, 0xE6, 0xEC, 0xF3, 0xF9, 0x00, 0x06, 0x0C, 0x06];

/// Used to decide which phoneme's blend lengths are used (lower score wins).
/// (`tab45856`)
pub const BLEND_RANK: [i32; 80] = [
...
/// Frames at the end of a phoneme devoted to interpolating to the next. (`tab45696`)
pub const OUT_BLEND_LENGTH: [i32; 80] = [
...
/// Frames at the start of a phoneme devoted to interpolating to its value. (`tab45776`)
pub const IN_BLEND_LENGTH: [i32; 80] = [
    0x00, 0x02, 0x02, 0x02, 0x02, 0x04, 0x04, 0x04, //
    0x04, 0x04, 0x04, 0x04, 0x04, 0x04, 0x04, 0x04, //
...
/// Two bitfields: low 3 bits select a 256-byte section of [`SAMPLE_TABLE`] (and
/// index [`SAMPLED_CONSONANT_VALUES0`]); high 5 bits give the inverted offset for
/// unvoiced consonants. (`tab45936`)
pub const SAMPLED_CONSONANT_FLAGS: [i32; 80] = [
...
/// Packed formant frequencies: `freq1 | freq2<<8 | freq3<<16`.
...
/// Packed formant amplitudes: `ampl1 | ampl2<<8 | ampl3<<16`.
pub const AMPL_DATA: [u32; 80] = [
    0x000000 | 0x000000 | 0x000000, // ' *' 00
...
/// Decibel → linear amplitude rescale (used in `PrepareFrames`).
pub const AMPLITUDE_RESCALE: [i32; 16] = [
