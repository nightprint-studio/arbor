...
pub(crate) fn install(_ctx: &ApiCtx, lua: &Lua, arbor: &Table) -> Result<()> {
    let hooks_table = lua.create_table().map_err(|e| PluginCoreError::Plugin(e.to_string()))?;

    install_list(lua, &hooks_table)?;
    install_describe(lua, &hooks_table)?;

    arbor.set("hooks", hooks_table).map_err(|e| PluginCoreError::Plugin(e.to_string()))?;
    Ok(())
}
...
fn install_list(lua: &Lua, t: &Table) -> Result<()> {
    ...
    t.set("list", fn_).map_err(|e| PluginCoreError::Plugin(e.to_string()))?;
    Ok(())
}
...
fn install_describe(lua: &Lua, t: &Table) -> Result<()> {
    ...
    t.set("describe", fn_).map_err(|e| PluginCoreError::Plugin(e.to_string()))?;
    Ok(())
}
