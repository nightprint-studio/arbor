...
use crate::lua_api::helpers::convert::lua_value_to_duration_secs;
use arbor_plugin_types::prelude::{Schedule, ScheduleTrigger};
use arbor_scheduler::prelude::validate_cron;

pub(crate) fn install(ctx: &ApiCtx, lua: &Lua, arbor: &Table) -> Result<()> {
    let sched_table = lua.create_table().map_err(|e| PluginCoreError::Plugin(e.to_string()))?;

    install_register(ctx, lua, &sched_table)?;
    install_list(ctx, lua, &sched_table)?;

    arbor.set("scheduler", sched_table).map_err(|e| PluginCoreError::Plugin(e.to_string()))?;
    Ok(())
}
...
fn install_register(ctx: &ApiCtx, lua: &Lua, sched_table: &Table) -> Result<()> {
    ...
    sched_table.set("register", fn_).map_err(|e| PluginCoreError::Plugin(e.to_string()))?;
    Ok(())
}
...
fn install_list(ctx: &ApiCtx, lua: &Lua, sched_table: &Table) -> Result<()> {
    ...
    sched_table.set("list", fn_).map_err(|e| PluginCoreError::Plugin(e.to_string()))?;
    Ok(())
}
