...
pub(crate) fn install(ctx: &ApiCtx, lua: &Lua, ui: &Table) -> Result<()> {
    let settings_ui = lua.create_table().map_err(|e| PluginCoreError::Plugin(e.to_string()))?;

    install_panel(ctx, lua, &settings_ui)?;
    install_open(ctx, lua, &settings_ui)?;
    install_close(ctx, lua, &settings_ui)?;

    ui.set("settings", settings_ui).map_err(|e| PluginCoreError::Plugin(e.to_string()))?;
    Ok(())
}
...
fn install_panel(ctx: &ApiCtx, lua: &Lua, settings_ui: &Table) -> Result<()> {
    ...
    settings_ui.set("panel", fn_).map_err(|e| PluginCoreError::Plugin(e.to_string()))?;
    Ok(())
}
...
fn install_open(ctx: &ApiCtx, lua: &Lua, settings_ui: &Table) -> Result<()> {
    ...
    settings_ui.set("open", fn_).map_err(|e| PluginCoreError::Plugin(e.to_string()))?;
    Ok(())
}
...
fn install_close(ctx: &ApiCtx, lua: &Lua, settings_ui: &Table) -> Result<()> {
    ...
    settings_ui.set("close", fn_).map_err(|e| PluginCoreError::Plugin(e.to_string()))?;
    Ok(())
}
