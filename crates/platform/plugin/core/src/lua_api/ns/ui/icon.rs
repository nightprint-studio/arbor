...
pub(crate) fn install(ctx: &ApiCtx, lua: &Lua, ui: &Table) -> Result<()> {
    let icon_table = lua.create_table().map_err(|e| PluginCoreError::Plugin(e.to_string()))?;
    ...
    icon_table.set("register", register_fn).map_err(|e| PluginCoreError::Plugin(e.to_string()))?;

    ui.set("icon", icon_table).map_err(|e| PluginCoreError::Plugin(e.to_string()))?;
    Ok(())
}
