...
fn install_open_path(ctx: &ApiCtx, lua: &Lua, ui: &Table) -> Result<()> {
    ...
    ui.set("open_path", fn_).map_err(|e| PluginCoreError::Plugin(e.to_string()))?;
    Ok(())
}
...
fn install_copy_to_clipboard(ctx: &ApiCtx, lua: &Lua, ui: &Table) -> Result<()> {
    ...
    let fn_ = lua.create_function(move |_, cfg: mlua::Table| {
        ...
        if let Some(ref c) = app_ctx {
            c.emit("plugin:ui-clipboard-write", serde_json::json!({
                "plugin": pname,
                "text":   text,
    ...
    ui.set("copy_to_clipboard", fn_).map_err(|e| PluginCoreError::Plugin(e.to_string()))?;
    Ok(())
}
...
fn install_open_job_output(ctx: &ApiCtx, lua: &Lua, ui: &Table) -> Result<()> {
    ...
    let fn_ = lua.create_function(move |_, job_id: String| {
        ...
        if let Some(ref c) = app_ctx {
            c.emit("plugin:ui-open-job-output", serde_json::json!({
                "plugin": pname,
                "job_id": job_id,
            }));
        }
    ...
    ui.set("open_job_output", fn_).map_err(|e| PluginCoreError::Plugin(e.to_string()))?;
    Ok(())
}
...
fn install_open_panel(ctx: &ApiCtx, lua: &Lua, ui: &Table) -> Result<()> {
    ...
    let fn_ = lua.create_function(move |_, panel_id: String| {
        ...
        if let Some(ref c) = app_ctx {
            c.emit("plugin:ui-open-panel", serde_json::json!({
                "plugin":   pname,
                "panel_id": panel_id,
            }));
        }
    ...
    ui.set("open_panel", fn_).map_err(|e| PluginCoreError::Plugin(e.to_string()))?;
    Ok(())
}
...
fn install_show_pipeline_run(ctx: &ApiCtx, lua: &Lua, ui: &Table) -> Result<()> {
    ...
    let fn_ = lua.create_function(move |_, run_id: String| {
        ...
        if let Some(ref c) = app_ctx {
            c.emit("plugin:ui-show-pipeline-run", serde_json::json!({
                "plugin": pname,
                "run_id": run_id,
            }));
        }
    ...
    ui.set("show_pipeline_run", fn_).map_err(|e| PluginCoreError::Plugin(e.to_string()))?;
    Ok(())
}
