...
pub(crate) fn install(ctx: &ApiCtx, lua: &Lua, ui: &Table) -> Result<()> {
    ...
    ui.set("tree", tree_table).map_err(|e| PluginCoreError::Plugin(e.to_string()))?;
    Ok(())
}
...
fn install_set(ctx: &ApiCtx, lua: &Lua, tree_table: &Table) -> Result<()> {
    ...
    tree_table.set("set", set_fn).map_err(|e| PluginCoreError::Plugin(e.to_string()))?;
    Ok(())
}
...
fn install_get(ctx: &ApiCtx, lua: &Lua, tree_table: &Table) -> Result<()> {
    ...
    tree_table.set("get", get_fn).map_err(|e| PluginCoreError::Plugin(e.to_string()))?;
    Ok(())
}
