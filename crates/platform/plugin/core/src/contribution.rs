...
///
/// Tauri-agnostic: the registry owning this coalescer hands a `&dyn AppCtx`
/// at `request` time, and the coalescer fans out via [`AppCtx::emit`].
...
///
/// The optional `app_ctx` slot is set once at app boot via
/// [`ContributionRegistry::install_app_ctx`] so frontend-emit calls
/// (`notify_changed`, `notify_containers_changed`) don't need to thread an
/// `Arc<dyn AppCtx>` through every call site. The registry is constructed
/// before the `AppCtx` is available (PluginHost::new is called from sync
/// `setup()` before `app.handle()` is materialised), so a deferred install
/// is the natural pattern.
#[derive(Default)]
pub struct ContributionRegistry {
    inner:   Arc<Mutex<RegistryInner>>,
    emitter: EventCoalescer,
    /// `dyn AppCtx` doesn't implement `Debug`, so the manual `Debug` impl
    /// below renders this as `<installed>` / `<none>` rather than dumping
    /// the trait object.
    app_ctx: Arc<Mutex<Option<Arc<dyn AppCtx>>>>,
}
...
impl std::fmt::Debug for ContributionRegistry {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let ctx_state = self
            ...
            .lock()
            .ok()
            .and_then(|g| g.as_ref().map(|_| "installed").or(Some("none")))
            .unwrap_or("poisoned");
        f.debug_struct("ContributionRegistry")
            .field("inner", &self.inner)
            .field("emitter", &self.emitter)
            .field("app_ctx", &ctx_state)
            .finish()
    }
}
...
impl ContributionRegistry {
    ...
    /// Attach the host `AppCtx`. Called once at app boot, before any plugin
    /// can fire a notify_*. Subsequent calls overwrite the previous handle
    /// (harmless — there's only ever one in production).
    pub fn install_app_ctx(&self, ctx: Arc<dyn AppCtx>) {
        if let Ok(mut g) = self.app_ctx.lock() {
            *g = Some(ctx);
        }
    }
    ...
    /// Snapshot the installed `AppCtx`, if any. Returns `None` when the
    /// registry was never paired with a host (test runs, the early-boot
    /// window before [`install_app_ctx`](Self::install_app_ctx)).
    fn ctx(&self) -> Option<Arc<dyn AppCtx>> {
    ...
    /// one frontend reload, not 50. No-op when no `AppCtx` has been installed.
    pub fn notify_changed(&self, point: &str) {
        if let Some(ctx) = self.ctx() {
            self.emitter.request(ctx, point);
        }
    }
    ...
    pub fn notify_containers_changed(&self) {
        if let Some(ctx) = self.ctx() {
            ctx.emit("arbor://containers-changed", serde_json::json!({}));
        }
    }
}
