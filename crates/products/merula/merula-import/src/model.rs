...
//!
//! Time is measured in **cycles** — merula's native unit (1 cycle = 1 bar by
...
pub struct Song {
    ...
    /// Tempo, in **cycles per second** (merula's `cps`).
    pub cps: f64,
    /// Total length in whole cycles (ceiling of the last note's end).
    pub len_cycles: u32,
}

/// Tunable knobs for the whole MIDI → `.merula` pipeline.
...
pub struct ImportOptions {
    ...
    /// `default_octave` for degree emission to be pitch-accurate (merula default
    /// is `4`, i.e. C4 = 60).
    pub ref_octave: i32,
}
