//! The typed AST for a `.merula` program.
//!
...
//! [`Pattern`](merula_pattern::prelude::Pattern), and the emitter prints it
//! back to source (`design/merula/editing-model.md` — keep the AST clean and
//! regular so emission is deterministic). It mirrors the grammar in
//! `design/merula/grammar.md` one-to-one.
//!
...
use merula_pattern::prelude::SourceSpan;
...
/// A whole `.merula` source file: a sequence of top-level items.
#[derive(Clone, Debug, PartialEq)]
pub struct Program {
    pub items: Vec<Item>,
}
...
pub enum ExprKind {
    ...
    /// `design/merula/grammar.md`). Evaluates to a single-note pattern, so it
...
/// slot/cycle, evaluated by inner-join (`design/merula/mini-notation.md`, level
