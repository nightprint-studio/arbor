//! Canonical, machine-readable reference for the merula DSL.
//!
...
//! once (`merula_lang_reference`) to feed autocomplete, hover docs, and the Docs
...
fn combinators() -> Vec<DslEntry> {
    vec![
        ...
        entry(
            ...
            "The output of a `.merula` file: a list of named channels (the mixer strips). Each is a `track(name, pattern)`.",
...
fn keywords() -> Vec<DslEntry> {
    vec![
        ...
        entry(
            ...
            "Bring selected top-level `fn`/`let` declarations from another `.merula` file into scope. The imported file's `tracks(…)` output is ignored.",
            vec![],
            "import { kick, snare } from \"lib/drums.merula\"",
        ),
        entry(
