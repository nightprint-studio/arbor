...
The `.merula` **language** layer of [merula](../../../design/merula), Arbor's live-coding music engine.
It turns source text into the pure [`Pattern`](../merula-pattern) algebra of **Fase 0** and back.
This is **Fase 1**: no audio, no scheduler — pure and unit-testable.

```text
source (.merula)  ──parse──▶  CST  ──walk──▶  AST  ──eval──▶  Pattern
                                              AST  ──emit──▶  source
```

## Layers (decoupled by design)

The layers never reach across each other (`design/merula/editing-model.md`):
...
- **Front end** — a [Tree-sitter](https://tree-sitter.github.io) grammar (`grammar.js`) plus a
  ...
  maximal munch, and `[]`/`<>` balancing (`design/merula/grammar.md §5`). Native byte ranges give every
  node a source span for free.
- **AST** (`ast.rs`) — the typed tree, the contract every other layer targets. Mirrors
  `design/merula/grammar.md` one-to-one; every node carries a `SourceSpan`.
...
`tree-sitter parse some.merula` (this one *does* compile the parser, so it needs the C toolchain).
...
Reach the API through the prelude (workspace convention): `use merula_lang::prelude::*;`.

Part of the merula crate stack: `merula-pattern` → **`merula-lang`** / `merula-audio` →
`merula-engine`. See [`design/merula/architecture.md`](../../../design/merula/architecture.md).
