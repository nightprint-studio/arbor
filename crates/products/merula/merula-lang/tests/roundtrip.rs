...
//!   - `parse` → `eval` on real `.merula` text produces the expected haps.

use std::rc::Rc;
use std::sync::Arc;

use merula_lang::prelude::{emit, evaluate, parse, EvalConfig, Item, NoImports, SilentLog};
use merula_pattern::prelude::{ControlMap, Hap, Pattern, TimeSpan};
...
fn import_statement_structure() {
    let program = parse(r#"import { kick, snare } from "lib/drums.merula""#).unwrap();
    match &program.items[0] {
        Item::Import(im) => {
            assert_eq!(im.names.len(), 2);
            assert_eq!(im.names[0].name, "kick");
            assert_eq!(im.path, "lib/drums.merula");
        }
