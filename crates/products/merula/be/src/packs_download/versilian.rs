...
//! VCSL — both by Versilian Studios, same on-disk format) into a playable merula
...
//! - **pitched** (filenames carry note names) → writes a `_merula.sfz` mapping the
...
//!
//! The SFZ uses exactly the opcodes the engine's loader understands (`merula-audio`'s
...
/// `root`, writing one `_merula.sfz` into each pitched sample folder as a side
...
pub fn generate(root: &Path) -> (String, usize) {
    let mut out = String::from("# Auto-generated Versilian registry (merula).\n\n");
...
/// Emit a pitched instrument: a `_merula.sfz` + `kind=sfz` entry per articulation.
fn emit_pitched(
    root: &Path,
    family: &str,
    inst: &str,
...
/// `s("…")`-friendly name (Dirt-Samples model — `s("anvil")`, `s("clap")`).
fn emit_unpitched(
    root: &Path,
    inst: &str,
    sets: &[(String, PathBuf)],
...
/// Whether a folder's wavs are **pitch-named** (most carry a note token).
...
/// Parse + map every wav in `dir` into an SFZ, written as `dir/_merula.sfz`.
fn write_set_sfz(root: &Path, dir: &Path) -> Option<String> {
    ...
    let mut sfz = String::from("// Auto-generated by merula from Versilian wavs.\n");
...
/// Parse a wav filename into (pitch, velocity-layer, round-robin).
...
/// Parse a note-name token (`A2`, `c4`, `Db3`, `F#5`) to a MIDI number (`c4 = 60`).
...
/// Map a pitch's velocity-layer keys to `[lovel, hivel]` bands.
...
/// Assign each sampled pitch a `[lokey, hikey]` range by splitting at midpoints.
...
/// The dotted-name family token for a top-level folder.
