//! General MIDI pack: convert a downloaded SoundFont (`.sf2`) into merula's wav +
//! SFZ form at **install time**.
//!
//! `rustysynth` is used here purely as a *parser* — we never run its synthesizer.
//! For every GM preset we walk its instrument regions, slice each referenced sample
//! out of the SoundFont's PCM into a `.wav`, and emit an `.sfz` mapping those
//! samples (key/velocity ranges, root key, tuning, loop, amp envelope) using exactly
//! the opcodes the engine's SFZ loader already understands. The result is a normal
//! `kind=sfz` registry — the audio engine stays sample/SFZ-based, no soundfont code
//! on the RT path.
//!
...
/// Convert the downloaded `.sf2` at `sf2_path` into wav + SFZ under `dir`. Returns
/// `(bytes_written, instrument_count, registry_rel)`. `is_cancelled` is polled per
/// preset so a mid-conversion Stop aborts; `emit_phase` reports coarse progress.
pub fn convert(
    dir: &Path,
    sf2_path: &Path,
    _pack: &Pack,
    is_cancelled: &dyn Fn() -> bool,
    emit_phase: &dyn Fn(&str, u64, u64),
    ...
    let mut registry = String::from("# Auto-generated General MIDI registry (merula).\n\n");
    ...
    for (idx, &pi) in order.iter().enumerate() {
        ...
        for pr in preset.get_regions() {
            ...
            for ir in inst.get_regions() {
                ...
                let rate =
                    headers.get(sid).map(|h| h.get_sample_rate()).unwrap_or(44_100).max(1) as u32;
        ...
        count += 1;
        emit_phase("extracting", idx as u64 + 1, total);
    }
...
/// The 128 General MIDI Level 1 program names (program 0–127), snake_cased to match
/// Strudel's `gm_*` instrument vocabulary.
