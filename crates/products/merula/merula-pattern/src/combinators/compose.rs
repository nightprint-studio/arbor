...
//! `tracks`).
//!
//! `par`/`seq`/`cat` are the merula host-language names; they are thin aliases
...
/// Alias of [`stack`] under the merula host name.
...
/// Alias of [`slowcat`] under the merula host name.
...
/// Alias of [`fastcat`] under the merula host name.
...
/// The output of a `.merula`: a list of named channels.
#[derive(Clone, Debug)]
pub struct Tracks<T> {
