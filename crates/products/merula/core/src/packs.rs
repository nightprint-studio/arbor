//! `packs` — downloadable sample **packs** (VSCO 2, VCSL, Dirt-Samples, drum
//! machines, General MIDI): the **read surface** + the lazy decode helper.
//!
...
//! listing for the sound bank + eval validator, the per-profile **active**
//! allow-list ([`active_packs`]), and the lazy [`load_subset_into`] the audio
//! thread decodes through.
//!
//! It lives in merula-core because the audio substrate ([`crate::audio_thread`])
//! decodes through [`load_subset_into`]. The two **read handlers** —
//! `merula_packs` (list) and `merula_pack_set_active` (toggle the allow-list) —
//! stay in merula-be and re-import [`list`] / [`installed_ids`] /
//! [`active_packs::set_active`] from here. The job-tracked **download / reindex /
//! delete** plumbing — and the `Layout::generate` tree-walkers — also stay in
//! merula-be (`packs_download`), reusing [`PACKS`], [`pack_dir`], [`PackStatus`],
//! and [`active_packs`], kept `pub` here. Ported from the shell's
...
/// that consumes them is implemented by merula-be's `packs_download` (install-time).
