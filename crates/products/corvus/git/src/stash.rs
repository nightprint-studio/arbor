...
pub fn stash_save(git: &GitCli, workdir: &std::path::Path, message: Option<&str>, include_untracked: bool) -> Result<StashEntry> {
    ...
    let branch = git_current_branch(git, workdir).unwrap_or_else(|| "HEAD".into());
    let short_sha = git_head_short_sha(git, workdir).unwrap_or_else(|| "unknown".into());
    let head_summary = git_head_summary(git, workdir).unwrap_or_else(|| "stash".into());
    ...
    if !out.status.success() {
        ...
        return Err(GitError::Other(format!("git stash push failed: {stderr}")));
    }

    // Resolve the OID of the newly created stash entry.
    let oid_out = git.command()
...
fn git_current_branch(git: &GitCli, workdir: &std::path::Path) -> Option<String> {
    let out = git.command()
...
fn git_head_short_sha(git: &GitCli, workdir: &std::path::Path) -> Option<String> {
    let out = git.command()
...
fn git_head_summary(git: &GitCli, workdir: &std::path::Path) -> Option<String> {
    let out = git.command()
...
fn run_stash_apply(
    ...
    if has_blockers {
        ...
        if !blocking.is_empty() {
            ...
            let (differing, identical) =
                filter_identical_stash_untracked(git, repo, index, &blocking, workdir);
    ...
    {
        return Err(GitError::StashNotFound(index));
    }
    ...
    Err(GitError::Other(format!("git stash {subcmd} failed: {stderr}")))
}

pub fn stash_apply(git: &GitCli, repo: &mut Repository, index: usize) -> Result<StashApplyResult> {
    let workdir = repo.workdir()
        .ok_or_else(|| GitError::Other("bare repository has no working directory".into()))?
        .to_path_buf();
    run_stash_apply(git, &workdir, repo, index, false)
}

pub fn stash_pop(git: &GitCli, repo: &mut Repository, index: usize) -> Result<StashApplyResult> {
    let workdir = repo.workdir()
        .ok_or_else(|| GitError::Other("bare repository has no working directory".into()))?
        .to_path_buf();
    run_stash_apply(git, &workdir, repo, index, true)
}

pub fn stash_drop(git: &GitCli, repo: &mut Repository, index: usize) -> Result<()> {
    let workdir = repo.workdir()
        .ok_or_else(|| GitError::Other("bare repository has no working directory".into()))?
        .to_path_buf();
    let stash_ref = format!("stash@{{{}}}", index);
...
pub fn stash_rename(repo: &mut Repository, index: usize, new_message: &str) -> Result<StashEntry> {
    ...
    let content = std::fs::read_to_string(&reflog_path)
        .map_err(|e| GitError::Other(format!("failed to read stash reflog: {e}")))?;
    ...
    std::fs::write(&tmp_path, new_content.as_bytes())
        .map_err(|e| GitError::Other(format!("failed to write temp reflog: {e}")))?;
    std::fs::rename(&tmp_path, &reflog_path).map_err(|e| {
        let _ = std::fs::remove_file(&tmp_path);
        GitError::Other(format!("failed to atomically replace stash reflog: {e}"))
    })?;
...
#[allow(dead_code)]
pub fn abort_stash_apply(git: &GitCli, workdir: &std::path::Path, snapshot: &dyn Fn(&Repository, &str)) -> Result<()> {
    abort_stash_apply_with_snapshot(git, workdir, None, snapshot)
}
...
pub fn force_stash_apply(
    ...
    let result = run_stash_apply(git, &workdir, repo, index, drop_on_success);
...
pub fn get_stash_file_content(
    ...
    use crate::encoding::{decode_with, detect, encoding_for_label};
...
fn collect_conflicted_files(repo: &Repository) -> Result<Vec<String>> {
    ...
    index.read(true).map_err(GitError::Git)?;
...
fn filter_identical_stash_untracked(
    ...
    let stash_oid_str = match git.command()
