//! `reflog` domain — read-only HEAD reflog projection, Tauri-free.
//!
//! Lifted verbatim from the shell `crate::git::reflog`; the only change is the
//! handle type, which is now this crate's own [`crate::repo::GitRepo`].
