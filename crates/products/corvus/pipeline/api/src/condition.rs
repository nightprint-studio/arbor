//! Conditions for pipeline `if/elif/else` blocks — the evaluation core.
//!
//! A condition is a small structured value tree (combinators + leaves):
//! leaf nodes compare two operands or check a single value, combinators wrap
//! arbitrary children. Operands are arbitrary strings — typically `${var}`
//! references — and are resolved via [`crate::vars::resolve_vars`] before the
//! leaf evaluates. A free-form [`Condition::Expr`] is parsed at evaluation time
//! by [`crate::condition_parser`].
//!
//! The structural `IfBlock` that carries `StepDef` branch bodies lives in the
//! host pipeline module (it's coupled to the step model); this crate holds only
...
pub fn evaluate(c: &Condition, ctx: &RunContext) -> bool {
    match c {
        ...
        Condition::Expr  { expr }       => match crate::condition_parser::parse(expr) {
