...
| `init_repo` / `open_repo` / `close_repo` (3) | the repo lifecycle via `corvus-git`. `init_repo`: git init + .gitignore/LICENSE/README + initial commit + optional push, fires `on_repo_init`; provider remote-creation resolves via the reverse-channel provider registry (`crate::provider::for_host`), the GitLab namespace-id lookup + initial-push HTTP-Basic credentials cross the reverse channel (`__gitlab_namespace_id`, `__git_credentials`). `open_repo` registers the tab + derives metadata + fires `on_repo_open`; `close_repo` deregisters + fires `on_repo_close` + runs the orphan-GC (`on_repo_deregistered`). All register the tab in corvus-be's own registry (`register_repo`); the launcher no longer proxies the lifecycle and keeps no repo registry |
