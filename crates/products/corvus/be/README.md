...
| `__repo_tab_path` / `__repo_open_tabs` | shell reads a tab's path / all open `(tab_id, path)` back — the launcher keeps no repo registry of its own, so studio, open-in-browser and the plugin host resolve repos through corvus-be |
...
| `stash_save` / `stash_apply` / `stash_pop` / `stash_drop` / `stash_rename` / `force_stash_apply` / `abort_stash_apply` / `list_stashes` / `list_graph_stash_refs` / `get_stash_file_content` / `write_workdir_file` (11) | the stash domain, via `corvus-git` (opens the repo by the pushed path); fires `corvus:stash_push` / `corvus:stash_pop` to the co-located host |
| `reset_to_commit` / `create_tag` / `delete_tag` | the reset + tags domain, via `corvus-git`; fires `corvus:tag_create` / `corvus:tag_delete` to the co-located host |
| `search_commits` | repo-wide commit search (read-only, no hooks), via `corvus-git` |
| `list_local_branches` / `list_remote_branches` / `list_tags` / `get_nearest_tag` / `list_merged_branches` / `list_merged_remote_branches` / `delete_branches` / `delete_remote_branches` / `rename_remote_branch` / `checkout_commit` (10) | the **link-free** slice of the branch domain, via `corvus-git`. `delete_branches` fires `corvus:branch_delete`, `rename_remote_branch` fires `corvus:branch_rename`, `checkout_commit` fires `corvus:checkout` (inline, co-located host). The remote-push delete/rename bind their `push` closure to the `__git_credentials` resolver. `create_branch` (alias registry), `checkout_commit_safe`, and every worktree-link-aware checkout/delete/rename stay in-process (the `WorktreeLinkRegistry` + checkout-sync orchestrator live in the shell's `AppState`) |
...
| `start_rebase` / `rebase_continue` / `rebase_abort` / `rebase_skip` / `get_rebase_todo` / `get_rebase_state` (6) | the rebase domain, via `corvus-git`; fires `corvus:rebase_start` / `corvus:rebase_abort` to the co-located host |
...
| `supports_security` / `fetch_security_summary` / `fetch_security_findings` / `export_security_report` (4) | the security-findings reads (tab-keyed via `provider_for_tab`); `fetch_security_summary` fires `corvus:security_summary_loaded`. `export_security_report` mints its job in the shell registry over the reverse channel (`JobHandle`), renders via the extracted `corvus-git-provider-api::security_export`, reads the plugin logo via `__branding_logo`, and emits `arbor://job-*` itself |
| `list_mrs` / `get_mr_detail` / `create_mr` / `get_mr_capabilities` / `probe_mr_feature` / `disable_mr_auto_merge` / `close_mr` / `reopen_mr` / `mark_mr_ready` / `add_mr_comment` / `get_mr_files` / `get_mr_commits` / `get_mr_commit_diff` / `get_merged_mr_hints` / `merge_mr` / `mr_start_conflict_resolution` (16) | the MR/PR domain; fires `corvus:mr_opened` (create), `corvus:mr_updated` (close/reopen/ready), `corvus:mr_merged` (merge). `merge_mr`'s GitHub branch-cleanup pushes `:refs/heads/<branch>` via the `__git_credentials` resolver; `mr_start_conflict_resolution` mints a job (`JobHandle`) + runs the extracted `corvus-git` merge-prep flow on a worker, streaming `arbor://mr-conflict-*` |
...
| `list_remotes` / `fetch_remote` / `push_branch` / `pull_branch` (4) | the network remote domain via `corvus-git`; git smart-HTTP credentials cross the reverse channel (`__git_credentials`); fires `corvus:fetch` / `corvus:push` / `corvus:pull`. `pull_branch` carries the full safe-pull flow (recovery snapshot → pre-pull stash → fetch/merge → re-apply) and streams `arbor://pull-progress` / `-done` |
| `list_commit_notes` / `check_note_remote_status` / `save_commit_note` / `delete_commit_note` / `push_note_namespace` (5) | the git-notes domain via `corvus-git`; fires `corvus:note_saved` / `corvus:note_deleted`. `push_note_namespace` pushes `refs/notes/*` over the shared `__git_credentials` resolver |
...
| `commit` / `stage_file` / `unstage_file` / `stage_all` / `unstage_all` / `discard_file` / `discard_all` / `stage_patch` / `cherry_pick` / `revert_commit` / `get_git_commit_template` (11) | the stage/index + commit domain (direct libgit2). `commit` fires the **vetoable** `corvus:pre_commit` (non-empty plugin return aborts with `"Commit blocked by plugin:\n{reason}"`) before mutating + `corvus:commit` after the repo handle drops — inline at the co-located host. `discard_*` snapshot via the pushed recovery `SnapshotPolicy` (`RecoveryKind::Discard`). `get_git_commit_template` expands `~` via `dirs` |
...
| `init_repo` / `open_repo` / `close_repo` (3) | the repo lifecycle via `corvus-git`. `init_repo`: git init + .gitignore/LICENSE/README + initial commit + optional push, fires `corvus:repo_init`; provider remote-creation resolves via the reverse-channel provider registry (`crate::provider::for_host`), the GitLab namespace-id lookup + initial-push HTTP-Basic credentials cross the reverse channel (`__gitlab_namespace_id`, `__git_credentials`). `open_repo` registers the tab + derives metadata + fires `corvus:repo_open`; `close_repo` deregisters + fires `corvus:repo_close` + runs the orphan-GC (`corvus:repo_deregistered`). All register the tab in corvus-be's own registry (`register_repo`); the launcher no longer proxies the lifecycle and keeps no repo registry |
...
fire-and-forget hooks (`corvus:stash_push` / `corvus:stash_pop`) directly to plugins
...
registry is a `Mutex` for that). Hooks (`corvus:security_summary_loaded`,
`corvus:mr_opened`, `corvus:mr_updated`) fire inline to the co-located host with
...
- **Per-registry state in `CorvusState`:** still pending — `branch` (the
  ...
  ticket-link cache. The `stage`/`commit` domain is **done** (the `corvus:pre_commit`
