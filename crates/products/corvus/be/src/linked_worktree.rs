...
//! member mutations fire `on_worktree_link_member_added` / `_removed` inline
...
/// Fires `on_worktree_link_member_added` ({link_id, repo_id}) after the lock drops.
#[arbor_rpc::handler]
...
/// Fires `on_worktree_link_member_removed` ({link_id, repo_id}) after the lock drops.
#[arbor_rpc::handler]
