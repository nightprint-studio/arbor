...
use crate::model::{LabConstraint, LabField};
use crate::names::{java_string, snake};
...
/// `java.util.List<String>` → `List`, `String[]` → `String[]`.
...
/// Every invalid value the field's constraints imply, in constraint order.
pub fn invalid_cases(field: &LabField) -> Vec<Case> {
...
fn violating(c: &LabConstraint, shape: Shape, simple: &str) -> Vec<Violating> {
    use SampleValue::*;
...
pub fn valid_value(field: &LabField) -> SampleValue {
    use SampleValue::*;
...
mod tests {
    ...
    fn field(type_name: &str, constraints: &[(&str, &[(&str, &str)])]) -> LabField {
        LabField {
            name: "value".into(),
