...
fn err(message: String, node: Node) -> Diagnostic {
    Diagnostic { message, severity: crate::check_id::CheckId::DuplicateInterface.severity().to_string(), code: crate::check_id::CheckId::DuplicateInterface.code().to_string(), start: node.start_byte(), end: node.end_byte() }
}
