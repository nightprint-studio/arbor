...
fn profile_big_interceptor() {
    ...
    pure!("syntax_errors", bennu_check::source::syntax::syntax_errors(root, &src));
    pure!("invalid_statements_nodes", bennu_check::source::statements::invalid_statements_nodes(&nodes, &src));
    pure!("missing_return_nodes", bennu_check::flow::returns::missing_return_nodes(&nodes, &src));
    pure!("return_statement_errors_nodes", bennu_check::flow::returns::return_statement_errors_nodes(&nodes, &src));
    pure!("switch_yield_errors_nodes", bennu_check::switching::switches::switch_yield_errors_nodes(&nodes, &src));
    pure!("switch_selector_errors_nodes", bennu_check::switching::switches::switch_selector_errors_nodes(&nodes, &src));
    pure!("duplicate_signatures_nodes", bennu_check::decls::duplicates::duplicate_signatures_nodes(&nodes, &src));
    pure!("redeclaration_errors_nodes", bennu_check::decls::redeclaration::redeclaration_errors_nodes(&nodes, &src));
    pure!("final_reassignment_errors_nodes", bennu_check::flow::finals::final_reassignment_errors_nodes(&nodes, &src));
    pure!("unreachable_code_nodes", bennu_check::flow::reachable::unreachable_code_nodes(&nodes, &src));
    pure!("declaration_errors_nodes", bennu_check::decls::declarations::declaration_errors_nodes(&nodes, &src));
    pure!("annotation_errors_nodes", bennu_check::annotation::annotations::annotation_errors_nodes(&nodes, &src));
    pure!("lambda_capture_errors_nodes", bennu_check::calls::lambdas::lambda_capture_errors_nodes(&nodes, &src));
    pure!("unused_imports (root)", bennu_check::source::imports::unused_imports(root, &src));
    pure!("duplicate_imports (root)", bennu_check::source::imports::duplicate_imports(root, &src));
    pure!("redundant_imports (root)", bennu_check::source::imports::redundant_imports(root, &src));
    pure!("version_errors_nodes", bennu_check::source::version::version_errors_nodes(root, &nodes, &src, 8));
    ...
    resolved!("unresolved_imports", _c, bennu_check::source::imports::unresolved_imports(root, &src, &resolver, true));
    resolved!("unknown_members_in", c, bennu_check::calls::members::unknown_members_in(root, &nodes, &src, &symbols, &resolver, &c));
