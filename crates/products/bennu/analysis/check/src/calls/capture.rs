...
//! §15.27.2). [`crate::calls::lambdas`] covers one direction — mutating a captured local from INSIDE a lambda
...
//! DEFINITELY-not-effectively-final case, exactly as [`crate::flow::finals`] does for `final` locals:
//!
...
fn check_scope(scope: Node, params: Option<Node>, bytes: &[u8], out: &mut Vec<Diagnostic>) {
    ...
    for (name, _reassign_site) in reassigns {
        ...
        out.push(Diagnostic {
            ...
            severity: crate::engine::check_id::CheckId::CapturedVariableNotFinal.severity().to_string(),
            code: crate::engine::check_id::CheckId::CapturedVariableNotFinal.code().to_string(),
