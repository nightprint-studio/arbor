...
/// never an undefined variable. Mirrors [`crate::typing::types::JAVA_LANG`] intent: a minimal resolver may not
...
pub fn undefined_var_errors_in(
    ...
    // `crate::support::nodes::generated_names`.
    ...
    for &n in nodes {
        ...
        if crate::support::resolve::inherited_member_type(&top_binary, name, resolver).is_some() {
            continue;
        }
        ...
        out.push(crate::engine::check_id::CheckId::UnresolvedSymbol.at(n, format!("Cannot resolve symbol `{name}`")));
    }
    out
}
...
mod tests {
    ...
    /// page of red on a class that compiles — see `crate::support::nodes::generated_names`.
    #[test]
