...
//! javac's `cannot find symbol: method foo()`. The companion of [`crate::calls::undefined_var`], which asks
...
//!
//!   * [`crate::calls::members`] (`unknown-member`) needs a receiver — it infers `x`'s type in `x.foo()` and
...
//!   * [`crate::calls::undefined_var`] (`unresolved-symbol`) judges bare *identifiers*, and explicitly
//!     excludes a method-invocation's name: `foo` in `foo()` is not a variable reference, and
//!     treating it as one would be wrong.
//!   * [`crate::hierarchy::static_access`] does walk bare calls, but only to ask whether the name is a known
...
//! [`crate::source::imports::unresolved_static_imports`] adjudicates the import lines that ARE written, and
//! here the defect is the line that isn't.
//!
//! ## Soundness — never a false positive
//!
//! Same contract as [`crate::calls::undefined_var`], and for the same reason: this runs continuously while
...
//!     ([`crate::support::nodes::generated_names`]) — under Lombok's `@Data` the legal bare call `getName()`
...
//!     `super.foo()` all belong to [`crate::calls::members`] / [`crate::hierarchy::super_method`], not here;
...
//!     ([`crate::support::scopes::scope_is_top_across_lambdas`]) — any of those classes can declare methods
//!     we did not gather.
//!
...
//!
//! **A local variable of the same name.** [`crate::calls::undefined_var`] skips on one because a local
...
use crate::support::nodes::generated_names;
...
/// The tree-driven core: mirrors [`crate::calls::undefined_var::undefined_var_errors_in`], iterating the
...
pub fn unresolved_call_errors_in(
    ...
    for &n in nodes {
        ...
        // A receiver makes this someone else's question — `crate::calls::members` infers the receiver's type
        ...
        out.push(
            crate::engine::check_id::CheckId::UnresolvedCall
