...
pub fn inherit_cycle_errors(source: &str, resolver: &dyn TypeResolver) -> Vec<Diagnostic> {
    ...
    with_parse(source, |root| {
        inherit_cycle_errors_in(&crate::engine::check::collect_nodes(root), source, &symbols, resolver)
    })
}
...
fn check_overrides(
    ...
    let Some(supers) = crate::support::supertypes::binaries_complete(n, bytes, symbols, resolver) else {
        return;
    };
    if supers.is_empty() {
