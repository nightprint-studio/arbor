...
//!
//! This is the case [`crate::hierarchy::inherit_cycle`]'s `@Override`-overrides-nothing check deliberately leaves
...
//! [`crate::flow::finals`] `final_override` matching) yet whose return type DEFINITELY isn't covariant.
//!
...
fn err(message: String, node: Node) -> Diagnostic {
    crate::engine::check_id::CheckId::CovariantReturn.at(node, message)
}
