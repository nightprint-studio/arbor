...
//! enclosing class's superclass/interface hierarchy. Resolver-backed sibling of [`crate::calls::members`]
...
//!
//! Conservative — never a false positive (see the module docs of [`crate::support::walk`]):
...
use crate::support::nodes::{is_class_type_node, simple_name};
...
/// Tree-driven core — mirrors [`crate::calls::members::unknown_members_in`]'s arg list so it wires the same
...
fn check_call(
    ...
    if !names.contains(method) {
        out.push(crate::engine::check_id::CheckId::UnresolvedSuperMethod.at(
            name,
            format!(
