...
//!
//! This mirrors [`crate::flow::finals::final_override_errors_in`]: we find, for a source method, the
...
//!   2. Only CHECKED exceptions are candidates ([`crate::throwing::checked_throw::is_checked`]), and only over a
//!      FULLY-KNOWN hierarchy ([`crate::support::walk::hierarchy_fully_known`]) so the checked verdict — and the
...
use crate::support::nodes::{child_of_kind, simple_name};

use crate::throwing::checked_throw::is_checked;
use crate::support::resolve::type_binary;
use crate::support::method_sig::{member_param_binaries, method_param_binaries};
use crate::support::nodes::{text};
use crate::support::walk::{for_each_supertype, hierarchy_fully_known, reaches};

/// The signature (mirrors [`crate::flow::finals::final_override_errors_in`]): iterate the shared pre-collected
...
fn check_type_widening(
    ...
    let Some(sup) = crate::support::supertypes::superclass(n, bytes) else { return };
    let Some(sup_bin) = crate::support::supertypes::binary(&sup.text, n, bytes, symbols, resolver) else {
        return;
    };
...
fn err(message: String, node: Node) -> Diagnostic {
    crate::engine::check_id::CheckId::CheckedExceptionWidening.at(node, message)
}
