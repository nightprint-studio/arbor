...
fn check_switch_fallthrough(switch: Node, _source: &str, out: &mut Vec<Diagnostic>) {
    ...
    for i in 0..groups.len() - 1 {
        ...
        if falls_off_end(last) {
            out.push(crate::engine::check_id::CheckId::SwitchFallthrough.at(group, FALLTHROUGH_MSG));
        }
    }
}
...
fn check_finally(finally_clause: Node, out: &mut Vec<Diagnostic>) {
    ...
    for h in hits {
        out.push(crate::engine::check_id::CheckId::FinallyAbrupt.at(h, FINALLY_MSG));
    }
}
