            ...
            if label_is_default(label, bytes) {
                if default_seen {
                    out.push(Diagnostic {
                        ...
                        severity: crate::engine::check_id::CheckId::DuplicateCaseLabel.severity().to_string(),
                        code: crate::engine::check_id::CheckId::DuplicateCaseLabel.code().to_string(),
            ...
            for cst in label.named_children(&mut lc) {
                ...
                if seen.contains(&key) {
                    out.push(Diagnostic {
                        ...
                        message: format!("Duplicate case label `{}`", crate::support::text::short(text)),
                        severity: crate::engine::check_id::CheckId::DuplicateCaseLabel.severity().to_string(),
                        code: crate::engine::check_id::CheckId::DuplicateCaseLabel.code().to_string(),
