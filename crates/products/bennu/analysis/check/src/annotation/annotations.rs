...
pub fn annotation_errors(root: Node, source: &str) -> Vec<Diagnostic> {
    annotation_errors_nodes(&crate::engine::check::collect_nodes(root), source)
}
        ...
        for (name, node) in annotations_of(n, bytes) {
            if allowed(&name, target) == Some(false) {
                out.push(Diagnostic {
                    ...
                    severity: crate::engine::check_id::CheckId::AnnotationNotApplicable.severity().to_string(),
                    code: crate::engine::check_id::CheckId::AnnotationNotApplicable.code().to_string(),
    ...
    for &n in nodes {
        ...
        for pair in n.named_children(&mut cw) {
            ...
            if seen.contains(&key) {
                out.push(crate::engine::check_id::CheckId::DuplicateAnnotationValue.at(
                    key_node,
