...
pub fn func_iface_errors_nodes(nodes: &[Node], source: &str) -> Vec<Diagnostic> {
    ...
    for &n in nodes {
        ...
        out.push(Diagnostic {
            ...
            severity: crate::engine::check_id::CheckId::NotAFunctionalInterface.severity().to_string(),
            code: crate::engine::check_id::CheckId::NotAFunctionalInterface.code().to_string(),
...
mod tests {
    ...
    fn errs(src: &str) -> Vec<String> {
        let tree = parse(src);
        func_iface_errors_nodes(&crate::engine::check::collect_nodes(tree.root_node()), src)
            .into_iter()
            .map(|d| d.message)
            .collect()
    }
