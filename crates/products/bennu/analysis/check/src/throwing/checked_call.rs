...
//!
//! This is the CALL-SITE sibling of [`crate::throwing::checked_throw`]: it shares the exact same
            ...
            "throw_statement" => {
                ...
                let binary = match thrown.kind() {
                    "object_creation_expression" => thrown
                        ...
                        .and_then(|name| crate::support::resolve::type_binary(name, &symbols, resolver)),
...
pub fn checked_call_errors_in(
    ...
    unhandled_calls_in(root, nodes, source, symbols, resolver, cache)
        .into_iter()
        .map(|u| {
            crate::engine::check_id::CheckId::UnhandledCheckedException.span(
                u.anchor.0,
                u.anchor.1,
                format!(
