...
pub fn generics_syntax_errors(root: Node, source: &str) -> Vec<Diagnostic> {
    generics_syntax_errors_nodes(&crate::engine::check::collect_nodes(root), source)
}
...
fn err(node: Node, message: impl Into<String>) -> Diagnostic {
    Diagnostic {
        ...
        severity: crate::engine::check_id::CheckId::IllegalGenericUsage.severity().to_string(),
        code: crate::engine::check_id::CheckId::IllegalGenericUsage.code().to_string(),
...
fn check_static_this_super(n: Node, out: &mut Vec<Diagnostic>) {
    ...
    while let Some(p) = cur {
        match p.kind() {
            ...
            "static_initializer" => {
                out.push(crate::engine::check_id::CheckId::StaticContextAccess.at(n, "Cannot use `this`/`super` in a static context"));
                return;
            }
            "method_declaration" => {
                if is_static_decl(p) {
