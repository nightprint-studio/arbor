...
fn err(message: String, node: Node, bytes: &[u8]) -> Diagnostic {
    ...
    Diagnostic { message, severity: crate::engine::check_id::CheckId::SpecialFileContent.severity().to_string(), code: crate::engine::check_id::CheckId::SpecialFileContent.code().to_string(), start, end }
}
