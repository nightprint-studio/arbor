...
pub fn invalid_statements(root: Node, source: &str) -> Vec<Diagnostic> {
    invalid_statements_nodes(&crate::engine::check::collect_nodes(root), source)
}
    ...
    for &n in nodes {
        ...
        if kind == "switch_expression" {
            if let Some(semi) = trailing_semicolon(n) {
                out.push(crate::engine::check_id::CheckId::EmptyStatement.at(
                    semi,
        ...
        if !is_statement_expression(kind) {
            ...
            out.push(Diagnostic {
                ...
                message: format!("`{}` is not a statement{hint}", crate::support::text::short(text)),
                severity: crate::engine::check_id::CheckId::NotAStatement.severity().to_string(),
                code: crate::engine::check_id::CheckId::NotAStatement.code().to_string(),
