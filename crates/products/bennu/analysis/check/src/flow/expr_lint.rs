...
fn self_assignment<'t>(assign: Node<'t>, bytes: &[u8]) -> Option<Diagnostic> {
    ...
    Some(crate::engine::check_id::CheckId::SelfAssignment.at(assign, format!("Self-assignment of `{lt}` has no effect")))
}
...
fn div_mod_by_zero<'t>(bin: Node<'t>, bytes: &[u8]) -> Option<Diagnostic> {
    ...
    Some(crate::engine::check_id::CheckId::DivisionByZero.at(bin, format!("{noun} by zero")))
}
...
fn empty_statements_in_block(block: Node, out: &mut Vec<Diagnostic>) {
    ...
    for ch in block.children(&mut c) {
        ...
        if ch.kind() == ";" {
            out.push(crate::engine::check_id::CheckId::EmptyStatement.at(ch, "Empty statement"));
        }
    }
}
...
fn string_reference_equality<'t>(bin: Node<'t>, bytes: &[u8]) -> Option<Diagnostic> {
    ...
    if left.kind() == "string_literal" || right.kind() == "string_literal" {
        return Some(crate::engine::check_id::CheckId::StringReferenceEquality.at(
            bin,
...
fn empty_if_body(stmt: Node) -> Option<Diagnostic> {
    ...
    (body.kind() == ";").then(|| {
        crate::engine::check_id::CheckId::at(
            crate::engine::check_id::CheckId::EmptyIfBody,
            body,
