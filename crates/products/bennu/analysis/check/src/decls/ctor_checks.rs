...
//! `ERROR` node — which the syntax pass ([`crate::source::syntax`]) already reports. A dedicated check here
...
fn check_method_named_like_class(method: Node, bytes: &[u8], out: &mut Vec<Diagnostic>) {
    ...
    if method_name == ty_name {
        out.push(Diagnostic {
            ...
            severity: crate::engine::check_id::CheckId::MethodNamedLikeConstructor.severity().to_string(),
            code: crate::engine::check_id::CheckId::MethodNamedLikeConstructor.code().to_string(),
