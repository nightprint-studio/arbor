...
//! [`crate::decls::declarations`]; this module is only the missing-body direction.
//!
...
//!     [`crate::source::syntax`], so we only act on a well-formed `method_declaration` that simply lacks a
...
fn err(node: Node, message: impl Into<String>) -> Diagnostic {
    Diagnostic {
        ...
        severity: crate::engine::check_id::CheckId::MissingMethodBody.severity().to_string(),
        code: crate::engine::check_id::CheckId::MissingMethodBody.code().to_string(),
