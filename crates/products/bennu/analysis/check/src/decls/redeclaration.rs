...
//! an enclosing type). Duplicate *method/constructor signatures* live in [`crate::decls::duplicates`].
//!
...
pub fn redeclaration_errors_in(root: Node, source: &str) -> Vec<Diagnostic> {
    redeclaration_errors_nodes(&crate::engine::check::collect_nodes(root), source)
}
...
fn err(message: String, node: Node) -> Diagnostic {
    Diagnostic {
        message,
        severity: crate::engine::check_id::CheckId::DuplicateDeclaration.severity().to_string(),
        code: crate::engine::check_id::CheckId::DuplicateDeclaration.code().to_string(),
