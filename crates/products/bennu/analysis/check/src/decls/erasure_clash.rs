...
//! [`crate::decls::duplicates`] — which compares raw parameter-type text — misses them). Example the compiler
//! rejects: `void f(List<String> a)` and `void f(List<Integer> a)` both erase to `f(List)`; likewise
//! `f(List<String>)` vs the raw `f(List)`.
//!
//! This is the erasure-aware SIBLING of [`crate::decls::duplicates`]: that check flags pairs whose parameter
...
pub fn erasure_clash_errors_in(root: Node, source: &str) -> Vec<Diagnostic> {
    erasure_clash_errors_nodes(&crate::engine::check::collect_nodes(root), source)
}
        ...
        for j in 0..i {
            ...
            if erased_i == erased_j && members[i].raw != members[j].raw {
                ...
                out.push(Diagnostic {
                    ...
                    severity: crate::engine::check_id::CheckId::ErasureClash.severity().to_string(),
                    code: crate::engine::check_id::CheckId::ErasureClash.code().to_string(),
