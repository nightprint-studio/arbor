...
pub fn type_binary(text: &str, symbols: &FileSymbols, resolver: &dyn TypeResolver) -> Option<String> {
    ...
    // shape of a string.
    type_binary_in(text, crate::support::type_scope::TypeScope::Unknown, symbols, resolver)
}

/// [`type_binary`], told exactly WHERE the name is written — the one entry point that takes a
/// [`TypeScope`](crate::support::type_scope::TypeScope). The two wrappers above pick a scope for their
/// callers; anything with a real position should come through here.
pub fn type_binary_in(
    text: &str,
    scope: crate::support::type_scope::TypeScope,
    symbols: &FileSymbols,
    ...
    bennu_java::prelude::resolve_written_type(
        text,
        &crate::support::type_scope::FileScope { symbols, resolver, scope },
    )
    .resolved()
...
/// [`crate::support::type_scope::FileScope::owner`]. Prefer this wherever the caller holds the node.
...
pub(crate) fn static_receiver_binary(
    ...
    if is_file_field || crate::support::scopes::resolves_as_local(head, outermost_type(obj)?, bytes) {
        return None;
    }
...
/// [`TypeScope`](crate::support::type_scope::TypeScope)).
pub fn enclosing_scope(
    node: tree_sitter::Node,
    bytes: &[u8],
    symbols: &FileSymbols,
) -> crate::support::type_scope::TypeScope {
    match bennu_java::prelude::enclosing_type_fqn(&node, bytes, symbols) {
        Some(fqn) => crate::support::type_scope::TypeScope::Inside(fqn.replace('.', "/")),
        None => crate::support::type_scope::TypeScope::CompilationUnit,
    }
}
