...
fn bennu_generate_hint(
    ...
    Ok(accessor.or_else(|| declaration_name_hint(&args.source, args.offset, args.case_sensitive)))
}
...
fn declaration_name_hint(source: &str, offset: usize, case_sensitive: bool) -> Option<AccessorHint> {
    let found = bennu_java::prelude::declaration_name_at(source, offset, case_sensitive)?;
...
mod tests {
    ...
    fn a_name_with_nothing_typed_is_drawn_whole_and_inserted_at_the_caret() {
        ...
        let hint = declaration_name_hint(src, src.len(), false).expect("a hint");
    ...
    fn no_hint_where_no_declaration_is_being_named() {
        ...
        assert!(declaration_name_hint(src, src.len(), false).is_none());
    }
}
