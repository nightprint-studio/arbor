...
mod tests {
    ...
    #[test]
    fn a_library_member_jump_lands_on_the_member() {
        let stub = "// Decompiled from bytecode — no source attached. Signatures only (method bodies\n\
                    // are not present in a .class file). Generated by Bennu.\n\n\
                    package org.acme.handler;\n\n\
                    public class HandlerFunctions {\n\
                    \x20   public static HandlerFunction<ServerResponse> http();\n\
                    \x20   public static HandlerFunction<ServerResponse> https(URI arg0);\n\
                    }\n";
        let at = member_jump_offset(
            stub,
    ...
    /// and the owner-scoped search — right for a rename — then finds nothing and the jump used to
    /// land on the class header. The file was opened because it declares the thing; landing on it
    /// beats landing on its first line.
    #[test]
    fn a_member_declared_in_a_nested_type_is_still_found() {
        let src = "package org.acme;\n\
                   public abstract class HandlerFunctions {\n\
                   \x20   public interface Factory {\n\
                   \x20       static HandlerFunction<ServerResponse> http() { return null; }\n\
                   \x20   }\n\
                   }\n";
        let at = member_jump_offset(
            src,
    ...
    /// A name that is nowhere in the file still lands on the type, never on byte zero.
    #[test]
    fn an_unknown_member_lands_on_the_type_declaration() {
        let src = "package org.acme;\npublic class Foo {\n    void bar() {}\n}\n";
        let at = member_jump_offset(
            src,
            "org/acme/Foo",
