...
fn static_import_type(imports: &[bennu_java::prelude::Import], name: &str) -> Option<String> {
    ...
    pub fn library_binary(&self, source: &str, name: &str) -> Option<String> {
        ...
        let binary = if name.contains('.') {
            ...
            // What the FILE binds the name to comes first — its imports, its package, its
            // wildcards, `java.lang`, in Java's order. Asking the resolver first is what broke
            // go-to on a star-imported library type: `resolve_simple_name` consults the project's
            // simple-name map before the file's wildcards, so a project class sharing the simple
            // name, in any package, answered instead — and was then refused below as project code,
            // leaving nothing to open. The buffer's own package is in it too, which is what a
            // library source view needs most: `ApplicationContext extends MessageSource,
            // ApplicationEventPublisher` imports neither.
            //
            // Confirmed against the classpath rather than assumed: the caret can be on any
            // capitalised word, and a binary nobody can decode turns "go-to found nothing" into
            // "go-to opened an empty stub".
            bennu_java::prelude::bind_simple_name(
                name,
                symbols.package.as_deref(),
                &symbols.imports,
                &|b| resolver.members_of(b).is_some(),
            )
            // Then the project-wide answer, for a name the file's own scope does not reach.
            .or_else(|| resolver.resolve_simple_name(name, &symbols.imports))
            // A name written only inside a **static** import resolves through no ordinary import
            // entry: `import static a.b.C.http;` binds `http`, not `C`, so neither the type nor the
            // member is a simple name the resolver can look up. The import itself says what they are.
            .or_else(|| Self::static_import_type(&symbols.imports, name))?
        };
...
#[cfg(test)]
// The same-package lookup that used to be tested here is `bennu_java::prelude::bind_simple_name`
// now, and its cases moved with it (`a_sibling_binds_only_when_it_is_there`).
mod source_render_tests {
    use super::*;
