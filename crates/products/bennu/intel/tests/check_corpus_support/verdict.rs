...
//!   the key maps to, **miss** when none does, **not covered** when `bennu_check::javac` maps the key
//!   to no check at all;
...
/// The Bennu codes `bennu_check::javac` maps `key` to; empty when it maps it to no check.
