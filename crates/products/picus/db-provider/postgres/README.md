...
| `catalog` | the `pg_catalog` queries — faster than `information_schema`, and the only place trigger bitmasks and expression-index columns exist |
| `sql` | pure helpers: quoting, statement classification, trigger-bit decoding. Fully unit-tested without a database |
