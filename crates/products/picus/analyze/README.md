...
Two axes, and keeping them apart is the point. `CONS001` and `CONS004` compare **one branch
against the other**; `CONS002` and `CONS003` compare **one branch's initialisation against its own
...
|---|---|---|---|
| `CONS001` | blocking | An object is touched by one dialect's branch at some role and by the other's not at all | The object is a **package** (Oracle-only); the branch has **no dialect**; the role exists in only one branch |
| `CONS002` | blocking | A datum the `init`/`data` folders write is written by no `update` script of the same branch | The updates never load that table at all; a value is **computed**; a row has no column list; the difference is only in a column one half never writes |
| `CONS003` | blocking | A datum an `update` script writes is written by no `init`/`data` script of the same branch | Same three, mirrored |
| `CONS004` | blocking | Both branches load the same table at the same role, with different columns or different rows | Any value is **computed** (`SYSDATE`, `now()`, a sequence) — the rows are then incomparable, not different |
| `DIA001` | blocking | A statement uses a construct belonging to the other dialect | The file was parsed as something other than its branch's dialect; the same construct repeats in one statement (one finding, not four) |
...
| `DUP002` | review | The same object is **created** twice in one branch | The two creations are in **different branches** (that is the point of the repository); one of them is an `ALTER`; they are a package spec and its body |
...
cross-branch rules use, so `1.50` and `1.5` are one number and `'X'` and `'x'` are two strings.
...
`CONS001` reports **once per object per branch**, at the most significant role where the gap
shows. A table absent from a whole branch would otherwise be three rows — init, data, update —
for one problem.
...
wants is `alsoAt`, which points at the branch that does do it.
...
table created once per branch, a create followed by an alter, two rows stamped with `SYSDATE`, an
