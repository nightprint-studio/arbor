...
//!
//! Tauri-free and shell-free: operates only on this crate's own
//! `SecuritySummary` / `SecurityFinding` DTOs plus `std`. The Arbor logo is
//! inlined here (`LOGO_SVG`) so the report header renders without a dependency
//! on the shell's stats module.
...
use crate::security::{
    FindingState, RiskScore, SecurityFinding, SecuritySummary, Severity, VulnTimeSeries,
};

// ---------------------------------------------------------------------------
// Arbor logo — inline SVG, IDs prefixed with "al-" to avoid collisions when
// embedded alongside other SVG charts in the same HTML document. Kept in sync
// with `corvus_git::stats::LOGO_SVG` by convention.
// ---------------------------------------------------------------------------
...
fn render_gauge(rs: &RiskScore) -> String {
