...
//! (REST coverage is sparse and inconsistent). We aggregate the project's
//! vulnerability connection + severity counts + day-by-day time series into
//! the shared `SecuritySummary` / `SecurityFinding` shapes.
//!
//! Folds together the old delegate (`gitlab/security.rs`: RepoRef →
//! project-path plumbing, token gating, host-side `apply_filters`) and the
//! GraphQL bodies from `security_impl.rs` (GitLab functions only). Error
//! strings are preserved verbatim and wrapped via `classify`; auth uses the
//! injected session, and the endpoint is `{http.base()}/api/graphql`.

use chrono::{Duration, Utc};

use corvus_git_provider_api::prelude::*;

use crate::http::{classify, GitlabHttp};
...
/// GitLab projects are addressed by their full path (`group/sub/project`). The
/// security domain only ever passes this value as the GraphQL `fullPath`
/// variable or into the `web_url` builder — both want the raw, un-encoded
/// path (matching the old `format!("{base_url}/{project_path}/-/security/dashboard")`),
/// so no `percent_encode_slash` is applied here.
...
/// vulnerability data to the current user.
///
/// Delegate gate: without credentials we can't probe, so we treat it as "not
/// supported" (`Ok(false)`) and let the gating UI hide the entry rather than
/// render a misleading "no findings" state.
pub(crate) async fn supports_security(
    ...
    gitlab_supports_security(http, path).await
}
...
pub(crate) async fn fetch_security_summary(
    ...
    let path = project_path(repo);
    fetch_gitlab_security_summary(http, path, range_days).await
}
...
pub(crate) async fn fetch_security_findings(
    ...
    let path = project_path(repo);
    let (findings, _truncated) = fetch_gitlab_security_findings(http, path, &filters).await?;
...
async fn gitlab_supports_security(
    http: &GitlabHttp,
    project_path: &str,
) -> Result<bool, ProviderError> {
    ...
    let url = format!("{}/api/graphql", http.base());
    let resp = http
        .send(|s| {
            http.client()
                .post(&url)
    ...
    if !resp.status().is_success() {
        ...
        return Err(classify(format!("GitLab security probe {status}: {body}")));
    }

    let v: serde_json::Value = resp
        .json()
        .await
        .map_err(|e| classify(format!("GitLab security probe parse: {e}")))?;
    ...
    let counts_present = v
        .pointer("/data/project/vulnerabilitySeveritiesCount")
        .map(|x| !x.is_null())
        .unwrap_or(false);
    let vulns_present = v
        .pointer("/data/project/vulnerabilities")
        .map(|x| !x.is_null())
        .unwrap_or(false);
...
async fn fetch_gitlab_security_findings(
    http: &GitlabHttp,
    project_path: &str,
    filters: &SecurityFilters,
    ...
    let report_types: Vec<String> = filters
        .report_types
        .iter()
    ...
    loop {
        ...
        if !resp.status().is_success() {
            ...
            return Err(classify(format!("GitLab vulnerabilities {s}: {b}")));
        }

        let v: serde_json::Value = resp
            .json()
            .await
            .map_err(|e| classify(format!("GitLab vulnerabilities parse: {e}")))?;

        if let Some(errs) = v.get("errors") {
            return Err(classify(format!("GitLab GraphQL errors: {errs}")));
        }

        let nodes = v
            .pointer("/data/project/vulnerabilities/nodes")
...
async fn fetch_gitlab_security_summary(
    http: &GitlabHttp,
    project_path: &str,
    range_days: u32,
) -> Result<SecuritySummary, ProviderError> {
    let counts = fetch_gitlab_severity_counts(http, project_path).await?;
    ...
    let time_series = fetch_gitlab_time_series(http, project_path, range_days)
        .await
        .ok()
        .flatten();
    ...
    let web_url = Some(format!(
        "{}/{project_path}/-/security/dashboard",
        http.base()
    ));
...
async fn fetch_gitlab_severity_counts(
    http: &GitlabHttp,
    project_path: &str,
) -> Result<SeverityCounts, ProviderError> {
    ...
    if !resp.status().is_success() {
        let s = resp.status();
        let b = resp.text().await.unwrap_or_default();
        return Err(classify(format!("GitLab severity counts {s}: {b}")));
    }

    let v: serde_json::Value = resp
        .json()
        .await
        .map_err(|e| classify(format!("GitLab severity counts parse: {e}")))?;
...
async fn fetch_gitlab_time_series(
    ...
    project_path: &str,
    range_days: u32,
) -> Result<Option<VulnTimeSeries>, ProviderError> {
    ...
    let v: serde_json::Value = resp
        .json()
        .await
        .map_err(|e| classify(format!("GitLab time series parse: {e}")))?;
    ...
    let points: Vec<TimePoint> = nodes
        .iter()
        .map(|n| TimePoint {
