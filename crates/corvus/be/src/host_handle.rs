...
//! RPC adapter ([`crate::plugin_rpc::CorvusRpcCtx`]) reads it via [`host`]. The
//! host is kept **out** of `CorvusState` on purpose — it is mlua-coupled and
//! owned by `main`, while `CorvusState` stays transport-only, so threading it
//! through the state would drag the whole `plugin-core` dependency into the
//! `corvus-core` crate. Publishing it here keeps the wiring a single line.
