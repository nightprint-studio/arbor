//! Alias resolution + branch-lifecycle helpers for worktree links.
//!
...
pub fn expected_branch_for_member(link: &WorktreeLink, repo_id: &str) -> Option<String> {
    ...
    if t.initiator_repo_id == repo_id {
        return Some(t.branch.clone());
    }
    Some(resolve_target_branch(link, &t.initiator_repo_id, &t.branch, repo_id))
}

/// Cleanup alias references when a branch is deleted. Removes entries pointing to
/// `(repo_id, branch)` across all links; groups falling below 2 members are
/// dropped. Returns the number of alias entries removed.
...
/// Smart rename: update alias entries `(repo_id, old)` → `(repo_id, new)`. If
/// after the rename all members of a group share the same branch name the group
/// is redundant and gets removed. Returns the number of groups affected.
pub fn on_branch_renamed(links: &mut [WorktreeLink], repo_id: &str, old_name: &str, new_name: &str) -> usize {
...
/// True when every entry in the group references the same branch name — the
/// alias mapping is then a no-op and can be removed.
...
/// Returns the link name if creating `(repo_id, branch)` would conflict with an
/// existing alias group target. Used by `create_branch` to refuse names reserved
/// by an alias mapping.
