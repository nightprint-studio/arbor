...
//! no gravatar exists).
//!
//! Lifted verbatim from the shell `crate::git::svg_export`; the only change is
//! the graph DTO path (`crate::git::graph` → this crate's [`crate::graph`]).
//! Error reporting stays the plain `Result<(), String>` the job layer expects.
...
use crate::graph::{EdgeType, GraphData, RefType};
...
fn edge_path(edge: &crate::graph::GraphEdge) -> String {
