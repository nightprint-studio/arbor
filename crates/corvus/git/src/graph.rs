//! `graph` domain — pure git logic, Tauri-free.
//!
//! The commit-graph lane algorithm (`load_graph` / `load_graph_for_file`),
//! single-commit detail (`get_commit_detail`), the repo-file-tree / last-touch
//! scan (`get_repo_files` / `get_files_last_commit` / `get_repo_file_tree`),
//! and the graph DTOs (`GraphData`, `CommitNode`, `GraphEdge`, …). Lifted
//! verbatim from the shell `crate::git::graph`; only the couplings the crate
//! refuses are swapped — `AppError` becomes the crate's [`GitError`].
//!
//! The graph itself only reads through `&Repository` (no git program needed),
//! so there is no [`GitCli`] parameter here — unlike the CLI-driven domains.
//! Stash markers are NOT collected here: the shell wrapper repoints to
//! [`crate::stash::collect_stash_refs`] after the walk (the `stashes` field is
//! left empty by the crate, exactly as before).
...
pub struct GraphData {
    ...
    pub offset: usize,
    /// Stashes anchored by their first-parent commit OID. Populated by the
    /// shell wrapper's `get_graph` (via [`crate::stash::collect_stash_refs`]);
    /// `load_graph*` leave this empty since they don't need mutable repo access.
    #[serde(default)]
    pub stashes: Vec<crate::stash::StashRef>,
}
...
fn load_graph_inner(repo: &Repository, offset: usize, limit: usize, path_filter: Option<&str>) -> Result<GraphData> {
    ...
    for (local_row, oid) in page.iter().enumerate() {
        ...
        let commit = repo
            .find_object(*oid, None)
            .and_then(|obj| obj.peel_to_commit())
            .map_err(|_| GitError::CommitNotFound(oid_str.clone()))?;
...
pub fn get_commit_detail(repo: &Repository, oid_str: &str) -> Result<CommitDetail> {
    let oid = Oid::from_str(oid_str).map_err(|_| GitError::CommitNotFound(oid_str.to_string()))?;
...
mod tests {
    ...
    // ── Pure-logic unit tests (no Repository needed) ─────────────────────────

    fn local(name: &str) -> RefLabel {
        RefLabel { name: name.into(), ref_type: RefType::LocalBranch, is_current: false }
    }
    fn remote(name: &str) -> RefLabel {
        RefLabel { name: name.into(), ref_type: RefType::RemoteBranch, is_current: false }
    }
    fn tag(name: &str) -> RefLabel {
        RefLabel { name: name.into(), ref_type: RefType::Tag, is_current: false }
    }

    #[test]
    fn dedupe_drops_remote_covered_by_local() {
        // origin/develop is redundant when a local `develop` sits on the same
        // commit; origin/feature (no local counterpart) survives, and tags are
        // never touched.
        let mut refs = vec![
            local("develop"),
        ...
        assert_eq!(names, vec!["develop", "origin/feature", "v1.0"]);
    }

    #[test]
    fn dedupe_noop_without_local_branches() {
        // No local branches → nothing to dedupe against, remotes are kept.
        let mut refs = vec![remote("origin/main"), tag("v2")];
        let before = refs.len();
        dedupe_refs(&mut refs);
    ...
    #[test]
    fn dedupe_keeps_remote_with_bare_name() {
        // A remote ref with no '/' (degenerate) falls back to its own name for
        // the comparison; with no matching local it must be kept.
        let mut refs = vec![local("main"), remote("weirdremote")];
        dedupe_refs(&mut refs);
        let names: Vec<&str> = refs.iter().map(|r| r.name.as_str()).collect();
        assert!(names.contains(&"weirdremote"));
        assert!(names.contains(&"main"));
    }

    #[test]
    fn edge_type_classifies_directions() {
        // Equal lanes → straight regardless of merge flag.
        assert_eq!(edge_type(2, 2, false), EdgeType::Straight);
        assert_eq!(edge_type(2, 2, true), EdgeType::Straight);
        // from > to (going left): fork vs merge variant.
        assert_eq!(edge_type(3, 1, false), EdgeType::ForkLeft);
        assert_eq!(edge_type(3, 1, true), EdgeType::MergeLeft);
        // from < to (going right): fork vs merge variant.
        assert_eq!(edge_type(1, 3, false), EdgeType::ForkRight);
        assert_eq!(edge_type(1, 3, true), EdgeType::MergeRight);
    }

    #[test]
    fn edge_type_serde_snake_case() {
        // The frontend matches on these snake_case wire strings.
        assert_eq!(serde_json::to_string(&EdgeType::ForkLeft).unwrap(), "\"fork_left\"");
        assert_eq!(serde_json::to_string(&EdgeType::MergeRight).unwrap(), "\"merge_right\"");
        assert_eq!(serde_json::to_string(&EdgeType::SquashMerge).unwrap(), "\"squash_merge\"");
        assert_eq!(serde_json::to_string(&RefType::RemoteBranch).unwrap(), "\"remote_branch\"");
    }

    #[test]
    fn lanes_alloc_secondary_never_returns_zero() {
        // Slot 0 stays reserved for the first-parent chain.
        let mut lanes = Lanes::new();
        let a = lanes.alloc_secondary("a");
        let b = lanes.alloc_secondary("b");
        assert!(a >= 1 && b >= 1 && a != b);
    }

    #[test]
    fn lanes_try_set_respects_existing_placement() {
        // Once an OID has a lane, try_set must not move it.
        let mut lanes = Lanes::new();
        let first = lanes.try_set(2, "x");
        let again = lanes.try_set(5, "x");
