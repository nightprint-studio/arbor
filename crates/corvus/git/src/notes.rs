//! `notes` domain — pure git logic, Tauri-free.
//!
//! Git-notes queries and mutations (list across every `refs/notes/*` namespace,
//! remote-status comparison, create/overwrite, delete) lifted verbatim from the
//! shell `crate::git::notes`. The only coupling swapped is the error type:
//! `AppError` becomes the crate's [`GitError`] (variant-for-variant, so the
//! shell maps it back with byte-identical wire strings).
//!
//! Everything here is plain `git2` — there is no git-program global, no recovery
//! snapshot, no app-config to inject, so unlike `stash`/`merge` these functions
//! take **no** [`GitCli`](crate::cli::GitCli).
//!
//! NOT moved (stays shell-side): the credential-coupled push of a notes
//! namespace (`push_note_namespace`) — it lives in the broker handler, delegates
//! to `crate::git::remote::push`, and is out of scope for this Tauri-free crate.
...
pub fn delete_note(
    ...
    repo.note_delete(oid, Some(&notes_ref), &sig, &sig)
        .map_err(|e| GitError::Other(format!("delete note: {e}")))?;
    Ok(())
}
...
fn parse_oid(s: &str) -> Result<Oid> {
    Oid::from_str(s).map_err(|_| GitError::CommitNotFound(s.to_string()))
}
...
mod tests {
    ...
    /// `NoteRemoteStatus` is the wire enum the frontend matches on — the
    /// serde tags must stay snake_case across the extraction.
    #[test]
    fn note_remote_status_serde_snake_case() {
        assert_eq!(
            serde_json::to_string(&NoteRemoteStatus::LocalOnly).unwrap(),
            "\"local_only\"",
        );
        assert_eq!(
            serde_json::to_string(&NoteRemoteStatus::InSync).unwrap(),
            "\"in_sync\"",
        );
        assert_eq!(
            serde_json::to_string(&NoteRemoteStatus::OutOfSync).unwrap(),
            "\"out_of_sync\"",
        );
        assert_eq!(
            serde_json::to_string(&NoteRemoteStatus::Unknown).unwrap(),
            "\"unknown\"",
        );
    }

    /// A malformed OID string must yield `CommitNotFound` (mapped to
    /// `AppError::CommitNotFound` shell-side) rather than a generic error —
    /// the frontend keys off this typed variant.
    #[test]
    fn parse_oid_rejects_non_hex() {
        match parse_oid("not-a-valid-oid") {
            Err(GitError::CommitNotFound(s)) => assert_eq!(s, "not-a-valid-oid"),
            other => panic!("expected CommitNotFound, got {other:?}"),
        }
    }

    /// A well-formed 40-char hex OID parses even if no such commit exists —
    /// `parse_oid` validates shape only, lookups happen later.
    #[test]
    fn parse_oid_accepts_valid_hex() {
        let valid = "0123456789abcdef0123456789abcdef01234567";
        assert!(parse_oid(valid).is_ok());
    }

    /// `list_notes` on a repository with no `refs/notes/*` refs returns an
    /// empty vec, never an error.
    #[test]
    fn list_notes_empty_when_no_notes_refs() {
        let dir = std::env::temp_dir().join(format!(
            "corvus-git-notes-test-{}-{}",
            std::process::id(),
        ...
        let valid = "0123456789abcdef0123456789abcdef01234567";
        let notes = list_notes(&repo, valid).expect("list_notes should not error");
