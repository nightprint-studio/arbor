//! `remote` domain — list/fetch/push/pull, Tauri-free.
//!
//! Lifted from the shell `crate::git::remote`; the two couplings the crate
//! refuses are injected:
//!
//!   * **Credential resolution** (OS keyring via `credential_store`) is passed in
//!     as a `credential_resolver` closure. The crate never touches the keyring;
//!     it only asks "give me the stored (user, pass) for this URL, if any". The
//!     shell binds this to `crate::auth::credential_store::resolve_credentials`.
//!   * **The git binary** is the explicit [`GitCli`] (used by `pull` for the
//!     fast-forward / merge shell-out), passed in by the caller.
//!
//! Everything else (git2 fetch/push/pull plumbing, the credentials callback,
//! per-ref rejection capture, manual remote-tracking ref update) is pure and
//! moves verbatim.
...
// ---------------------------------------------------------------------------
// Injected credential resolver
// ---------------------------------------------------------------------------
//
// The keyring lives shell-side (OS keychain access cannot move into a
// Tauri-free crate). The caller injects a resolver that maps a remote URL to an
// optional stored `(username, password)`; `Err(String)` carries a lookup
// failure message which is logged and treated as "no credentials" (same
// behaviour as before the extraction).
//
...
pub fn fetch(
    ...
    let stored = resolve_stored(&url, credential_resolver);
...
pub fn pull(
    ...
    let branch_name = head
        .shorthand()
        .ok_or_else(|| GitError::Other("detached HEAD".into()))?;
    ...
    let fetch_head_id = {
        ...
        match from_config {
            ...
            None => repo
                ...
                .map_err(|_| GitError::Other(format!(
    ...
    if merge_analysis.is_fast_forward() {
        ...
        let workdir = repo.workdir().ok_or_else(|| {
            GitError::Other("bare repository has no working directory".into())
        })?;

        // `git.command()` already applies `no_window()`, so the CLI shell-out
        // stays console-less on Windows just like the pre-extraction code.
        let out = git.command()
    ...
    if merge_analysis.is_normal() {
        ...
        return Err(GitError::Other(format!("git merge failed: {msg}")));
    }
