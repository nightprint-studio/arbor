//! The original decode/encode helpers, unchanged.
//!
//! Everything here predates the provenance-carrying detection in
//! [`super::detect`] and is consumed today by `corvus-git`, the studio
//! backends (FROZEN F16) and the shell. Its behaviour is **frozen**: the
//! functions here answer "what does this crate have always done", and the
//! characterisation tests in [`super::tests`] pin that answer. New callers
//! that want the guess to be explainable should use
//! [`super::detect_in_context`] instead.
...
///
/// Note that step 2 also claims every **pure-ASCII** file for UTF-8, which is
/// a guess rather than evidence — ASCII is equally valid windows-1252. The
/// newer [`super::evidence`] chain reports that case as ambiguous instead;
/// this function keeps the old answer so existing callers don't shift.
...
/// behaviour.
///
/// Lossy by design: a character the target encoding cannot represent is
/// replaced by `encoding_rs`' HTML-style numeric reference. Use
/// [`super::encode_for_disk_strict`] when silent corruption is not acceptable.
...
/// byte-order mark.
///
/// Callers that need a lossless round-trip have to remember whether the
/// original file had one and re-prepend at save — note that
/// `decode_without_bom_handling` (used by [`decode_bytes`] and
/// [`decode_bytes_full`]) does **not** strip it, so the BOM also survives as a
/// leading U+FEFF inside the decoded `String`. [`super::decode_in_context`]
/// removes it instead.
...
///
/// Known limitation, kept for compatibility: `encoding_rs` has no UTF-16
/// *encoder*, so a `"utf-16le"` / `"utf-16be"` label silently produces UTF-8
/// bytes under a UTF-16 BOM. [`super::encode_for_disk_strict`] encodes UTF-16
/// by hand and does not have that hole.
