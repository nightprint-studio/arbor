//! Lua-side hook dispatch helpers.
//!
//! Reads handlers registered via `arbor.events.on(...)` (stored under
//! `__arbor_hooks__` in each plugin's Lua VM) and dispatches a fired event
//! to every matching subscription. Pattern matching is glob-based — `*`
//! matches any substring including across separators.
//!
//! Slated for removal in session 7 of PR #4, when [`HookDispatcher`] +
//! `LuaHookListener` (in `arbor-plugin-api` / `arbor-plugin-core::hook_router`)
//! supersede every direct call to this module.
