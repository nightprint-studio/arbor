...
pub type LuaTuple = mlua::Result<(mlua::Value, mlua::Value)>;

/// Build the `(val, nil)` success tuple. `val` may itself be nil (e.g. lookup miss).
pub fn ok2<T: mlua::IntoLua>(lua: &Lua, v: T) -> LuaTuple {
    Ok((v.into_lua(lua)?, mlua::Value::Nil))
}

/// Build the `(nil, err_msg)` failure tuple.
pub fn err2(lua: &Lua, msg: impl Into<String>) -> LuaTuple {
...
pub fn boolerr2(lua: &Lua, ok_flag: bool, err_when_false: Option<String>) -> LuaTuple {
    if ok_flag {
