...
use crate::contribution::ContainerDef;

pub(crate) fn install(ctx: &ApiCtx, lua: &Lua, ui: &Table) -> Result<()> {
    let container_table = lua.create_table().map_err(|e| PluginCoreError::Plugin(e.to_string()))?;

    install_register(ctx, lua, &container_table)?;
    install_open(ctx, lua, &container_table)?;
    install_close(ctx, lua, &container_table)?;

    ui.set("container", container_table).map_err(|e| PluginCoreError::Plugin(e.to_string()))?;
    Ok(())
}
...
fn install_register(ctx: &ApiCtx, lua: &Lua, container_table: &Table) -> Result<()> {
    ...
    container_table.set("register", fn_).map_err(|e| PluginCoreError::Plugin(e.to_string()))?;
    Ok(())
}
...
fn install_open(ctx: &ApiCtx, lua: &Lua, container_table: &Table) -> Result<()> {
    ...
    container_table.set("open", fn_).map_err(|e| PluginCoreError::Plugin(e.to_string()))?;
    Ok(())
}
...
fn install_close(ctx: &ApiCtx, lua: &Lua, container_table: &Table) -> Result<()> {
    ...
    container_table.set("close", fn_).map_err(|e| PluginCoreError::Plugin(e.to_string()))?;
    Ok(())
}
