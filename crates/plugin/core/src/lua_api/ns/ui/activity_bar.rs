...
fn install_add_graph_combo(ctx: &ApiCtx, lua: &Lua, ui: &Table) -> Result<()> {
    ...
    ui.set("add_graph_combo", fn_).map_err(|e| PluginCoreError::Plugin(e.to_string()))?;
    Ok(())
}
...
fn install_set_combo_options(ctx: &ApiCtx, lua: &Lua, ui: &Table) -> Result<()> {
    ...
    ui.set("set_combo_options", fn_).map_err(|e| PluginCoreError::Plugin(e.to_string()))?;
    Ok(())
}
...
fn install_add_separator(ctx: &ApiCtx, lua: &Lua, ui: &Table) -> Result<()> {
    ...
    ui.set("add_separator", fn_).map_err(|e| PluginCoreError::Plugin(e.to_string()))?;
    Ok(())
}
