...
//!
//! `branding` is **not** here: it needs Tauri-internal types (window icon,
//! `AppState.branding`) and stays in the shell crate as a separate
//! `LuaNamespaceInstaller` that attaches its functions to the `arbor.ui`
//! table this module publishes. It runs after `install` (the shell preserves
//! ordering), so the table already exists when branding looks it up.
...
pub(crate) fn install(ctx: &ApiCtx, lua: &Lua, arbor: &Table) -> Result<()> {
    let ui = lua.create_table().map_err(|e| PluginCoreError::Plugin(e.to_string()))?;
    ...
    arbor.set("ui", ui).map_err(|e| PluginCoreError::Plugin(e.to_string()))?;
    Ok(())
}
