...
use crate::lua_api::helpers::convert::strip_utf8_bom;
use crate::lua_api::helpers::fs_perm::{check_fs_read, check_fs_write, FsPerm};
use crate::lua_api::helpers::glob::walk_glob;
use crate::lua_api::helpers::json_patch::set_json_at_path;
use crate::lua_api::helpers::tuple::{LuaTuple, err2, ok2};
use crate::lua_api::helpers::xml_patch::apply_xml_set;

pub(crate) fn install(ctx: &ApiCtx, lua: &Lua, arbor: &Table) -> Result<()> {
    let fs_table = lua.create_table().map_err(|e| PluginCoreError::Plugin(e.to_string()))?;
    ...
    arbor.set("fs", fs_table).map_err(|e| PluginCoreError::Plugin(e.to_string()))?;
    Ok(())
}
...
fn install_read_ops(lua: &Lua, fs_table: &Table, fp: &FsPerm) -> Result<()> {
    // exists(path) → boolean   (read perm — never fails)
    {
        ...
        fs_table.set("exists", fn_).map_err(|e| PluginCoreError::Plugin(e.to_string()))?;
    }
    ...
    // is_file(path) → boolean
    {
        ...
        fs_table.set("is_file", fn_).map_err(|e| PluginCoreError::Plugin(e.to_string()))?;
    }
    ...
    // is_dir(path) → boolean
    {
        ...
        fs_table.set("is_dir", fn_).map_err(|e| PluginCoreError::Plugin(e.to_string()))?;
    }
    ...
    // list(dir) → (entries[], nil) | (nil, err)
    {
        ...
        fs_table.set("list", fn_).map_err(|e| PluginCoreError::Plugin(e.to_string()))?;
    }
    ...
    // glob{root, pattern, include_dirs?, max_depth?} → (paths[], nil) | (nil, err)
    {
        ...
        fs_table.set("glob", fn_).map_err(|e| PluginCoreError::Plugin(e.to_string()))?;
    }
...
fn install_write_ops(lua: &Lua, fs_table: &Table, fp: &FsPerm) -> Result<()> {
    ...
    // Parent directories are created. Writes are always BOM-free.
    {
        ...
        fs_table.set("append", fn_).map_err(|e| PluginCoreError::Plugin(e.to_string()))?;
    }
    ...
    // touch(path) → (true, nil) | (nil, err)
    {
        ...
        fs_table.set("touch", fn_).map_err(|e| PluginCoreError::Plugin(e.to_string()))?;
    }
    ...
    // beforehand (lets Windows' `rename` succeed). Parent of dest is created.
    {
        ...
        fs_table.set("move", fn_).map_err(|e| PluginCoreError::Plugin(e.to_string()))?;
    }
    ...
    // delete(path) → (true, nil) | (nil, err)
    {
        ...
        fs_table.set("delete", fn_).map_err(|e| PluginCoreError::Plugin(e.to_string()))?;
    }
    ...
    // copy(src, dst) → (true, nil) | (nil, err)
    {
        ...
        fs_table.set("copy", fn_).map_err(|e| PluginCoreError::Plugin(e.to_string()))?;
    }
...
fn install_json_set(lua: &Lua, fs_table: &Table, fp: &FsPerm) -> Result<()> {
    ...
    fs_table.set("json_set", fn_).map_err(|e| PluginCoreError::Plugin(e.to_string()))?;
    Ok(())
}
...
fn install_yaml_set(lua: &Lua, fs_table: &Table, fp: &FsPerm) -> Result<()> {
    ...
    fs_table.set("yaml_set", fn_).map_err(|e| PluginCoreError::Plugin(e.to_string()))?;
    Ok(())
}
...
fn install_toml_set(lua: &Lua, fs_table: &Table, fp: &FsPerm) -> Result<()> {
    ...
    fs_table.set("toml_set", fn_).map_err(|e| PluginCoreError::Plugin(e.to_string()))?;
    Ok(())
}
...
fn install_xml_set(lua: &Lua, fs_table: &Table, fp: &FsPerm) -> Result<()> {
    ...
    fs_table.set("xml_set", fn_).map_err(|e| PluginCoreError::Plugin(e.to_string()))?;
    Ok(())
}
