...
pub(crate) fn install(ctx: &ApiCtx, lua: &Lua, arbor: &Table) -> Result<()> {
    let kb_table = lua.create_table().map_err(|e| PluginCoreError::Plugin(e.to_string()))?;

    install_register(ctx, lua, &kb_table)?;

    arbor.set("keybinding", kb_table).map_err(|e| PluginCoreError::Plugin(e.to_string()))?;
    Ok(())
}
...
fn install_register(ctx: &ApiCtx, lua: &Lua, kb_table: &Table) -> Result<()> {
    ...
    kb_table.set("register", fn_).map_err(|e| PluginCoreError::Plugin(e.to_string()))?;
    Ok(())
}
