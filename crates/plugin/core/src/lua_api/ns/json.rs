...
pub(crate) fn install(_ctx: &ApiCtx, lua: &Lua, arbor: &Table) -> Result<()> {
    let json_table = lua.create_table().map_err(|e| PluginCoreError::Plugin(e.to_string()))?;
    ...
    json_table.set("encode", encode_fn).map_err(|e| PluginCoreError::Plugin(e.to_string()))?;
    ...
    let decode_fn = lua
        ...
        .map_err(|e| PluginCoreError::Plugin(e.to_string()))?;
    json_table.set("decode", decode_fn).map_err(|e| PluginCoreError::Plugin(e.to_string()))?;

    arbor.set("json", json_table).map_err(|e| PluginCoreError::Plugin(e.to_string()))?;
    Ok(())
}
