//! [`TomlRefactor`] — a standalone [`RefactorOps`] for the project-wide
...
//! lives at the api/launcher layer (it needs the repo scanner / index, which
//! `core` and this crate don't name). That orchestrator needs a `RefactorOps`
//! to build + apply sites against TOML files on disk; this is it.
//!
//! It mirrors `DefaultBackend`'s own `RefactorOps` impl (sets via
//! `SetPrimitive`, deletes reverse-index, AsDelete null routing), expressed
//! directly against the crate's `mutate` / `project` / `kind` primitives so
//! the launcher can use it without owning a `DefaultBackend` doc registry.
...
/// Stateless TOML leaf operations for `core::refactor`'s project-wide flows.
pub struct TomlRefactor;

impl RefactorOps for TomlRefactor {
    ...
    fn apply_string_rename(
        ...
        let root = project::parse_to_value(text)
            .ok_or_else(|| StudioError::App("TOML parse failed during rename".into()))?;
        for path in paths {
    ...
    fn coerce_set_value(
        ...
        // FROZEN F13: TOML has no native null → AsDelete routes it to a delete.
        Ok(match raw {
            ExprValue::Null      => CoerceOutcome::DeleteInstead,
