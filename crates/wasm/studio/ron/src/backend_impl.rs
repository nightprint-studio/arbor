...
//!
//! Wraps the `RonStudioRegistry` behind a `Mutex` so the trait can expose
//! `&self` methods. Convert-and-forward to the registry methods + map
//! RON-specific types (`NodeKind`, `NodeView`, `DiffHunk`, …) into the
//! format-agnostic shapes from `arbor-studio-types`.
//!
//! RON stays a **hand-written special** — its F12/F13 do NOT delegate to
//! `core::refactor` (unlike JSON / the simple formats): the
//! tag-preserving AST, indent-carrying `apply_string_rename`,
//! `Result`-returning op builder and `raw_current` active-doc path are
//! the same precedent as RON's special diff/query. Project-wide F12/F13 +
//! `list_files` run against the caller-injected
//! [`crate::index_provider::RonIndexProvider`] (the repo scanner +
//! cross-ref index live in the launcher / `arbor-studio-api`, which the
//! crate must not name).
...
use crate::ast;
use crate::registry::{
    self as legacy, NodeKind, PrimitiveValue, RonStudioRegistry,
};
use crate::schema;
use crate::index_provider::{NoIndexProvider, RonRenameDef, SharedIndexProvider};
use arbor_studio_core::prelude::edit_expr::{self, Value as ExprValue};
...
use arbor_studio_types::prelude::{
    ...
    BulkEditValueSource, CrateProbe, CrossRefScope, DiffHunk,
    DiffStatus, DiffTreeNode, DocSnapshot, EncodingInfo, FileEntry,
    FormatDescriptor, IconRef, KindStyle, KindTone, MutateResult, NodeView,
    NullPolicy, ParseResult, QueryHit, QuerySyntax, RenameCollision,
    RenameDirtyBlocker, RenameFailure, RenameOpenDoc, RenamePreview,
    RenameResult, RenameSite, RenameSiteScope, Schema, SchemaHint,
    SchemaHintOrigin, SchemaSourceKind, StudioError, StudioMutation, StudioResult,
...
impl RonBackend {
    /// Build a RON backend with the given index/scanner provider. The
    ...
    fn lock(&self) -> StudioResult<std::sync::MutexGuard<'_, RonStudioRegistry>> {
        self.regs
            .lock()
            .map_err(|_| StudioError::App("ron_studio registry poisoned".into()))
    }
}
...
/// Public factory — RON backend with no index provider (active-doc only).
...
impl StudioFormatBackend for RonBackend {
    ...
    fn diff(&self, doc_id: &str) -> StudioResult<Vec<DiffHunk>> {
        // The registry already emits the shared `DiffHunk` (it delegates
        // to `core::diff::unified`), so no per-format mapping is needed.
    ...
    async fn rename_preview(
        ...
        let preview = tokio::task::spawn_blocking(move || -> StudioResult<RenamePreview> {
            // Aggregate the RON slice of the cross-ref index for the
            // rename target (and the collision target) via the injected
            // provider. The launcher's provider does the smart-reindex /
            // fresh-scan fallback; the crate can't name that index.
            let inputs = index.rename_inputs(
                &repo_root, &old_value, new_value_hint.as_deref(),
            )?;
            ...
            // The provider already filtered `collision_defs` to the hint
            // (empty when no distinct hint was given).
            let collisions: Vec<RenameCollision> = inputs.collision_defs
                .into_iter()
                .map(|d: RonRenameDef| RenameCollision {
                    absolute_path: d.absolute_path,
                    relative_path: d.relative_path,
                    field_path:    d.def_path,
                    key_name:      d.def_field,
    ...
    async fn bulk_edit_preview(
        ...
        match scope {
            BulkEditScope::ActiveDoc => {
                ...
                if let Ok(root) = ast::parse(&current_text) {
            ...
            BulkEditScope::ProjectWide => {
                ...
                tokio::task::spawn_blocking(move || -> StudioResult<BulkEditPreview> {
                    ...
                    for f in &files {
                        ...
                        let Ok(root) = ast::parse(&text) else { continue; };
                        let hits = legacy::query_ast(&root, &query)?;
                        for h in hits {
            ...
            BulkEditScope::ProjectWide => {
                ...
                let result = tokio::task::spawn_blocking(move || -> StudioResult<BulkEditResult> {
                    ...
                    for (abs_path, sites_for_file) in by_file {
                        ...
                        let new_text = ast::to_pretty_string_with(&root_mut, "  ");
...
fn coerce_number_for_target(target: &ast::RonAst, n: f64) -> legacy::BulkSetValue {
    use ast::RonAst;
    use legacy::BulkSetValue as B;
...
/// consistent.
fn render_set_preview(target: &ast::RonAst, v: &legacy::BulkSetValue) -> String {
    use legacy::BulkSetValue as B;
    let is_option = matches!(target, ast::RonAst::Option(_));
    let inner = match v {
        B::String(s) => format!("\"{s}\""),
        B::Int(i)    => i.to_string(),
        B::Float(f)  => format!("{f}"),
...
fn ast_to_eval_value(v: &ast::RonAst) -> Option<ExprValue> {
    use ast::RonAst;
    match v {
