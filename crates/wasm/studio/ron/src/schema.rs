...
// Schema-view DTOs moved to `arbor-studio-types` in Stage 1 of the studio
// crate extraction. This module keeps only the loader *logic* (syn-based
// `.rs` walking); it now produces those shared DTOs instead of owning them.
use arbor_studio_types::prelude::{
    CandidateKind, CrateProbe, FieldDef, ResolvedType, RootCandidate, Schema,
    SchemaStats, TypeDef, TypeSource, VariantDef, VariantShape,
};
