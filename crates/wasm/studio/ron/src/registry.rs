...
use crate::ast;
use crate::ast::RonAst;
use arbor_studio_types::prelude::DiffHunk;
...
struct Doc {
    ...
    // ── Undo / redo ───────────────────────────────────────────────────────
    //
    // Snapshot-based history (full document text per entry). Engine lives
    // in `arbor_studio_core::prelude::history::History`. RON opts into
    // no-op suppression (`new_dedup`) — a replayed identical mutation must
    // not pollute history — and a cap of 128 entries. `set_text` records
    // as coalescing text (`record_text`); tree mutations never coalesce
    // (`record_struct`).
...
impl RonStudioRegistry {
    pub fn parse(
        ...
        self.docs.insert(id.clone(), Doc {
            ...
            indent:      "  ".to_string(),
            history:     History::new_dedup(text, HISTORY_MAX_ENTRIES),
        });
        Ok(ParseResult {
            doc_id:      id,
            size_bytes:  size,
    ...
    pub fn set_text(&mut self, doc_id: &str, text: String) -> Result<UpdateResult> {
        ...
        // Typing in the textarea — coalescing text edit.
    ...
    /// Internal — push the cursor's snapshot into `current` and rebuild
    /// the parse cache. Used by both undo and redo so the frontend gets
    /// the same MutateResult shape either way. (`History` already
    /// dis-armed the coalesce gate.)
    ...
    pub fn history_state(&self, doc_id: &str) -> Result<(bool, bool)> {
        let doc = self.doc(doc_id)?;
        Ok((doc.history.can_undo(), doc.history.can_redo()))
    }
...
//
// The line-level unified diff is the format-agnostic engine in
// `arbor-studio-core` (3-line context, grouped hunks). RON delegates to it
// and emits the shared `DiffHunk` DTO directly — only the *tree* diff below
// stays RON-special (tag-aware shape matching).

impl RonStudioRegistry {
    pub fn diff(&self, doc_id: &str) -> Result<Vec<DiffHunk>> {
        let doc = self.doc(doc_id)?;
        Ok(arbor_studio_core::prelude::diff::unified(&doc.original, &doc.current))
    }
}
