...
mod tests {
    ...
    // ── Additional precedence-level coverage (blueprint §6 edit_expr) ──

    #[test]
    fn logical_or_and_precedence() {
        // && binds tighter than ||: false || (true && false) == false.
        let r = compile("false || true && false").unwrap().eval(&Value::Null).unwrap();
        assert_eq!(r, Value::Bool(false));
        let r = compile("true || true && false").unwrap().eval(&Value::Null).unwrap();
        assert_eq!(r, Value::Bool(true));
    }

    #[test]
    fn comparison_chain() {
        let r = compile("old >= 5").unwrap().eval(&Value::Number(5.0)).unwrap();
        assert_eq!(r, Value::Bool(true));
        let r = compile("old < 5").unwrap().eval(&Value::Number(5.0)).unwrap();
        assert_eq!(r, Value::Bool(false));
    }
    ...
    fn arithmetic_precedence() {
        // 2 + 3 * 4 == 14, not 20.
        let r = compile("2 + 3 * 4").unwrap().eval(&Value::Null).unwrap();
        assert_eq!(r, Value::Number(14.0));
        let r = compile("(2 + 3) * 4").unwrap().eval(&Value::Null).unwrap();
    ...
    fn unary_neg_and_not() {
        let r = compile("-old").unwrap().eval(&Value::Number(7.0)).unwrap();
        assert_eq!(r, Value::Number(-7.0));
        let r = compile("!old").unwrap().eval(&Value::Bool(false)).unwrap();
        assert_eq!(r, Value::Bool(true));
    }

    #[test]
    fn to_casts() {
        let r = compile("old.to_number()").unwrap().eval(&Value::String("42".into())).unwrap();
        assert_eq!(r, Value::Number(42.0));
        let r = compile("old.to_string()").unwrap().eval(&Value::Number(3.0)).unwrap();
        assert_eq!(r, Value::String("3".into()));
        let r = compile("old.to_bool()").unwrap().eval(&Value::Number(0.0)).unwrap();
        assert_eq!(r, Value::Bool(false));
    }

    #[test]
    fn coalesce_null_guard_keeps_non_null() {
        let r = compile("old ?? \"fallback\"").unwrap()
            .eval(&Value::String("kept".into())).unwrap();
        assert_eq!(r, Value::String("kept".into()));
    }
}
