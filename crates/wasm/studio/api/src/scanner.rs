//! Cross-reference scanner + file index used by the Studio sidebar.
//!
//! Walks the active repo for the data-file kinds we have first-class
//! viewers for (`.ron`, `.json`, `.toml`, `.yaml`, `.properties`) and
//! returns them as a flat list that the frontend folds into a tree, plus
//! the project-wide cross-reference / find-usages / broken-ref scans.
//! Stays deliberately format-agnostic — schema resolution lives in the
//! per-format crates; this module is the FS walk + per-format def/usage
//! delegation.

use arbor_studio_ron::prelude::{self as ron_studio, ast::{self, RonAst}, SchemaHint, SchemaHintOrigin};
use arbor_studio_types::prelude::{StudioError, StudioResult};
use serde::{Deserialize, Serialize};
use std::path::Path;

use crate::config::{
    self, is_excluded, load as load_config, matches_pattern, resolve_reference_fields,
};
...
/// Every supported file kind — the default filter the scanners use when
/// the caller passes an empty `kinds` slice.
pub const ALL_KINDS: [StudioFileKind; 5] = [
    StudioFileKind::Ron,
    StudioFileKind::Json,
    StudioFileKind::Toml,
    StudioFileKind::Yaml,
...
pub fn scan_repo(folder: &str, kinds: &[StudioFileKind]) -> StudioResult<Vec<StudioFileEntry>> {
    let root = Path::new(folder);
    if !root.is_dir() {
        return Err(StudioError::App(format!("Not a directory: {folder}")));
    }
...
fn walk_external(
    ext:   &crate::config::ExternalEntry,
...
#[allow(dead_code)]
pub fn scan_cross_refs(folder: &str) -> StudioResult<Vec<CrossRefDef>> {
...
pub fn scan_cross_refs_for(folder: &str, kinds: &[StudioFileKind]) -> StudioResult<Vec<CrossRefDef>> {
    let root = Path::new(folder);
    if !root.is_dir() {
        return Err(StudioError::App(format!("Not a directory: {folder}")));
    }
    let effective: &[StudioFileKind] = if kinds.is_empty() { &ALL_KINDS } else { kinds };
    ...
    for f in files {
        ...
        match f.kind {
            ...
            StudioFileKind::Json => {
                ...
                let Ok(value) = arbor_studio_json::ast::parse_with(&text, false) else { continue; };
            ...
            StudioFileKind::Toml => {
                ...
                let Some(value) = arbor_studio_toml::parse_to_value(&text) else { continue; };
            ...
            StudioFileKind::Yaml => {
                ...
                let Some(value) = arbor_studio_yaml::parse_to_value(&text) else { continue; };
                collect_defs_toml_into(&value, &[], &f, &mut out);
            }
            StudioFileKind::Properties => {
...
/// `serde_json::Value` (see `arbor_studio_toml::parse_to_value`), so we
/// walk the same shape JSON does. Arrays-of-tables show as Value::Array
...
fn collect_defs_json_into(
    value: &arbor_studio_json::ast::JsonAst,
    path:  &[String],
    entry: &StudioFileEntry,
    out:   &mut Vec<CrossRefDef>,
) {
    use arbor_studio_json::ast::JsonAst;
    match value {
        JsonAst::Object(o) => {
            for prop in &o.props {
                let mut p = path.to_vec();
...
#[allow(dead_code)]
pub fn find_usages(folder: &str, target: &str) -> StudioResult<Vec<UsageMatch>> {
...
pub fn find_usages_for(folder: &str, target: &str, kinds: &[StudioFileKind]) -> StudioResult<Vec<UsageMatch>> {
    if target.is_empty() {
        return Ok(Vec::new());
    }
    let root = Path::new(folder);
...
/// Recursive AST walker mirroring `children_of` in `arbor_studio_ron`:
...
/// emitted by `arbor_studio_toml::parse_to_value` so the convention
...
/// Project-wide reference validation: for every file under `folder`,
/// walk every reference field (built-in convention OR configured
/// patterns) and collect the ones whose string value doesn't match any
/// `id`/`name` definition seen elsewhere in the project. Empty result
/// is the happy path. Same exclude-folder rules as `scan_cross_refs` /
/// `find_usages`.
///
...
#[allow(dead_code)]
pub fn scan_broken_refs(folder: &str) -> StudioResult<Vec<BrokenRef>> {
    scan_broken_refs_for(folder, &[StudioFileKind::Ron])
}

pub fn scan_broken_refs_for(folder: &str, kinds: &[StudioFileKind]) -> StudioResult<Vec<BrokenRef>> {
    let root = Path::new(folder);
    if !root.is_dir() {
        return Err(StudioError::App(format!("Not a directory: {folder}")));
    }
...
pub(crate) fn properties_flat_to_path(flat: &str) -> Vec<String> {
...
fn collect_defs_properties_into(
    ...
    for (flat_key, _value) in arbor_studio_properties::collect_kv_pairs(text) {
...
fn collect_usages_properties(
    ...
    for (flat_key, value) in arbor_studio_properties::collect_kv_pairs(text) {
...
/// tree-jump expects — matches `arbor_studio_ron`'s `key_to_string` but
