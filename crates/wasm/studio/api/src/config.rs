...
//!
//! The same file format that `arbor_studio_ron::detect_schema_hint` already
...
//! `arbor_studio_ron::parse_sidecar_config`; the host only ever creates /
//! writes the single file at the repo root.
...
pub fn load(repo_root: &str) -> StudioResult<StudioConfig> {
    ...
    let text = std::fs::read_to_string(&path)
        .map_err(|e| StudioError::App(format!("read {label}: {e}")))?;
    toml::from_str::<StudioConfig>(&text)
        .map_err(|e| StudioError::App(format!("parse {label}: {e}")))
}

pub fn save(repo_root: &str, cfg: &StudioConfig) -> StudioResult<()> {
    ...
    let text = toml::to_string_pretty(cfg)
        .map_err(|e| StudioError::App(format!("encode {SIDECAR_PATH}: {e}")))?;
    std::fs::write(&path, text)
        .map_err(|e| StudioError::App(format!("write {SIDECAR_PATH}: {e}")))?;
    Ok(())
}
...
/// walk-up sidecar lookup in `arbor_studio_ron::parse_sidecar_config`
...
// ── Glob ────────────────────────────────────────────────────────────────────
// Same `*` / `**` semantics as `arbor_studio_ron::glob_match`, simplified for
