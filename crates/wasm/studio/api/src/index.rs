...
use arbor_studio_json::ast::{self as json_ast, JsonAst};
use arbor_studio_ron::ast::{self as ron_ast, RonAst};
use arbor_studio_types::prelude::{StudioError, StudioResult};
use crate::config::{
    self as studio_config, matches_pattern, resolve_reference_fields, StudioConfig,
};
use crate::scanner::{scan_repo, BrokenRef, CrossRefDef, StudioFileKind, UsageMatch, ALL_KINDS};
...
pub fn save(repo_root: &str, idx: &StudioIndex) -> StudioResult<()> {
    let path = index_path(repo_root);
    if let Some(parent) = path.parent() {
        std::fs::create_dir_all(parent)
            .map_err(|e| StudioError::App(format!("mkdir .arbor: {e}")))?;
    }
    let text = serde_json::to_string_pretty(idx)
        .map_err(|e| StudioError::App(format!("encode studio-index: {e}")))?;
    std::fs::write(&path, text)
        .map_err(|e| StudioError::App(format!("write studio-index: {e}")))?;
    Ok(())
}
...
fn hash_config(cfg: &StudioConfig) -> u64 {
    ...
    let mut ext: Vec<&crate::config::ExternalEntry> = cfg.externals.iter().collect();
...
pub fn refresh_for(
    ...
    let effective_kinds: &[StudioFileKind] = if kinds.is_empty() { &ALL_KINDS } else { kinds };
...
fn parse_and_extract(
    ...
    match kind {
        ...
        StudioFileKind::Toml => {
            ...
            // straightforward.
            let value = arbor_studio_toml::parse_to_value(&text)?;
        ...
        StudioFileKind::Yaml => {
            ...
            // TOML (`arbor_studio_yaml::parse_to_value`), so we reuse the
            // TOML walkers verbatim. Multi-doc streams collapse to
            // `Value::Array`; recursion handles it.
            let value = arbor_studio_yaml::parse_to_value(&text)?;
            collect_defs_toml(&value, &[], &mut defs);
            collect_refs_toml(&value, &[], patterns, &mut refs);
        }
...
fn collect_defs_properties_index(text: &str, out: &mut Vec<IndexedDef>) {
    for (flat_key, _v) in arbor_studio_properties::collect_kv_pairs(text) {
        let path = crate::scanner::properties_flat_to_path(&flat_key);
