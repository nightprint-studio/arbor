...
//!
//! Phase 3.b promoted the backend from a read-only view (Phase 3.a) to
...
//! through `git::encoding::encode_for_disk_with_bom` (FROZEN F16). The
//! descriptor's `supports_lossless_edit` flips to `true` accordingly.
//!
//! What's still gated off for later sub-phases:
//!   - Cross-refs / broken-refs / F12 rename / F13 bulk edit (Phase 3.c).
//!   - JSON Schema sidebar (Phase 3.c).
//!   - JSONC support (relaxed parsing, comments, `.jsonc` association) — Phase 3.d.
//!
//! State (the `JsonStudioRegistry` map) lives behind a `Mutex` inside
//! the backend so the trait can expose `&self` methods — same pattern
//! as `RonBackend`.
...
use crate::json_studio::{
    self as legacy, DocParseMode, JsonStudioRegistry, NodeKind,
    bulk_edits::{JsonBulkOp, JsonSetValue},
};
...
impl JsonBackend {
    ...
    fn lock(&self) -> StudioResult<std::sync::MutexGuard<'_, JsonStudioRegistry>> {
        self.regs
            .lock()
            .map_err(|_| StudioError::App(AppError::Other("json_studio registry poisoned".into())))
    }
}
...
impl StudioFormatBackend for JsonBackend {
    ...
    // ── Phase 3.d — Strip JSONC features ─────────────────────────────
    ...
    async fn list_files(&self, folder: String) -> StudioResult<Vec<FileEntry>> {
        // JSON files live in the same per-repo Studio sidebar walk as
        // RON — defer to `studio::scan_repo` filtered to JSON.
        let entries = tokio::task::spawn_blocking(move || {
            crate::studio::scan_repo(&folder, &[crate::studio::StudioFileKind::Json])
        })
            .await
    ...
    async fn rename_preview(
        ...
        let preview = tokio::task::spawn_blocking(move || -> StudioResult<RenamePreview> {
            ...
            // untouched. Fall back to whatever's already on disk if
            // refresh fails (filesystem hiccup, permission).
            let idx = match crate::studio::index::refresh_for(
                ...
                Ok(i)  => i,
                Err(e) => {
                    tracing::warn!("rename_preview (json): index refresh failed, falling back to fresh scan ({e})");
            ...
            // Collisions — surface defs whose value equals the hint.
            ...
            // Dirty blockers — any open doc whose source matches an
            // affected file AND is dirty (FROZEN F12).
    ...
    async fn rename_apply(
        ...
        let result = tokio::task::spawn_blocking(move || -> StudioResult<RenameResult> {
            ...
            // its own encoding (FROZEN F16) so the flush phase re-
            // encodes per-file, never globally.
            struct PendingWrite {
                abs_path:       String,
                new_text:       String,
            ...
            // Phase B — flush sequentially. Failures recorded per-file
            // (no auto rollback of already-written files, FROZEN F12).
            let mut written: Vec<String>        = Vec::new();
            let mut failed:  Vec<RenameFailure> = Vec::new();
            for w in pending {
                match crate::json_studio::write_to_disk(
    ...
    async fn bulk_edit_preview(
        ...
        // Compile up-front so the per-site loop doesn't pay it. Compile
        // errors land in the top-level banner.
            ...
            BulkEditScope::ProjectWide => {
                ...
                tokio::task::spawn_blocking(move || -> StudioResult<BulkEditPreview> {
                    ...
                    let files = crate::studio::scan_repo(
                        &repo_root,
                        &[crate::studio::StudioFileKind::Json],
                    )?;
                    for f in &files {
                        ...
                        // AST keeps comments / trailing commas safe.
                        let Ok(ast_root) =
                            crate::json_studio::ast::parse_with(&text, /* strict */ false)
                            else { continue; };
                        let root = crate::json_studio::ast::ast_to_value(&ast_root);
                    ...
                    // Dirty blockers — same shape as F12.
                    let affected_paths: HashSet<String> = sites
                        .iter()
                        .map(|s| canonicalise_path_key(&s.absolute_path))
                        .collect();
            ...
            BulkEditScope::ProjectWide => {
                ...
                let result = tokio::task::spawn_blocking(move || -> StudioResult<BulkEditResult> {
                    ...
                    for (abs_path, sites_for_file) in by_file {
                        ...
                        let ast_root = crate::json_studio::ast::parse_with(
                            &text, /* strict */ false,
                        ).map_err(|e| AppError::Other(format!("parse {abs_path}: {e}")))?;
                        let root_value = crate::json_studio::ast::ast_to_value(&ast_root);
                        ...
                        let new_text = legacy::bulk_edits::apply_bulk_edits_text(&text, &ops)
                            .map_err(|e| AppError::Other(format!(
    ...
    async fn schema_load(
        ...
        let schema = tokio::task::spawn_blocking(move || -> StudioResult<Schema> {
            Ok(legacy::schema::load(&src, &root)?)
        })
        .await
    ...
    async fn schema_view_source(
        ...
        let ts = tokio::task::spawn_blocking(move || -> StudioResult<TypeSource> {
            Ok(legacy::schema::get_type_source(&src, &canonical)?)
        })
        .await
...
fn build_site_for_preview(
    ...
    let kind        = legacy::json_kind_str(target).to_string();
    let old_preview = legacy::json_preview_for(target);
...
/// Compute the `JsonSetValue` to write at a `set` site. Returns
/// `Err(reason)` when the site can't be set (eval error, etc.).
/// JSON is dynamically typed so no kind-mismatch rejection happens
/// here — setting a string on a previously-number field is allowed
/// (descriptor `null_handling = Native`).
fn compute_new_value(
    target:       &serde_json::Value,
    value_source: &Option<BulkEditValueSource>,
    compiled:     Option<&CompiledExpr>,
) -> Result<JsonSetValue, String> {
    ...
    Ok(match raw_value {
        ExprValue::Null      => JsonSetValue::Null,
        ExprValue::Bool(b)   => JsonSetValue::Bool(b),
        ExprValue::Number(n) => JsonSetValue::Number(n),
        ExprValue::String(s) => JsonSetValue::String(s),
    })
}
...
fn json_to_eval_value(v: &serde_json::Value) -> Option<ExprValue> {
    match v {
        ...
        // Containers are rejected at preview time — no eval binding.
        _ => None,
    }
}

fn render_set_preview(v: &JsonSetValue) -> String {
    match v {
        JsonSetValue::String(s) => format!("\"{s}\""),
        JsonSetValue::Number(n) => {
        ...
        JsonSetValue::Null      => "null".into(),
    }
}

/// Resolve each FE-submitted site against `root_value` and build the
/// list of `(path, JsonBulkOp)`s to feed `apply_bulk_edits_text`. The
/// returned `(applied_count, skipped_count)` tracks how many sites
/// actually contribute to the batch vs. fell through validation.
fn build_ops_from_sites(
    ...
    value_source: &Option<BulkEditValueSource>,
    compiled:     Option<&CompiledExpr>,
) -> (Vec<(Vec<String>, JsonBulkOp)>, usize, usize) {
    let mut ops:     Vec<(Vec<String>, JsonBulkOp)> = Vec::with_capacity(sites.len());
    ...
    for site in sites {
        ...
        match action {
            BulkEditAction::Delete => {
                ...
                ops.push((site.field_path.clone(), JsonBulkOp::Delete));
                applied += 1;
            }
            BulkEditAction::Set => {
                match compute_new_value(target, value_source, compiled) {
                    Ok(v) => {
                        ops.push((site.field_path.clone(), JsonBulkOp::Set(v)));
                        applied += 1;
                    }
                    Err(_) => skipped += 1,
                }
...
fn build_descriptor() -> FormatDescriptor {
    ...
    FormatDescriptor {
        id:                          "json".into(),
        label:                       "JSON".into(),
        // Phase 3.d: `.jsonc` joins the association. The plugin Lua
        // picker also includes the extension; the studio sidebar maps
        // `jsonc` → `StudioFileKind::Json` so the same backend handles
        // both kinds.
        file_extensions:             vec![".json".into(), ".jsonc".into()],
        icon:                        IconRef::Iconify { name: "vscode-icons:file-type-json".into() },

        // Phase 3.b lit this up; Phase 3.d keeps it on. Mutations
        // splice AST byte ranges in tree mode; stream-mode docs return
        // `Unsupported` on mutations (large files, navigation-only).
        supports_lossless_edit:      true,
        // Phase 3.d: comments + trailing commas accepted in tree mode
        // (`jsonc-parser` lenient). Strict `.json` files don't have
        // them; `.jsonc` files do. The flag advertises the capability;
        // per-doc behaviour is driven by `is_jsonc` + the file content.
        ...
        // Phase 3.d: the catalogue is per-format; per-doc the FE checks
        // `parse_result.has_jsonc_features` + `is_jsonc` to know whether
        // to actually surface this banner.
        save_warnings:               vec![
            crate::studio::format::descriptor::SaveWarningKind::JsoncCommentsInJson,
        ],
        ...
        // F12 + F13 — flipped on in Phase 3.c.a / 3.c.b. The backend
        // implements `rename_preview` / `rename_apply` and the
        // `bulk_edit_*` pair against a JSON-only slice of the studio
        // index (cross-refs) plus byte-splice mutations for the active
        // doc / project-wide flows.
...
fn parse_result_into(p: legacy::ParseResult, encoding: EncodingInfo) -> ParseResult {
    ParseResult {
        ...
        // Mirror the RON backend wrapper: FE pushed the text (or the
        // command layer just read it) so round-tripping it through
        // `original` is wasted bandwidth. The modal calls
        // `raw_original` lazily.
        original:    String::new(),
