...
//! the `.onnx` to `<merula-data>/models/<file>` with progress, tracked as a hidden
//! job routed to the merula window. The transcriber backend selection
//! (`merula::import`) checks [`is_installed`] / [`model_path`] to pick the ONNX
...
use crate::merula::config::{self, MerulaConfig};
...
fn url_for(cfg: &MerulaConfig, id: &str) -> Option<String> {
    match id {
...
#[tauri::command]
pub async fn merula_models() -> Result<Vec<ModelStatus>, crate::error::AppError> {
    let cfg = config::load();
...
#[tauri::command]
pub async fn merula_download_model(app: AppHandle, id: String) -> Result<String, crate::error::AppError> {
    let cfg = config::load();
...
#[tauri::command]
pub async fn merula_delete_model(id: String) -> Result<(), crate::error::AppError> {
    let cfg = config::load();
...
fn start_download(app: &AppHandle, id: &str, name: &str, url: &str, dest: PathBuf) -> String {
    ...
    let _ = app.emit(
        "arbor://job-started",
        serde_json::json!({
            "job_id": &job_id, "name": &job_name, "plugin_name": "merula",
