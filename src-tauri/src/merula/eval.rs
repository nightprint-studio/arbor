...
use super::events::{emit, Diagnostic, MerulaDiagnostics, LogLine, EVT_LOG};
...
/// A [`LogSink`] that forwards (gated) log lines to the merula window as
/// `merula:log`. `Send + Sync` because the per-hap `.log` transform captures it
