...
//!
//! Everything merula pushes to the front end is scoped to the merula window
...
//! exactly one place, mirrored 1:1 by `src/lib/ipc/merula.ts`.
//!
...
/// Emit a merula event to the merula window only. Best-effort: a missing window
/// (closed) is a silent no-op.
pub fn emit<T: Serialize + Clone>(app: &AppHandle, event: &str, payload: T) {
    let _ = app.emit_to(MERULA_WINDOW_LABEL, event, payload);
}
