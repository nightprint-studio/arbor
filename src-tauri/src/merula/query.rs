...
pub struct QueryHaps {
    ...
    /// `merula_render` picks the offline-bounce tempo. `None` when the script set
...
/// The `merula_eval_snippet` result: an arbitrary `.merula` chunk evaluated in
...
pub struct SnippetEval {
    ...
    /// emitted on `merula:diagnostics` (that channel belongs to the main editor).
...
/// [`merula_query`] (over the staged arrangement) and the snippet evaluator (over
...
pub async fn merula_query(
    ...
    // same choice `merula_render` makes — so the estimate is right without playback.
    let (tracks, cps): (Option<Tracks<ControlMap>>, Option<f64>) = {
        let latest = merula.latest.lock().unwrap_or_else(|e| e.into_inner());
        match latest.as_ref() {
