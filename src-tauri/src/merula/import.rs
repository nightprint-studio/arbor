...
//!   (skips transcription).
//!
//! Long-running runs register a hidden job (routed to the merula window) and emit
...
fn default_split_stems(o: &ImportOpts) -> bool {
    ...
    #[cfg(feature = "onnx")]
    {
        ...
        return crate::merula::models::is_installed(&cfg, crate::merula::models::DEMUCS_ID);
    }
...
fn select_transcriber() -> Box<dyn merula::prelude::Transcriber> {
    #[cfg(feature = "onnx")]
    {
        use crate::merula::models;
        let cfg = crate::merula::config::load();
        if let Some(bp) = models::model_path(&cfg, models::BASIC_PITCH_ID) {
            if bp.exists() {
                ...
                match merula::prelude::OnnxTranscriber::new(&bp, demucs) {
                    ...
                    Err(e) => tracing::warn!("merula: ONNX backend init failed ({e}); using DSP fallback"),
                }
            }
        }
    }
...
#[tauri::command]
pub async fn merula_convert_wav_to_midi(
    app: AppHandle,
    input: String,
    output: String,
    op_id: Option<String>,
    ...
    std::thread::Builder::new()
        .name(format!("merula-wav2midi-{job_id}"))
        .spawn(move || {
            let mut last = -1i32;
...
#[tauri::command]
pub async fn merula_import_audio_as_merula(
    app: AppHandle,
    input: String,
    op_id: Option<String>,
    opts: Option<ImportOpts>,
    ...
    let iopts = o.import();
    let in_path = PathBuf::from(input);
    let job_id = register_job(&app, &format!("Import {} → .merula", in_path.display()), "audio-to-merula", op_id);
    ...
    result.map_err(AppError::Merula)
}
...
#[tauri::command]
pub async fn merula_import_midi_as_merula(
    input: String,
    opts: Option<ImportOpts>,
) -> Result<String, AppError> {
...
/// Register a hidden, merula-routed job and announce it. When `preset_id` is given
...
fn register_job(app: &AppHandle, name: &str, command: &str, preset_id: Option<String>) -> String {
    ...
    let _ = app.emit(
        "arbor://job-started",
        serde_json::json!({
            "job_id": &id, "name": name, "plugin_name": "merula",
            "command": command, "category": "Imports", "hidden": true, "target": "merula",
        }),
    );
    id
}
