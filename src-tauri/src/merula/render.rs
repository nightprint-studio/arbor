...
use merula::prelude::{render_offline_with_progress, ControlMap, RenderConfig, RenderOutcome, Tracks};

use crate::jobs::{JobInfo, JobRegistry, JobStatus};

/// Front-end render options. `cycles` is required (a `Pattern` has no intrinsic
/// length); format defaults come from `[merula].render` config unless overridden.
...
pub fn resolve_config(base: RenderConfig, opts: &RenderOpts) -> RenderConfig {
    use merula::prelude::{BitDepth, Format};
    RenderConfig {
...
/// Register a hidden, cancellable merula render/stems job (routed to the merula
...
fn register_render_job(app: &AppHandle, name: &str, command: &str) -> String {
    ...
    let job_id = {
        ...
        jobs.register(JobInfo {
            ...
            // user-facing surface of the merula **Downloads & Exports** overlay
            ...
            is_system: false,
            finished_at: None,
            // Route to the merula window's feedback host (it mounts
            // <FeedbackHost id="merula">), so renders surface there, not in main.
...
pub fn spawn_render(
    ...
    if let Err(e) = std::thread::Builder::new()
        ...
        .spawn(move || {
            ...
            let on_progress = move |p: merula::prelude::RenderProgress| {
    ...
    {
        tracing::error!("merula: failed to spawn render thread: {e}");
    }
...
pub fn spawn_render_stems(
    ...
    {
        tracing::error!("merula: failed to spawn stems thread: {e}");
    }
