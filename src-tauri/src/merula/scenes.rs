...
//! reads them through [`merula_scenes`] to lay out its grid: one row per scene,
...
#[tauri::command]
pub async fn merula_scenes(merula: State<'_, MerulaState>) -> Result<Scenes, AppError> {
...
pub async fn merula_launch(
    ...
    let cfg = super::merula_config();
    merula.stage_tracks(&cfg, staged.0, staged.1, staged.2).await;
    Ok(())
}
