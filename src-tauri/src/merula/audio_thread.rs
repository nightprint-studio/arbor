...
use super::control::{MerulaControl, Prepared};
use super::events::{
...
pub fn run(
    ...
    let mut session = match Session::open(&cfg, loaded) {
        Ok(s) => s,
        Err(e) => {
            tracing::error!("merula: failed to open audio output: {e}");
...
impl Session {
    ...
    fn apply(&mut self, app: &AppHandle, cfg: &MerulaConfig, msg: MerulaControl) -> bool {
        match msg {
            ...
            MerulaControl::SetOutputDevice { device } => self.set_output_device(app, device),
    ...
    fn swap_registry(&mut self, app: &AppHandle, prep: Prepared, cps: f64) -> Option<(bool, f64)> {
        ...
        match open_output_stream(self.device.as_deref(), Vec::new(), registry.clone()) {
            ...
            Err(e) => {
                tracing::warn!("merula: registry rebuild failed ({e}); keeping current voices");
                emit(app, EVT_AUDIO_ERROR, AudioErrorEvent { message: e.to_string() });
                None
            }
        }
    ...
    fn set_output_device(&mut self, app: &AppHandle, device: Option<String>) {
        ...
        match open_output_stream(device.as_deref(), Vec::new(), self.registry.clone()) {
            ...
            Err(e) => {
                tracing::warn!("merula: output-device switch failed ({e}); keeping current device");
                emit(app, EVT_AUDIO_ERROR, AudioErrorEvent { message: e.to_string() });
            }
        }
    }
