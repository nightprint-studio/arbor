//! Dedicated **grove** window — the music live-coding DAW shell.
//!
//! `grove` is Arbor's built-in music live-coding tool (a small language + audio
//! engine; see `design/grove/`). Its authoring surface is a standalone,
...
//! ([`GROVE_WINDOW_LABEL`]) to mount `GroveWindow.svelte` instead of `AppShell`.
//!
//! **Step 0 scope**: the window only hosts the *mocked* GroveShell UI (fake
...
/// Window label for the dedicated grove window. The frontend reads
/// `getCurrentWindow().label` and matches this to switch into grove mode.
pub const GROVE_WINDOW_LABEL: &str = "grove";
...
/// Open the dedicated grove window, or focus it if it already exists (single
...
pub fn open_or_focus(app: &AppHandle) {
    ...
    if let Err(e) = app.run_on_main_thread(move || create_or_focus(&handle)) {
        tracing::error!("failed to dispatch grove window to main thread: {e}");
    }
}
...
fn create_or_focus(app: &AppHandle) {
    if let Some(w) = app.get_webview_window(GROVE_WINDOW_LABEL) {
        let _ = w.unminimize();
        let _ = w.show();
        let _ = w.set_focus();
        return;
    }
    build_grove_window(app);
}

/// Build the frameless grove window.
///
...
/// packaged builds. Frameless to match Arbor; the GroveShell paints its own
...
fn build_grove_window(app: &AppHandle) {
    let res = WebviewWindowBuilder::new(app, GROVE_WINDOW_LABEL, WebviewUrl::default())
        .title("grove — Arbor")
    ...
    if let Err(e) = res {
        tracing::error!("failed to open grove window: {e}");
    }
}

/// IPC entry point so the in-app Command Palette ("Open grove (Music)") can
/// summon the window.
///
...
pub async fn open_grove_window(app: AppHandle) {
    open_or_focus(&app);
}
