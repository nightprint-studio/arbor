...
//! exactly what's installed. The built-in default synth (`synth`) — nemus's
//! universal fallback for any unresolved name — is always present.

use serde::Serialize;

use arbor_nemus::prelude::{InstrumentKind, Registry};
...
/// The `nemus_sounds` result. Always includes the built-in default synth.
#[derive(Debug, Clone, Serialize)]
...
#[tauri::command]
pub async fn nemus_sounds() -> Result<SoundList, AppError> {
