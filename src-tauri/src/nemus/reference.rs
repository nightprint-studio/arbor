...
//!
//! `nemus_lang_reference` returns the whole `.nemus` language catalogue (every
//! combinator, generator, signal, transform, mini-notation operator, …) from the
//! authoritative source in `arbor-nemus-lang` ([`reference`]). The frontend loads
...
//!
//! `arbor-nemus-lang` stays serde-free (its only deps are `tree-sitter` + `cc`),
...
use arbor_nemus::prelude::{reference, DslEntry, DslParam};
use serde::Serialize;
...
/// [`DslKind::as_str`](arbor_nemus::prelude::DslKind::as_str).
#[derive(Serialize)]
...
/// Return the full `.nemus` DSL reference catalogue.
#[tauri::command]
pub fn nemus_lang_reference() -> Result<Vec<DslEntryDto>, AppError> {
