...
//!
//! `nemus_scales` returns every mode `.scale("root:mode")` accepts (canonical
//! name + aliases + one-octave semitone intervals) from the authoritative table in
//! `arbor-nemus-pattern` ([`mode_table`]). The editor loads it once and drives its
...
use arbor_nemus::prelude::mode_table;
use serde::Serialize;
...
#[tauri::command]
pub fn nemus_scales() -> Result<Vec<ScaleModeDto>, AppError> {
    Ok(mode_table()
        .iter()
        .map(|m| ScaleModeDto {
            name: m.name,
