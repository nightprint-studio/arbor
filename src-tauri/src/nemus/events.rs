...
//!
//! Everything nemus pushes to the front end is scoped to the nemus window
...
//! exactly one place, mirrored 1:1 by `src/lib/ipc/nemus.ts`.
//!
...
/// The `nemus:diagnostics` payload, also returned by `nemus_eval` so the caller
/// gets the result inline and via the event.
#[derive(Debug, Clone, Serialize, PartialEq)]
pub struct NemusDiagnostics {
    pub errors: Vec<Diagnostic>,
}

impl NemusDiagnostics {
    /// A clean (no-errors) result.
    pub fn ok() -> Self {
        NemusDiagnostics { errors: Vec::new() }
    }

    /// A single-error result.
    pub fn one(d: Diagnostic) -> Self {
        NemusDiagnostics { errors: vec![d] }
    }
}

/// The `nemus:transport` payload: where the playhead is, the tempo, and whether
...
/// The `nemus:meters` payload: audio-engine telemetry sampled at the tick rate.
...
/// The `nemus:active_haps` payload: every source range sounding at the playhead.
#[derive(Debug, Clone, Serialize, PartialEq)]
pub struct ActiveHaps {
    pub haps: Vec<ActiveHap>,
}

/// The `nemus:log` payload: one (already threshold-gated) log line.
#[derive(Debug, Clone, Serialize, PartialEq)]
pub struct LogLine {
...
/// The `nemus:pack_progress` payload during any sample-pack install job. Carries
...
/// The `nemus:audio_error` payload: the audio device could not be opened, so the
...
/// Emit a nemus event to the nemus window only. Best-effort: a missing window
/// (closed) is a silent no-op.
pub fn emit<T: Serialize + Clone>(app: &AppHandle, event: &str, payload: T) {
    let _ = app.emit_to(NEMUS_WINDOW_LABEL, event, payload);
}
