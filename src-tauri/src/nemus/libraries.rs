//! nemus **external libraries**: GitHub-hosted `.nemus` modules a project depends
//! on, declared in `nemus.toml`'s `[libraries]` table, fetched (pinned to a commit
...
//! # nemus.toml
//! [libraries]
//! drums = "github:octocat/nemus-drums@v1.2"   # owner/repo[@ref]; ref optional
//! ```
//!
//! A **sync** resolves each declared library's ref to a commit SHA, downloads that
//! commit's zipball, extracts it (flattened) under
//! `…/nemus/libraries/<sha>/`, and records `name → { source, sha }` in the
//! project's `nemus.lock` — so a re-sync (or another machine) rebuilds the exact
...
use arbor_core::prelude::{client, nemus_data_dir};
...
fn write_lock(dir: &Path, lock: &LockFile) -> Result<(), String> {
    let header = "# nemus.lock — auto-generated. Pins each [libraries] entry to a commit SHA.\n\n";
    let body = toml::to_string_pretty(lock).map_err(|e| format!("serialize lock: {e}"))?;
    std::fs::write(lock_path(dir), format!("{header}{body}"))
        .map_err(|e| format!("write nemus.lock: {e}"))
}

// ── Cache ─────────────────────────────────────────────────────────────────────

/// Root of the shared library cache (`…/nemus/libraries`).
fn cache_root() -> PathBuf {
    nemus_data_dir().join("libraries")
}
...
/// The declared libraries with their lock/sync state. `nemus_libraries` command.
#[tauri::command]
pub async fn nemus_libraries(project_dir: String) -> Result<Vec<LibraryStatus>, AppError> {
...
/// lock. `nemus_sync_libraries` command.
#[tauri::command]
pub async fn nemus_sync_libraries(app: AppHandle, project_dir: String) -> Result<String, AppError> {
...
fn start_sync(app: &AppHandle, project_dir: PathBuf) -> String {
    ...
    let job_id = {
        ...
        jobs.register(JobInfo {
            ...
            command: "nemus_sync_libraries".to_string(),
    ...
    let _ = app.emit("arbor://job-started", serde_json::json!({
        "job_id": &job_id, "name": "Sync libraries", "plugin_name": "nemus",
        "command": "nemus_sync_libraries", "category": "Downloads",
