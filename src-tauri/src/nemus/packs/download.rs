...
use arbor_nemus::prelude::{list_manifest_instruments, InstrumentInfo, Registry};

use super::{pack_dir, Pack, PackStatus};
use crate::nemus::config::NemusConfig;
use crate::nemus::events::{emit, PackProgress, EVT_PACK_PROGRESS};
...
pub fn status(cfg: &NemusConfig, pack: &Pack) -> PackStatus {
...
pub fn installed_names(cfg: &NemusConfig, pack: &Pack) -> Vec<String> {
    let dir = pack_dir(cfg, pack.id);
...
pub fn list_into(cfg: &NemusConfig, pack: &Pack, out: &mut Vec<InstrumentInfo>) {
    let dir = pack_dir(cfg, pack.id);
    let Some(manifest) = read_manifest(&dir) else {
        return;
    };
...
pub fn load_subset_into(
    ...
    if let Err(e) = reg.load_manifest_subset_into(&registry_path, needed) {
        tracing::warn!("nemus: pack `{}` subset load failed ({e}); skipping", pack.id);
    }
}
...
/// `nemus:pack_progress`.
pub fn start(app: &AppHandle, cfg: &NemusConfig, pack: &'static Pack) -> String {
    let dir = pack_dir(cfg, pack.id);
    ...
    let job_id = {
        ...
        jobs.register(JobInfo {
            ...
            // surface is the nemus **Downloads & Exports** overlay (live %, phase,
            ...
            is_system: false,
            finished_at: None,
            // Route to the nemus window's feedback host so downloads surface
...
fn finish_job(app: &AppHandle, job_id: &str, result: Result<JobOutcome, String>) {
    let (status, success, error) = match result {
        ...
        Err(e) => {
            tracing::warn!("nemus: pack download/install failed ({job_id}): {e}");
