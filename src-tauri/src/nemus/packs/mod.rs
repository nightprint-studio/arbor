...
use arbor_nemus::prelude::{InstrumentInfo, Registry};
...
/// Every pack nemus knows how to install. Order is the sound-bank display order.
...
/// Install directory for a pack. VSCO lives at `<nemus-data>/vsco` (or the
/// `vsco_dir` override); every other pack lives under `<nemus-data>/packs/<id>`
/// (or the `packs_dir` override).
pub fn pack_dir(cfg: &NemusConfig, id: &str) -> PathBuf {
    if id == "vsco" {
        if let Some(dir) = &cfg.vsco_dir {
            return PathBuf::from(dir);
        }
        return arbor_core::prelude::nemus_data_dir().join("vsco");
    }
    let base = match &cfg.packs_dir {
        Some(d) => PathBuf::from(d),
        None => arbor_core::prelude::nemus_data_dir().join("packs"),
    };
    base.join(id)
}
...
pub fn delete(cfg: &NemusConfig, id: &str) -> Result<(), String> {
    if pack(id).is_none() {
...
pub fn list(cfg: &NemusConfig) -> Vec<PackStatus> {
...
pub fn installed_instrument_names(cfg: &NemusConfig) -> Vec<String> {
...
pub fn instrument_pack_map(cfg: &NemusConfig) -> std::collections::HashMap<String, (String, String)> {
...
pub fn list_instruments_into(cfg: &NemusConfig, out: &mut Vec<InstrumentInfo>) {
    for p in PACKS {
...
pub fn start_download(app: &AppHandle, cfg: &NemusConfig, id: &str) -> Result<String, String> {
    let pack = pack(id).ok_or_else(|| format!("unknown sample pack `{id}`"))?;
