...
//!
//! For the OAuth refresh path see `cloud/oauth_google.rs` which owns the
    ...
    match auth {
        GcsAuth::SaFile { path } => {
            let json = std::fs::read_to_string(path)
                .map_err(|e| AppError::AuthFailed(format!("read {path}: {e}")))?;
        ...
        GcsAuth::Adc => {
            ...
            let json = std::fs::read_to_string(&path)
                .map_err(|e| AppError::AuthFailed(format!("read ADC {}: {e}", path.display())))?;
            ...
            match adc_type(&json) {
                ...
                other => Err(AppError::AuthFailed(format!(
        ...
        GcsAuth::Oauth { secret_ref } => {
            ...
            let stored: StoredOAuth = serde_json::from_str(&blob)
                .map_err(|e| AppError::AuthFailed(format!("malformed OAuth blob: {e}")))?;
            ...
            let refreshed = crate::cloud::oauth_google::refresh_with(&stored).await?;
...
async fn refresh_authorized_user(json: &str) -> Result<String> {
    let cfg: AdcAuthorizedUser = serde_json::from_str(json)
        .map_err(|e| AppError::AuthFailed(format!("malformed authorized_user ADC: {e}")))?;
    ...
    let resp = reqwest::Client::new()
        ...
        .send()
        .await
        .map_err(|e| AppError::AuthFailed(format!("ADC refresh request failed: {e}")))?;

    if !resp.status().is_success() {
        let status = resp.status();
        let body = resp.text().await.unwrap_or_default();
        return Err(AppError::AuthFailed(format!("ADC refresh HTTP {status}: {body}")));
    }
    #[derive(Deserialize)]
    struct R { access_token: String }
    let r: R = resp.json().await
        .map_err(|e| AppError::AuthFailed(format!("ADC refresh decode: {e}")))?;
    Ok(r.access_token)
}
...
async fn gcloud_print_access_token() -> Result<String> {
    ...
    .await
    .map_err(|e| AppError::AuthFailed(format!("gcloud spawn join: {e}")))?
    .map_err(|e| AppError::AuthFailed(format!(
