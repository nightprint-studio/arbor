...
/// target plugin runs. After the plugin-relocation flip, universal plugins load in the
/// product backends, so a hook the shell raises for one of them — a modal round-trip's
/// confirm, an OAuth-done, a job's progress or completion — must be forwarded there or the
/// plugin never sees it and the UI hangs (the "Loading…" stall). This routes through each
/// backend's `fire_plugin_action` (the exact cross-process twin of the shell's plugin-
