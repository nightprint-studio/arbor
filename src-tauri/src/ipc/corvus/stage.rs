//! `stage` domain — git-index handlers routed through the in-process broker.
//!
//! Each handler is the body the matching `#[tauri::command]` used to run
//! inline; `#[corvus::handler]` self-registers it under its own function name,
//! reached generically through the router. Behavior (locks held, the
//! `revparse_single("HEAD")` workarounds for the vendored-libgit2 peel bug,
//! recovery snapshots, errors) is byte-identical — only the call path changed.
//!
//! The git work runs directly against the live [`crate::AppState`] repo manager
//! (libgit2, no Tauri / no credentials). The generic `rpc` command already wraps
//! dispatch in `spawn_blocking`, so no per-handler async is needed.
//!
//! `commit` lives here too and fires hooks **inline**: the vetoable
//! `on_pre_commit` (a non-empty plugin return aborts) before the mutation, and
//! `on_commit` after. This works because the plugin host is reached through the
//! handler's `&AppState` (`state.hook_dispatcher` / `state.fire_hook`) — the
//! same in-process host the old `#[tauri::command]` used. Co-locating the host
//! with the handler is exactly why the broker no longer needs a separate
//! vetoable round-trip seam (see `docs/plugin-relocation-inventory.md`, Wave 0).
...
#[corvus::handler]
fn commit(state: &AppState, tab_id: String, message: String, amend: bool) -> Result<String, AppError> {
    // ── Pre-commit veto ────────────────────────────────────────────────
...
#[corvus::handler]
fn stage_file(state: &AppState, tab_id: String, path: String) -> Result<(), AppError> {
    let mut mgr = state.lock_repos()?;
    let repo = mgr.get_mut(&tab_id)?;
    let r = repo.inner_mut();
...
#[corvus::handler]
fn unstage_file(state: &AppState, tab_id: String, path: String) -> Result<(), AppError> {
    let mut mgr = state.lock_repos()?;
    let repo = mgr.get_mut(&tab_id)?;
    let r = repo.inner();
...
#[corvus::handler]
fn stage_all(state: &AppState, tab_id: String) -> Result<(), AppError> {
    let mut mgr = state.lock_repos()?;
    let repo = mgr.get_mut(&tab_id)?;
...
#[corvus::handler]
fn unstage_all(state: &AppState, tab_id: String) -> Result<(), AppError> {
    let mut mgr = state.lock_repos()?;
    let repo = mgr.get_mut(&tab_id)?;
    let r = repo.inner();
...
#[corvus::handler]
fn discard_file(state: &AppState, tab_id: String, path: String) -> Result<(), AppError> {
    let mut mgr = state.lock_repos()?;
    let repo = mgr.get_mut(&tab_id)?;
    let r = repo.inner_mut();
...
#[corvus::handler]
fn discard_all(state: &AppState, tab_id: String) -> Result<(), AppError> {
    let mut mgr = state.lock_repos()?;
    let repo = mgr.get_mut(&tab_id)?;
    let r = repo.inner_mut();
...
#[corvus::handler]
fn stage_patch(state: &AppState, tab_id: String, patch: String) -> Result<(), AppError> {
    let mut mgr = state.lock_repos()?;
    let repo = mgr.get_mut(&tab_id)?;
    let r = repo.inner_mut();
...
#[corvus::handler]
fn cherry_pick(state: &AppState, tab_id: String, oid: String) -> Result<CherryPickResult, AppError> {
    let mut mgr = state.lock_repos()?;
    let repo = mgr.get_mut(&tab_id)?;
...
#[corvus::handler]
fn revert_commit(state: &AppState, tab_id: String, oid: String) -> Result<CherryPickResult, AppError> {
    let mut mgr = state.lock_repos()?;
    let repo = mgr.get_mut(&tab_id)?;
    let git_oid = git2::Oid::from_str(&oid)
        .map_err(|_| AppError::CommitNotFound(oid.clone()))?;
...
#[corvus::handler]
fn get_git_commit_template(state: &AppState, tab_id: String) -> Result<Option<String>, AppError> {
    let mut mgr = state.lock_repos()?;
