...
//! `list_remotes` is the leaf, credential-free remote-config query (no network).
//! `fetch_remote` / `push_branch` / `pull_branch` are the network-coupled ops:
//! they inject host-scoped HTTPS auth and run the blocking git work on the
//! blocking pool (the generic `rpc` command awaits these `async` handlers on the
//! runtime). `pull_branch` streams `arbor://pull-progress` / `arbor://pull-done`
//! through the backend [`EventSink`] (Model-D-safe: in-process it forwards to the
//! Tauri emitter; post-split it crosses the IPC event channel) instead of an
//! `AppHandle`. They fire `on_fetch` / `on_push` / `on_pull` inline.
//!
//! `open_in_browser` stays inline in the legacy command module — it drives the
//! system opener through an `AppHandle` (OS glue, not a router method).
...
async fn fetch_remote(
    ...
    // Run the blocking fetch on tokio's blocking pool so the runtime (and any
    // other .await-ing handler) stays responsive.
...
async fn pull_branch(
    ...
    // into the blocking task, so pull phases can stream without holding any
    // non-Send Tauri handle. `None` when no op_id (or the backend isn't wired).
    let tab_id_task = tab_id.clone();
    let remote_task = remote.clone();
    let progress: Option<(Arc<dyn EventSink>, String)> = match (op_id.clone(), state.event_sink()) {
        (Some(oid), Some(sink)) => Some((sink, oid)),
        _ => None,
    };
    ...
    if let Some(ref oid) = op_id {
        match &result {
            Ok(pr) => {
                ...
                state.emit("arbor://pull-done", serde_json::json!({
                    "op_id":   oid,
                    "status":  status,
                    "error":   error,
...
fn emit_phase(progress: PullProgress<'_>, phase: PullPhase, detail: Option<&str>) {
    ...
    sink.emit("arbor://pull-progress", serde_json::json!({
        "op_id":   oid,
        "phase":   phase.key(),
        "label":   phase.label(),
        "detail":  detail,
...
///
/// `progress`, when non-None, is a (EventSink, op_id) pair used to emit
