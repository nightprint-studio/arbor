...
// ---------------------------------------------------------------------------
// IDE detection (streams progress + results to the frontend)
// ---------------------------------------------------------------------------
...
#[corvus::handler]
fn start_ide_detection(state: &AppState) -> Result<String, AppError> {
    ...
    // Probe each IDE in a detached thread — never blocks the calling thread.
    ...
    let _thread = std::thread::Builder::new()
        ...
        .spawn(move || {
            ...
            sink_bg.emit("arbor://ide-detection-done", serde_json::json!(&results));
        });
