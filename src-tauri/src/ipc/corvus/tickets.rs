//! `tickets` (ticket-links) domain — handlers routed through the in-process broker.
//!
//! Each handler is the body the matching `#[tauri::command]` ran inline;
//! `#[corvus::handler]` self-registers it under its own function name (reading
//! the signature to generate the JSON-arg decode), so the command is reached
//! generically through the router. Behavior (locks held, errors, regex/cache
//! logic) is byte-identical — only the call path changed.
//!
//! The git/IO logic already lives in the reusable shell module
//! [`crate::git::ticket_links`] (git2 + regex + TOML, `AppState`-coupled via
//! caches/config), so handlers delegate to it directly — no `corvus-git` crate
//! extraction. There are NO fire-and-forget hooks in this domain.
...
fn effective_config(state: &AppState, tab_id: &str) -> Result<TicketLinkConfig, AppError> {
...
) -> Result<HashMap<String, Vec<TicketLink>>, AppError> {
...
#[corvus::handler]
fn get_ticket_link_config(
    state:  &AppState,
    tab_id: String,
) -> Result<TicketLinkConfig, AppError> {
...
///
/// The original `#[tauri::command]` took no state and returned a bare `String`.
/// The handler shape requires a context first-arg and a `Result` return, so we
/// add an unused `_state` and wrap in `Ok(..)` — `Ok(s)` serializes to the same
/// bare JSON string on the wire, so the FE contract is unchanged.
#[corvus::handler]
fn validate_ticket_regex(_state: &AppState, pattern: String) -> Result<String, AppError> {
...
fn find_commits_for_ticket(
    ...
    tab_id:    String,
    ticket_id: String,
) -> Result<Vec<LinkedCommitRef>, AppError> {
    if !state.lock_config()?.ticket_links.enabled {
        return Ok(vec![]);
    }
