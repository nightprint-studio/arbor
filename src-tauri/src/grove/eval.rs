//! Re-eval orchestration: `.grove` source → `Tracks`, plus the host-injected
...
//! turned into [`GroveDiagnostics`] with spans — they are *diagnostics*, not
//! command failures, so `grove_eval` still returns `Ok`.
...
/// A [`LogSink`] that forwards (gated) log lines to the grove window as
/// `grove:log`. `Send + Sync` because the per-hap `.log` transform captures it
...
pub fn evaluate_source(
    ...
    base_dir: PathBuf,
    cfg: EvalConfig,
) -> Result<EvalOutput, GroveDiagnostics> {
    let program = parse(source).map_err(|e| GroveDiagnostics::one(to_diagnostic(&e)))?;
    ...
    evaluate(&program, loader, log, cfg).map_err(|e| GroveDiagnostics::one(to_diagnostic(&e)))
}
