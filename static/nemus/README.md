# nemus — Tree-sitter WebAssembly assets

The nemus editor (CodeMirror 6) parses `.nemus` source **in the WebView** with the
same `arbor-nemus-lang` Tree-sitter grammar the backend uses, via
[`web-tree-sitter`](https://www.npmjs.com/package/web-tree-sitter). Two `.wasm`
files must live here (served at `/nemus/*.wasm`):
...
| `tree-sitter-nemus.wasm` | the **nemus grammar** compiled to wasm | built from the crate (below) |
...
From the repo root, run **`build-nemus-wasm.bat`** — it builds the grammar wasm
...
tree-sitter build --wasm            # emits tree-sitter-nemus.wasm

# 2. place both wasm files here (paths relative to repo root)
cp crates/nemus/arbor-nemus-lang/tree-sitter-nemus.wasm static/nemus/
cp node_modules/web-tree-sitter/tree-sitter.wasm        static/nemus/
```
