/**
 * MerulaShell UI state — the panel/layout/selection spine. The transport (run /
 ...
 * project + its `.merula` sources live in `stores/project.svelte`. This store owns
 ...
 *
 * Layout (panel choices + collapse flags) is mirrored to the persisted merula
 * window state via `layoutSnapshot()` / `applyLayout()` (MerulaShell wires the
 * persistence + restore).
 */
 ...
 *  literals (the editor resolves spans + applies the edit — see merula-edit). */
...
function createMerulaStore() {
  ...
  // `.merula`; see mixer store `muteToSource`). Survives eval re-baseline because
  ...
  // full-screen stage for live play. MerulaShell drives the actual window fullscreen
  ...
  // ── Overlays (single mount in MerulaShell; opened from menu + shortcuts) ──────
  ...
  // File rename / delete targets — hosted once in MerulaShell, opened from BOTH the
  ...
  return {
    ...
    layoutSnapshot(): MerulaLayoutState {
      return {
    ...
    applyLayout(l: MerulaLayoutState) {
...
export const merulaStore = createMerulaStore();

/** Levels at or above a threshold (console emission gating). */
export function levelsAtOrAbove(threshold: string): Set<string> {
  const start = LOG_LEVELS.indexOf(threshold as MerulaLogThreshold);
  return new Set<string>(LOG_LEVELS.slice(start < 0 ? 0 : start));
}

// Re-export so ConsolePanel / MerulaTitleBar keep a single import site for the
