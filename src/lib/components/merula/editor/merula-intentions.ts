/**
 * merula intentions ("Alt+Enter" quick-fixes / context actions) — pure, CM-free.
 *
 ...
 * extract from `merula-refactor`), fix an unresolved `inst("…")` / `s("…")` by
...
function parseScaleSpec(spec: string, modes: MerulaScaleMode[]): ParsedScale | null {
...
export function changeScalePlan(
  tree: Tree, src: string, offset: number, newSpec: string, modes: MerulaScaleMode[],
