<script lang="ts">
  /**
   * MerulaEditor — the CodeMirror 6 host for one `.merula` buffer. Format-specific
   * sibling of the shared `StudioTextPane`, but merula-coupled: it wires the live
  ...
  import { buildControlEdits, type ControlEdit, type EditChange } from './merula-edit';
  import { renamePlan, extractTarget, extractLetPlan, inlinePlan, freshName } from './merula-refactor';
  import { collectIntentions, changeScalePlan, type IntentionItem } from './merula-intentions';
  import { scalesStore } from '../stores/scales.svelte';
  import type { UsageItem, UsageAnchor } from '../stores/usages.svelte';
  import { diagnosticsStore, activeHapsStore, merulaEngine } from '../stores/engine.svelte';
  ...
  function handleSelection(range: { from: number; to: number } | null) {
    ...
    const ranges = [range];
    const tree = view ? getMerulaTree(view) : null;
    if (tree) {
  ...
  async function onCtxSelect(id: string) {
    ...
    if (id === 'play') void merulaEngine.playSnippet(await withFileDeps(src, text), projectStore.project?.path);
    else if (id === 'scratch') { scratchStore.load(text); merulaStore.showBottom('scratch'); }
  }
   ...
   *  the MerulaShell's Ctrl+F when the editor pane has focus). */
  ...
  export async function formatDocument(): Promise<{ ok: boolean; error?: string }> {
    ...
    try {
      formatted = await merulaFormat(src);
    } catch (e) {
  ...
  export function getStructure(): MerulaSymbol[] {
    if (!view) return [];
    const tree = getMerulaTree(view);
    if (!tree) return [];
  ...
  // The pure planners live in `merula-refactor`; these resolve the live tree +
