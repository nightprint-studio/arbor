/**
 * merula language intelligence for CodeMirror — autocomplete + hover docs.
 *
 * Both features are driven by the **canonical DSL catalogue** (`referenceStore`,
 * fed by `merula_lang_reference`) plus the file's **local symbols** (let / fn /
 * track / import) read straight from the live Tree-sitter tree (`merula-lang`).
 ...
 *
 *   - {@link merulaCompletion} — a `CompletionSource`. After `.` it offers the
 ...
 *   - {@link merulaHover} — a `hoverTooltip` that renders the signature, summary,
...
function symbolType(kind: MerulaSymbolKind): string {
...
function entryCompletion(e: MerulaDslEntry): Completion {
  return {
    label: e.name,
 ...
 *  it is offered for every voice — see merula-audio's voice pool docs. */
...
function articulationCompletions(instruments: MerulaInstrument[]): Completion[] {
...
function instrumentCompletion(i: MerulaInstrument): Completion {
  return {
    label: i.name,
...
function symbolCompletion(s: MerulaSymbol): Completion {
  return {
    label: s.name,
...
function localSymbols(view: EditorView | undefined): MerulaSymbol[] {
  if (!view) return [];
...
/**
 * The merula completion source. Detects method position (a `.` immediately before
 ...
 */
export function merulaCompletion(src: MerulaIntelSource): CompletionSource {
  return (context: CompletionContext): CompletionResult | null => {
    ...
    const tree = context.view ? getMerulaTree(context.view) : null;
    if (tree) {
...
export function merulaHover(src: MerulaIntelSource): Extension {
 ...
 *  example). Styled by `.cm-grv-doc*` classes (see merula-cm theme). */
function infoDom(e: MerulaDslEntry): HTMLElement {
...
function symbolDom(s: MerulaSymbol): HTMLElement {
  const root = document.createElement('div');
  root.className = 'cm-grv-doc';
  const sig = document.createElement('div');
  sig.className = 'cm-grv-doc-sig';
...
export function merulaLanguageIntel(src: MerulaIntelSource): Extension {
  return [
    autocompletion({
      override: [merulaCompletion(src)],
