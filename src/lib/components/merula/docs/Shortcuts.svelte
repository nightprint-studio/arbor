<script lang="ts">
  ...
  const global = MERULA_BINDINGS.filter((b) => b.scope === 'global');
  const editor = MERULA_BINDINGS.filter((b) => b.scope === 'editor');
</script>

<h1>Keyboard shortcuts</h1>
<p class="doc-lead">
  merula is keyboard-first: every action is reachable without the mouse. Open the command
