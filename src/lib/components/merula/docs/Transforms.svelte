...
<MerulaCode code={`let theme = cat(m_a, m_b, m_c, m_d)   // 4 bars, one motif per cycle
...
<MerulaCode code={`let pad = chords.inst("synth.pad").eq("hpf", 80, 0).eq("peak", 3000, -4, 1.2)
...
<MerulaCode code={`let violini = theme.inst("strings.violin_section").gain(0.9).room(0.18).art("legato")
