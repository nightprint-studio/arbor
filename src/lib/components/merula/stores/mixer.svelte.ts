 ...
 * identity the engine exposes (`merula_query` haps and `merula:meters` rows are
 ...
 * are pushed to the running session via `merula_set_track` and the source stays
 * authoritative, so every eval re-baselines and the overrides reset to neutral
 * (see {@link rebaseline}).
 *
 * Mute / solo live in the shared MerulaShell store (keyed by `String(index)`) so
 ...
 * straight back to it (`merula_set_literal` via the editor's Tree-sitter tree).
 ...
 * the `.merula`. No explicit commit button is needed; `commit`/`commitAll` just
...
function createMixerStore() {
  ...
  // Shared reverb-return decay (seconds). Like `master`, this has no `.merula`
  // source representation, so it's NOT cleared on rebaseline — but it IS persisted
  // per-project (`.merula/mix.json`) so the master mix survives a reopen.
  ...
  // project's `.merula/mix.json`. Suppressed while loading a project's saved mix.
  ...
  function persistMix() {
    ...
    mixPersistTimer = setTimeout(() => {
      ...
      void setMerulaProjectMix(path, { master_gain: master, reverb_decay: reverbDecay }).catch(() => {});
    }, 300);
  }
  ...
  function flushCommits() {
    commitTimer = null;
    for (const [index, m] of pendingEdits) merulaStore.requestCommit(index, [...m.values()]);
    pendingEdits.clear();
  }
  ...
  function flushOne(index: number) {
    ...
    merulaStore.requestCommit(index, [...m.values()]);
  }
  ...
  function commitNow(index: number, edits: ControlEdit[]) {
    ...
    merulaStore.requestCommit(index, edits);
  }
  ...
  function muteToSource(i: number, mute: boolean) {
    ...
    if (mute) void merulaSetTrack('gain', i, 0);
    ...
    if (mute) {
      ...
      merulaStore.setPremuteGain(k, gains[i] ?? controlsStore.gain(i) ?? GAIN_UNITY);
      if (!calculated) commitNow(i, [{ kind: 'gain', value: 0 }]);
    } else {
      const prev = merulaStore.premuteGain(k) ?? GAIN_UNITY;
      merulaStore.clearPremuteGain(k);
      void merulaSetTrack('gain', i, gains[i] ?? prev); // live: back to pre-mute gain
  ...
  return {
    ...
    setMasterGain(v: number) { master = v; void merulaSetTrack('master_gain', null, v); persistMix(); },
    ...
    async loadMix(projectPath: string) {
      loadingMix = true;
      try {
        const mix = await getMerulaProjectMix(projectPath);
    ...
    syncMaster() {
      void merulaSetTrack('master_gain', null, master);
      void merulaSetReverb(reverbDecay);
    },
    ...
    setReverbDecay(v: number) {
      ...
      reverbTimer = setTimeout(() => { reverbTimer = null; void merulaSetReverb(v); }, 140);
      persistMix();
    },
    ...
    isMuted(i: number)  { return merulaStore.isMuted(String(i)); },
    isSoloed(i: number) { return merulaStore.isSoloed(String(i)); },
    ...
    toggleMute(i: number) {
      muteToSource(i, !merulaStore.isMuted(String(i)));
    },
    toggleSolo(i: number) {
      const k = String(i);
      merulaStore.toggleSolo(k);
      void merulaSetTrack('solo', i, merulaStore.isSoloed(k) ? 1 : 0);
    },
    ...
    get anySolo() { return tracks.some((t) => merulaStore.isSoloed(String(t.index))); },
    ...
    get selectedIndex(): number | null {
      const id = merulaStore.selectedTrackId;
      if (id == null) return null;
      const n = Number(id);
      return Number.isInteger(n) ? n : null;
    },
    select(i: number) { merulaStore.selectTrack(String(i)); },
     ...
     *
     *  The **master gain is NOT reset**: it is mixer-only (no `.merula`
