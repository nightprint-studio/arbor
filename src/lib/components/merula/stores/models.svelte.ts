/**
 * merula transcription-models store — install status of the on-demand ONNX models
 ...
 * (`merula_download_model` returns a job id; progress on `arbor://job-progress`,
 * completion on `arbor://job-done`) rather than the sample-pack-specific
 * `merula:pack_progress` stream. Each download is also surfaced in the shared
 * Downloads & Exports overlay (`transfersStore`), keyed by model id.
 *
 * Subscribe once on mount (MerulaShell), unlisten on teardown.
 */
...
function createModelsStore() {
  let models   = $state<MerulaModelStatus[]>([]);
  ...
  async function refreshModels() {
    try { models = await merulaModels(); } catch { /* keep last */ }
  }
  ...
  return {
    ...
    async download(id: string) {
      ...
      try {
        const job = await merulaDownloadModel(id);
