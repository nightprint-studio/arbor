/**
 * merula DSL reference store — the canonical language catalogue (`merula_lang_reference`).
 *
 ...
 * Rust (`merula-lang`). This single source feeds the editor's autocomplete,
...
function createReferenceStore() {
  ...
  let byNameMap = $state<Map<string, MerulaDslEntry>>(new Map());
