 ...
 * peaks by rendering the loop **silently** in the backend (`merula_analyze_levels`),
...
function createLevelAnalysisStore() {
  let clips = $state<MerulaClipWindow[]>([]);
  ...
  return {
    ...
    async analyze(source: string, projectDir?: string): Promise<void> {
      ...
      running = true;
      try {
        const res = await merulaAnalyzeLevels(source, projectDir);
        clips = res.clips;
