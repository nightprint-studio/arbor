/**
 * merula config store — reactive mirror of the typed `[merula]` settings
 ...
 * the merula window boot), setters persist.
 */
...
const DEFAULT_RENDER: MerulaRenderConfig = { sample_rate: 48_000, bit_depth: 'int24', tail_max_secs: 4.0, format: 'wav' };

function createConfigStore() {
  let defaultOctave = $state(4);
  let defaultCps    = $state(0.5);
  let logThreshold  = $state<string>('info');
  let render        = $state<MerulaRenderConfig>({ ...DEFAULT_RENDER });
  ...
  function snapshot(): MerulaConfig {
    return {
  ...
  function persist() { void setMerulaConfig(snapshot()).catch(() => {}); }
  ...
  return {
    ...
    async loadConfig() {
      try {
        const cfg = await getMerulaConfig();
    ...
    setOutputDevice(name: string | null) {
      ...
      outputDevice = name;
      void merulaSetOutputDevice(name).catch(() => {});
    },
    setRender(next: MerulaRenderConfig) {
      render = { ...next };
      persist();
    },
  };
