/**
 * merula Scratch / expression-evaluator panel state — **multi-tab**.
 *
 * The Scratch panel lets you take arbitrary `.merula` chunks — pasted, typed, or
...
function createScratchStore() {
  ...
  // Persist {id,name,source}+active to `<merula-data>/scratch.json` (the eval
  ...
  return {
    ...
    async restore() {
      try {
        const snap = await getMerulaScratchTabs();
    ...
    setResult(id: string, r: MerulaSnippetEval | null) { const t = find(id); if (t) t.result = r; },
