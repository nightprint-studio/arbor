/**
 * merula "file structure" store — drives the floating structure popover (Ctrl+F12).
 *
 ...
 * a `MerulaSymbol` carrying the UTF-16 offset (exact jump via `merulaStore.requestGoto`)
 * and a 1-based line. Window-local UI state, rune-store pattern (factory + getters).
 */

import type { MerulaSymbol } from '../editor/merula-lang';

function createStructureStore() {
  let open  = $state(false);
  let items = $state<MerulaSymbol[]>([]);
  ...
  return {
    ...
    openWith(next: MerulaSymbol[]) { items = next; open = true; },
    /** Close the picker. */
    close() { open = false; },
  };
}
