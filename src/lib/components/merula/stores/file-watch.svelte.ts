/**
 * merula file-watch store — detects external changes to the open `.merula` file on
...
import { merulaEngine } from './engine.svelte';
...
function createFileWatchStore() {
  ...
  return {
     ...
     *     `.merula` shows without a manual reload.
    ...
    reload() {
      ...
      if (projectStore.activeFilePath === p.path) {
        void merulaEngine.eval(p.content, projectStore.project?.path);
      }
    },
