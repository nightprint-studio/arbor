 ...
 *
 * These ride the existing seek (`merulaEngine.seek`) rather than adding real-time
 * transport state: the loop is enforced by seeking back to its start when the
 * playhead crosses the end (the watch lives in {@link MerulaShell}, always
...
function createTransportUiStore() {
  ...
  return {
    ...
    syncMetronome() { void merulaSetMetronome(metronome); },
    ...
    syncCountIn() { void merulaSetCountIn(countIn); },
    ...
    seekNextMarker(): Marker | null {
      ...
      if (m) { cursor = m.cycle; void merulaEngine.seek(m.cycle); }
      return m;
    },
