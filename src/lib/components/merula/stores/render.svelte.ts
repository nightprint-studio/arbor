/**
 * merula export/render status — drives the title-bar feedback for the offline WAV
 * bounce. The render runs as a background job, so `merulaRender` resolves with a
...
function createRenderStore() {
  ...
  // The backend render job id, known once `merulaRender` resolves. Held so the
   ...
   *  transfer cancelled. If the job id isn't known yet (the `merulaRender` call
  ...
  return {
    ...
    /**
     * Track a render: `promise` is the `merulaRender` call (resolves to a job id),
