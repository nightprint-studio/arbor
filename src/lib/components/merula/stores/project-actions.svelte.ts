/**
 * merula project-action launcher — the single owner of the file/project picker
 ...
 * keyboard shortcuts in MerulaShell — drives the SAME picker + confirm logic
 ...
 * picker is open; `MerulaProjectActions.svelte` renders it once for the window.
 */
...
/** Starter content for a freshly-created `.merula` file. */
const STARTER_MERULA = `cps(0.5)
...
function createProjectActions() {
  ...
  let picker = $state<MerulaPicker>(null);
  ...
  // silently rewrites the global config), then sent as `merula_render` opts.
  ...
  function onConfirm(path: string) {
    ...
    if (mode === 'new') {
      ...
      // name + "for whom" form is a follow-up). Writes merula.toml + a starter.
      ...
      void (async () => {
        try {
          await fsWriteTextFile(path, STARTER_MERULA);
  ...
  async function runMidiExport(path: string) {
    ...
    try {
      const r = await merulaExportMidi(
        projectStore.activeSource,
        path,
        projectStore.project?.path,
      );
  ...
  return {
    ...
    /** Persist the chosen format to the merula render config (sticky across
    ...
    /** Open the "new merula project" folder picker. */
    newProject()  { picker = 'new'; },
    /** Open the "new .merula file" save picker (in the open project). With no
     *  project open there's nowhere to put it, so fall back to New Project. */
    newFile()     { picker = projectStore.project ? 'new-file' : 'new'; },
    /** Open the "open merula project" folder picker. */
    openProject() { picker = 'open-project'; },
    /** Open the "open .merula file" picker. */
