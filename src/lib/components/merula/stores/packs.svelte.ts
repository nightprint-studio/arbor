/**
 * merula sample-packs store — install status of every downloadable pack (VSCO 2,
 ...
 *
 * Two backend streams drive the store: `merula:pack_progress` (live %/phase) and
...
function createPacksStore() {
  let packs    = $state<MerulaPack[]>([]);
  // Keyed by pack id; a pack is "downloading" while it has a live job id.
  let progress = $state<Record<string, MerulaPackProgress | null>>({});
  ...
  async function refreshPacks() {
    try { packs = await merulaPacks(); } catch { /* keep last */ }
  }
  ...
  return {
    ...
    async download(id: string) {
      ...
      try {
        const job = await merulaPackDownload(id);
    ...
    async subscribe(): Promise<UnlistenFn> {
      const unProgress = await onMerulaPackProgress((p) => {
