 ...
 * engine store; this one owns the *one* pull the arrangement needs: `merula_query`
 ...
 * inline `merula_eval` result + the `merula:diagnostics` echo coalesce into one
...
function maxPolyphony(haps: MerulaQueryHap[]): number {
  let sustained = 0;
...
function buildLanes(haps: MerulaQueryHap[], allSections: MerulaQuerySection[]): VizLane[] {
  const byTrack = new Map<number, MerulaQueryHap[]>();
  for (const h of haps) {
    const arr = byTrack.get(h.track);
    if (arr) arr.push(h);
    else byTrack.set(h.track, [h]);
  }
  const secByTrack = new Map<number, MerulaQuerySection[]>();
...
function createArrangementStore() {
  let haps     = $state<MerulaQueryHap[]>([]);
  let sections = $state<MerulaQuerySection[]>([]);
  ...
  async function run(cycles: number) {
    loading = true;
    try {
      const res = await merulaQuery(cycles);
  ...
  return {
     ...
     *  `merula_eval` result and the `merula:diagnostics` echo both bump the trigger;
