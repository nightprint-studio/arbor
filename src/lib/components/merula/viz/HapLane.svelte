<script lang="ts">
  /**
   * One arrangement lane drawn from **real haps** (the `merula_query` result),
  ...
  const velOf = (h: MerulaQueryHap) => Math.max(0, Math.min(1, h.gain ?? 1));
  ...
  function hapTip(h: MerulaQueryHap) {
  ...
  function pick(h: MerulaQueryHap, e: MouseEvent) {
