<script lang="ts">
   ...
   *  a symbol to jump the editor to its declaration (via the MerulaShell relay).
  ...
  import { outlineTreeFromSource, type MerulaOutlineNode } from '../editor/merula-lang';

  /** Play just this declaration one-shot (isolated, dependency-aware snippet). */
  function playNode(e: MerulaOutlineNode, ev: MouseEvent) {
    ev.stopPropagation();
    if (e.playSource) void merulaEngine.playSnippet(e.playSource, projectStore.project?.path);
  }

  let entries = $state<MerulaOutlineNode[]>([]);
...
<PanelShell title="Outline" count={entries.length}>
  ...
  {#if !projectStore.activeFilePath}
    <EmptyState message="Open a .merula file to see its tracks, functions and constants." />
          ...
          >
            ...
            {#each items as e (e.id)}
              {#if g.kind === 'track' && e.children?.length}
                ...
                <SidebarItem onclick={() => merulaStore.requestGoto(e.offset, e.line)}>
                  {#snippet icon()}
                ...
                {#if isTrackOpen(e.id)}
                  {#each secs as s (s.id)}
                    <SidebarItem indent={26} onclick={() => merulaStore.requestGoto(s.offset, s.line)}>
