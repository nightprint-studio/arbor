<script lang="ts">
   ...
   * compiled into a tiny `.merula` snippet (see `snippet.ts`) and sent to
   * `merula_audition_expr`, which evaluates it with the real language and plays one
  ...
  function fire(trigger: { note?: number | null; degree?: number | null }) {
    if (!inst) return;
    merulaAuditionExpr(buildSnippet(inst, controls(), trigger)).catch(() => { /* engine not ready */ });
  }
  function play(value: number) { fire(scaleMode ? { degree: value } : { note: value }); }
</script>

<div class="ipp">
  <BottomPanelHeader title="Preview" onClose={() => merulaStore.toggleBottom('preview')}>
