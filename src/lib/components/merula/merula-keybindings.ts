/**
 * Merula keybindings (Step 0). The first, intentionally small set from ui.md —
 ...
 *
 * Ctrl+Click (go-to-declaration) is handled inside the MerulaEditor CodeMirror
 * mousedown handler (it needs the click target), so it isn't a keydown binding here.
 */

export interface MerulaBinding {
  id:    string;
  key:   string;
  ctrl?: boolean;
  shift?: boolean;
...
export const MERULA_BINDINGS: MerulaBinding[] = [
  // Editor-scoped (only when the tab pane has focus).
  { id: 'goto_line',    key: 'g', ctrl: true,  scope: 'editor', description: 'Go to line' },
  { id: 'new_file',     key: 'n', ctrl: true,  scope: 'editor', description: 'New .merula in the tab pane' },
  ...
  // devtools toggle.
  { id: 'import_audio', key: 'i', alt: true, shift: true, scope: 'global', description: 'Import audio / MIDI as .merula' },
  // Mixer.
...
export function matchesMerula(e: KeyboardEvent, b: MerulaBinding): boolean {
