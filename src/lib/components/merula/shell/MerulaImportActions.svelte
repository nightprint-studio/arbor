<script lang="ts">
  /**
   * Renders the merula audio/MIDI import dialogs once per window, driven by the
   * `importActions` store. Mounted in MerulaShell next to MerulaProjectActions so
...
{#if importActions.choiceFor}
  <Modal onClose={importActions.cancel} size="sm" ariaLabel="Import audio">
    ...
    <ul class="opts">
      <li><strong>Import as .merula</strong> — transcribe it and open an editable
        <code>.merula</code> file (the intermediate MIDI stays in memory).</li>
    ...
    {#snippet footer()}
      <ModalFooter align="between">
        ...
        <div style="display:flex; gap:8px">
          ...
          <Button variant="primary" onclick={importActions.chooseImport}>Import as .merula</Button>
        </div>
      </ModalFooter>
    {/snippet}
  </Modal>
