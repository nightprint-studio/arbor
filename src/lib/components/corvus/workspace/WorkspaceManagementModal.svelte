<script lang="ts">
  ...
  import { SCRATCH_ID, workspaceColorVar, WS_COLOR_COUNT } from '$lib/types/corvus/workspace';
  import type {
  ...
  import { validateRepoPaths, validateRepoPath, type RepoPathStatus } from '$lib/ipc/corvus/missing';
  ...
  async function openRepoTab(entry: RepoRegistryEntry, sourceWsId: string) {
    try {
      ...
      const { openRepo } = await import('$lib/ipc/corvus/graph');
