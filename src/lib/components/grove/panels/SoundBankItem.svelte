<script lang="ts">
   ...
   *
   * Grove-local — composes shared/ui chrome + the tooltip action; the data shape
   * (`GroveInstrument`) is grove-specific, so this lives under grove/panels.
   */
  ...
  let { inst }: { inst: GroveInstrument } = $props();
