<script lang="ts">
  /**
   * Jobs — the grove window's background-job output panel. Renders the same job
   ...
   * `jobsStore` (already mounted + target-routed by `<FeedbackHost id="grove">`
   * in GroveWindow) is the single source of truth, and the output drill-in
   * composes the shared `LogStream`. Only the panel chrome is grove-local
   * because the bottom-panel routing is grove-specific (groveStore). If a second
  ...
  // grove jobs are never hidden, but keep the showHidden filter honest in case
  // a future grove job opts in — mirror the overlay's listing semantics.
...
<div class="jobs">
  {#if activeJob}
    <!-- ── Output drill-in ─────────────────────────────────────────────────── -->
    <BottomPanelHeader title={activeJob.name} onClose={() => groveStore.toggleBottom('jobs')}>
      {#snippet icon()}
  ...
  {:else}
    <!-- ── Job listing ─────────────────────────────────────────────────────── -->
    <BottomPanelHeader title="Jobs" onClose={() => groveStore.toggleBottom('jobs')}>
