<script lang="ts">
  /** Files panel — the open project's `.grove` files, grouped by folder. Click
...
{#snippet fileRow(f: GroveProjectFile)}
...
<PanelShell title="Files" count={files.length}>
  ...
  {#snippet actions()}
    <button class="ps-btn ps-btn-accent" onclick={() => projectActions.newFile()} use:tooltip={'New .grove (Ctrl+N)'} aria-label="New file"><Plus size={14} /></button>
  {/snippet}
