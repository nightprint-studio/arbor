/**
 * grove language intelligence for CodeMirror — autocomplete + hover docs.
 *
 * Both features are driven by the **canonical DSL catalogue** (`referenceStore`,
 * fed by `grove_lang_reference`) plus the file's **local symbols** (let / fn /
 * track / import) read straight from the live Tree-sitter tree (`grove-lang`).
 ...
 *
 *   - {@link groveCompletion} — a `CompletionSource`. After `.` it offers the
 ...
 *   - {@link groveHover} — a `hoverTooltip` that renders the signature, summary,
...
function symbolType(kind: GroveSymbolKind): string {
...
function entryCompletion(e: GroveDslEntry): Completion {
  return {
    label: e.name,
...
function localSymbols(view: EditorView | undefined): GroveSymbol[] {
  if (!view) return [];
...
/**
 * The grove completion source. Detects method position (a `.` immediately before
...
export function groveHover(src: GroveIntelSource): Extension {
 ...
 *  example). Styled by `.cm-grv-doc*` classes (see grove-cm theme). */
function infoDom(e: GroveDslEntry): HTMLElement {
...
function symbolDom(s: GroveSymbol): HTMLElement {
  const root = document.createElement('div');
  root.className = 'cm-grv-doc';
  const sig = document.createElement('div');
  sig.className = 'cm-grv-doc-sig';
...
export function groveLanguageIntel(src: GroveIntelSource): Extension {
  return [
    autocompletion({
      override: [groveCompletion(src)],
