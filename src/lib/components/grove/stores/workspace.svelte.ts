/**
 * grove workspace store — reactive mirror of the persisted grove-window state
 * (`get_grove_state` / `set_grove_state`): the recent-projects list, the
 * last-opened project (reopened on launch) and the panel layout. NOT
 * localStorage and NOT the per-project grove.toml — this is the dedicated grove
...
import {
  getGroveState, setGroveState,
  type GroveLayoutState, type GroveWorkspaceState,
} from '$lib/ipc/grove';

const RECENTS_CAP = 10;
const LAYOUT_PERSIST_DELAY = 400;

const DEFAULT_LAYOUT: GroveLayoutState = {
  left_panel:    'files',
...
function createWorkspaceStore() {
  ...
  let layout         = $state<GroveLayoutState>({ ...DEFAULT_LAYOUT });
  let loaded         = $state(false);

  function snapshot(): GroveWorkspaceState {
    return {
  ...
  function persist() { void setGroveState(snapshot()).catch(() => {}); }
  ...
  return {
    ...
    persistLayout(next: GroveLayoutState) {
      layout = { ...next };
