...
<p>Beyond tabs, Arbor opens several top-level windows — Canopy, one or more Git windows, the File Explorer, Merula, Bennu, Picus, Garrulus, Tyto. Each window is named after what it is showing (the repository, project or folder), so it is recognisable in the taskbar, in Alt-Tab and in the macOS Window menu.</p>
<p><strong>Garrulus</strong> is the notes window: a vault of plain markdown files — links, tags, typed frontmatter — kept in step across machines from one button in its title bar. Nothing there syncs on a timer; the background only checks, and everything that writes is something you pressed. It has its own documentation under <kbd>F1</kbd> once it is open.</p>
