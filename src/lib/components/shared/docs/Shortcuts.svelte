...
<table class="shortcuts-table">
  ...
  <tbody>
    ...
    <tr><td><kbd>Ctrl</kbd>+<kbd>Shift</kbd>+<kbd>R</kbd></td><td>Start a Tyto capture — or stop the running recording. Registered system-wide so it works even when Arbor isn't focused. Off by default and rebindable; enable / change it in Tyto's settings. The tray icon (menu-bar extra on macOS) carries the same actions</td></tr>
...
<h2>merula (Music live-coding)</h2>
<p class="hint">
  The merula window keeps its own binding set (it's a self-contained studio, not the
  main Git UI), so these are fixed rather than rebindable. Editor-scoped keys fire
  only when the code tab pane has focus. See the <strong>merula (Music)</strong> page
...
<table class="shortcuts-table">
  ...
  <tbody>
    ...
    <tr><td><kbd>Ctrl</kbd>+<kbd>Shift</kbd>+<kbd>P</kbd></td><td>Command Palette (merula)</td></tr>
    ...
    <tr><td><kbd>Ctrl</kbd>+<kbd>N</kbd></td><td>New <code>.merula</code> file (editor pane)</td></tr>
    ...
    <tr><td><kbd>Ctrl</kbd>+<kbd>,</kbd></td><td>Open merula Settings</td></tr>
    <tr><td><kbd>F1</kbd></td><td>Show merula keyboard shortcuts</td></tr>
  </tbody>
</table>
