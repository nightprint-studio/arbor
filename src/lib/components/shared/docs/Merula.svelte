...
<h1>merula — Music live-coding</h1>

<p class="doc-lead">merula is a standalone music live-coding studio that ships inside Arbor — its own window with a code editor, a read-only arrangement view, a mixer, and a live audio engine. You write music as <code>.merula</code> source; the engine evaluates it and plays it back in real time.</p>

<div class="hint">merula is <strong>code-first</strong>: the source is the instrument. The panels are feedback and fine-tuning — the arrangement, mixer and inspector reflect what the code produces, they don't replace it.</div>

<h2>Opening merula</h2>
<p>Open the merula window from the Command Palette — search for <em>Open merula (Music)</em>. It opens in its own window, independent of any repository, so you can keep it alongside your work (a game's <code>music/</code> folder can live in the same project as its code).</p>

<h2>Projects</h2>
<p>A merula <strong>project</strong> is a folder with a <code>merula.toml</code> manifest (a <code>name</code> and an <code>audience</code> — "who the music is for") plus its <code>.merula</code> files. Files listed under <code>libraries</code> are imported-only: their <code>tracks(…)</code> output is ignored, only their <code>fn</code> / <code>let</code> declarations are exported for re-use.</p>
<ol class="step-list">
  <li>Create a project (<kbd>Ctrl</kbd>+<kbd>Shift</kbd>+<kbd>N</kbd>) or open one (<kbd>Ctrl</kbd>+<kbd>O</kbd>) — pick a folder.</li>
  <li>The entry <code>.merula</code> opens as an editor tab; the <strong>Files</strong> panel lists the rest.</li>
...
<div class="feature-grid">
  <div class="feature-card">
    ...
    <div class="fc-desc">CodeMirror with merula syntax highlighting, inline error underlines, and a live highlight of whatever is sounding right now. Edits re-evaluate automatically. Autocomplete (<kbd>Ctrl</kbd>+<kbd>Space</kbd>) suggests the language's combinators, transforms and your own declarations — and, inside <code>inst("…")</code>, every instrument the engine can resolve, and inside <code>art("…")</code>, the available articulations; hover a name for its signature and docs. The usual editing comforts are there too: comment toggling, bracket / quote autoclosing, soft wrapping, and folding of multi-line blocks. If the open file changes on disk outside merula, it offers to reload it.</div>
  </div>
  <div class="feature-card">
  ...
  <div class="feature-card">
    ...
    <div class="fc-desc">Reuse <code>.merula</code> modules from GitHub. Declare them in <code>merula.toml</code> under <code>[libraries]</code> (e.g. <code>drums = "github:owner/repo@v1"</code>), then import their <code>let</code> / <code>fn</code> with <code>import &#123; … &#125; from "$lib/drums/groove.merula"</code>. <strong>Sync libraries</strong> (Command Palette) downloads each one pinned to a commit SHA into a shared cache and records it in <code>merula.lock</code> for reproducible builds; opening a project auto-fetches any that are missing. Public GitHub repos.</div>
  </div>
  <div class="feature-card">
...
<ul class="prop-list">
  ...
  <li><code>import {'{'} kick, snare {'}'} from "lib/drums.merula"</code> pulls top-level declarations from another file (its own <code>tracks(…)</code> output is ignored).</li>
...
<ul class="prop-list">
  ...
  <li><code>room</code> (mixer) and <code>delay</code> (inspector: time / feedback / mix) are <strong>code-first</strong>: turning a knob writes the literal straight into the track's <code>.merula</code> source — adding the method to the chain if it isn't there yet — and re-evaluates.</li>
...
<p>The merula window has its own Command Palette (<kbd>Ctrl</kbd>+<kbd>Shift</kbd>+<kbd>P</kbd>) listing every window action — transport, project operations, panel toggles, settings. Type to filter, <kbd>↑</kbd> / <kbd>↓</kbd> to move, <kbd>Enter</kbd> to run.</p>

<h2>Keyboard shortcuts</h2>
<p>The full merula cheat-sheet is in the window's gear menu under <em>Keyboard Shortcuts</em> (<kbd>F1</kbd>). The essentials:</p>
<table class="shortcuts-table">
  ...
  <tbody>
    ...
    <tr><td><kbd>Ctrl</kbd>+<kbd>Shift</kbd>+<kbd>O</kbd></td><td>Open a <code>.merula</code> file</td></tr>
    <tr><td><kbd>Ctrl</kbd>+<kbd>N</kbd></td><td>New <code>.merula</code> file (editor)</td></tr>
    ...
    <tr><td><kbd>Alt</kbd>+<kbd>Shift</kbd>+<kbd>I</kbd></td><td>Import audio / MIDI as <code>.merula</code></td></tr>
...
<h2>Settings</h2>
<p>merula's settings live in the typed <code>[merula]</code> section of the global Arbor config (open them from the gear menu or <kbd>Ctrl</kbd>+<kbd>,</kbd>). Changes apply immediately.</p>
<ul class="prop-list">
