...
<h1>Git Flow</h1>
<p class="doc-lead">Arbor includes a built-in Git Flow implementation based on the <strong>Vincent Driessen branching model</strong> — structured workflows for feature, release, and hotfix branches, with optional PR/MR integration and ticket-based branch naming.</p>
...
<ul>
  ...
  <li>The <strong>first time</strong> you start a feature or release in this mode for a given project, a confirmation dialog explains that the branch will be cut from <code>main</code>. Confirming the dialog stores an acknowledgement per project — subsequent starts go through silently.</li>
...
<ul>
  <li><strong>Start</strong> — creates <code>feature/&lt;name&gt;</code> from <code>develop</code> (or from <code>main</code> if <code>develop</code> doesn't exist) and checks it out</li>
  <li><strong>Finish</strong> — merges feature branch into <code>develop</code> with <code>--no-ff</code> (or into <code>main</code> when <code>develop</code> is missing); optionally deletes the branch after</li>
</ul>

<h3>Release branches</h3>
<ul>
  <li><strong>Start</strong> — creates <code>release/&lt;version&gt;</code> from <code>develop</code> (falls back to <code>main</code> when <code>develop</code> is missing)</li>
  <li><strong>Finish</strong> — merges into <code>main</code> and, when present, into <code>develop</code>; optionally creates a version tag</li>
</ul>

<h3>Hotfix branches</h3>
<ul>
  <li><strong>Start</strong> — creates <code>hotfix/&lt;name&gt;</code> from <code>main</code> (the production branch)</li>
  <li><strong>Finish</strong> — merges into both <code>main</code> and <code>develop</code>; optionally creates a tag</li>
</ul>
...
<table class="shortcuts-table">
  ...
  <tbody>
    <tr><td><code>finish.feature_use_pr</code></td><td><strong>Force</strong> PR/MR — feature finish always pushes the branch and opens the PR/MR form (no local merge)</td></tr>
...
<p>When a finish type is <strong>not</strong> forced, the Finish button becomes a <strong>split button</strong>: the primary click uses the configured default, and the chevron <code>▾</code> lets you choose between "Finish normally (merge locally)" and "Finish with PR/MR" for that individual operation.</p>
<p>Configure in <strong>Settings → Git Flow</strong>. Each setting can be overridden per project.</p>

<h2>Ticket-based branch naming</h2>
<p>When an issue tracker is configured for the project (see <strong>Settings → Repository → Issue Tracker</strong>), the "Start Feature" form shows a <strong>Ticket</strong> field with a picker button. Clicking it opens a full-screen modal with the same search and filter interface as the Issues sidebar — search bar, status / project / milestone / assignee chips — and issue cards with colored status icons, labels, and assignees.</p>
...
<ul>
  <li>The ticket picker is available <strong>by default</strong> whenever a tracker is configured — no flag required.</li>
...
<ul>
  <li><strong>Global</strong> — in <code>~/.config/arbor/config.toml</code> under <code>[gitflow]</code> — applies to all repositories</li>
  <li><strong>Per-repo</strong> — in <code>&lt;repo&gt;/.arbor/config.toml</code> under <code>[gitflow]</code> — overrides the global config for that repo only</li>
</ul>
<p>Both layers are editable from <strong>Settings → Git Flow</strong>.</p>
