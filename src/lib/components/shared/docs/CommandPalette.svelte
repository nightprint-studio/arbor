...
<table class="shortcuts-table">
  ...
  <tbody>
    ...
    <tr><td><kbd>â†‘</kbd> / <kbd>â†“</kbd></td><td>Move selection up / down</td></tr>
...
<ul>
  <li><strong>Branch</strong> â€” <em>Checkout</em>, <em>Merge</em>, <em>Delete Branch</em>, <em>Rename Branch</em>, <em>Push Branch</em>, <em>Focus Branch in Graph</em></li>
  <li><strong>Navigate</strong> â€” <em>Go to Commit</em>, <em>Go to Tag</em>, <em>Blame File</em>, <em>Show Commits Touching File</em></li>
  <li><strong>Commit</strong> â€” <em>Cherry-pick</em>, <em>Revert Commit</em>, <em>Reset Soft / Mixed / Hard</em>, <em>Create Branch Here</em>, <em>Create Tag</em> (Enter on empty input tags HEAD), <em>Copy Commit SHA</em></li>
  <li><strong>Stash</strong> â€” <em>Apply Stash</em>, <em>Pop Stash</em>, <em>Drop Stash</em></li>
  <li><strong>Tag</strong> â€” <em>Delete Tag</em>, <em>Push Tag</em></li>
  <li><strong>Remote</strong> â€” <em>Fetch from Remote</em>, <em>Pull from Remote</em>, <em>Push Branch to Remote</em></li>
  <li><strong>Tabs</strong> â€” <em>Switch Tab</em>, <em>Close Tab</em></li>
  <li><strong>Repository</strong> â€” <em>Open Recent Repository</em></li>
  <li><strong>Merge Requests</strong> â€” <em>Open Pull / Merge Request</em> (opens the create MR/PR modal)</li>
  <li><strong>Appearance</strong> â€” <em>Switch Theme</em></li>
  <li><strong>Repository actions (leaves)</strong> â€” Open / Init / Clone / Reload Repository</li>
  <li><strong>Workspaces</strong> â€” <em>Switch Workspace</em>, <em>Open Project</em>, <em>Open from Workspace</em>, Manage Workspaces, Create Workspace</li>
  <li><strong>Worktrees</strong> â€” <em>Worktree Info</em>, <em>Switch Worktree</em></li>
  <li><strong>Deep Links</strong> â€” <em>Copy arbor:// Link to Commit / Checkout Branch / Branch Worktree / MR</em> (the <em>Open Repository</em> link is a leaf action under <strong>Copy</strong>)</li>
  <li><strong>Linked Worktrees</strong> â€” Manage Linked Worktrees, Link this Worktreeâ€¦, Unlink from "&lt;link&gt;", Enable / Disable Sync for "&lt;link&gt;" (latter four shown only when applicable to the current repo)</li>
  <li><strong>Tabs (leaves)</strong> â€” Close Current Tab, Next / Previous Tab</li>
  <li><strong>Git (leaves)</strong> â€” Pull, Push, Fetch All Remotes, New Branch, Stash Changes</li>
  <li><strong>Stage &amp; Commit</strong> â€” Commit, Amend Last Commit, Stage All, Unstage All, Discard All, Undo Last Commit</li>
  <li><strong>Rebase / Merge</strong> â€” Continue / Skip / Abort Rebase, Abort Merge (visible only while the repo is in that state)</li>
  <li><strong>Panels</strong> â€” Toggle Stage / Detail / Terminal / Jobs / Notifications / Sidebar; Show Branches / Git Flow / MRs / Issues / Files / Reflog / Stats / Pipelines</li>
  <li><strong>Copy</strong> â€” Copy Current Branch Name, Copy Current SHA, Copy <code>origin</code> URL, <em>Copy arbor:// Link to Open Repository</em></li>
  <li><strong>System</strong> â€” Settings, Plugin Manager, Reload Plugins, Documentation, About Arbor</li>
  <li><strong>Submodules</strong> â€” Update All Submodules</li>
  <li><strong>Navigation</strong> â€” Jump to HEAD, Open in IDE</li>
  <li><strong>Open With</strong> â€” one entry per detected / custom IDE (only when a repo is open)</li>
  <li><strong>Plugin Commands</strong> â€” registered via <code>arbor.command.register()</code></li>
</ul>
<p>Verb commands show a <code>â€º</code> chevron on the right to indicate they open a target picker. Leaf commands execute immediately. Conditional leaves (e.g. <em>Continue Rebase</em>, <em>Unstage All</em>) only show when the action is applicable.</p>

<h3>Phase 2 â€” Target picker</h3>
<p>Selecting a verb inserts a coloured chip at the start of the input (e.g. <kbd>âŒ¥ Checkout â€º</kbd>) and the list becomes the verb's target set. The input placeholder flips to match the verb's target â€” <em>"Filter branchesâ€¦"</em>, <em>"Filter stashesâ€¦"</em>, <em>"Filter remotesâ€¦"</em>, etc. <kbd>Enter</kbd> runs the verb on the highlighted row; clicking the chip (or <kbd>Backspace</kbd> on empty input) removes it and returns to Phase 1.</p>
...
<table class="shortcuts-table">
  ...
  <tbody>
    ...
    <tr><td><code>Merge</code></td><td>â€”</td><td>Merges the branch into HEAD</td></tr>
...
<table class="shortcuts-table">
  ...
  <tbody>
    ...
    <tr><td><code>Go to Commit</code></td><td><code>commit</code>, <code>commits</code></td><td>commit</td><td>Full-text commit search (summary, author, hash) â€” min. 2 characters</td></tr>
    <tr><td><code>Blame File</code></td><td><code>blame</code>, <code>annotate</code></td><td>project-file</td><td>Opens the Git Blame modal for any file in the project â€” does <em>not</em> touch the File Tree sidebar</td></tr>
    <tr><td><code>Show Commits Touching File</code></td><td><code>file-history</code>, <code>log-file</code>, <code>history</code></td><td>project-file</td><td>Filters the graph by a file picked from the full project â€” does <em>not</em> open the File Tree sidebar</td></tr>
    ...
    <tr><td><code>Reset Mixed</code></td><td>â€”</td><td>commit</td><td>Move HEAD + reset index; keep workdir</td></tr>
    <tr><td><code>Reset Hard</code></td><td><code>rh</code></td><td>commit</td><td><strong>Destructive</strong> â€” requires confirmation. Resets HEAD, index and workdir</td></tr>
    ...
    <tr><td><code>Create Tag</code></td><td><code>th</code>, <code>tag-here</code>, <code>create-tag</code></td><td>commit</td><td>Top entry <code>here</code> (selected by default â€” Enter creates a tag at HEAD); type a commit term to pre-seed the modal elsewhere</td></tr>
...
<table class="shortcuts-table">
  <thead><tr><th>Command</th><th>Aliases</th><th>Target</th><th>What it does</th></tr></thead>
  <tbody>
    <tr><td><code>Worktree Info</code></td><td><code>wt</code>, <code>wtinfo</code>, <code>worktree</code></td><td>worktree</td><td>Opens the info panel for any worktree of the active project â€” same modal as the sidebar list, but reachable without expanding the <em>Worktrees</em> section</td></tr>
    <tr><td><code>Switch Worktree</code></td><td><code>wts</code>, <code>switch-wt</code></td><td>worktree</td><td>Swaps the active tab's context to the chosen worktree (or focuses an existing tab on that path) â€” same logic as double-clicking a row in the sidebar</td></tr>
  </tbody>
</table>
...
<p>Build a shareable <code>arbor://</code> URL and copy it to the clipboard. The active tab's first remote is embedded as <code>?url=</code>, so the link resolves on any machine that has access to the same remote. If the repository has no remote configured, the palette toasts a warning rather than producing a non-shareable link â€” see the <em>Deep Links</em> doc page for the full URL schema.</p>
<table class="shortcuts-table">
...
<p>Typing a verb name (or any alias) followed by a space â€” or a colon â€” promotes it to a chip immediately and keeps whatever you typed after as the target filter. This lets power users skip the list entirely:</p>
<ul>
  <li><code>co main</code> â†’ chip <kbd>Checkout</kbd>, filter <em>main</em></li>
  <li><code>merge develop</code> â†’ chip <kbd>Merge</kbd>, filter <em>develop</em></li>
  <li><code>tag:v1</code> â†’ chip <kbd>Go to Tag</kbd>, filter <em>v1</em></li>
  <li><code>rm feature/old</code> â†’ chip <kbd>Delete Branch</kbd>, filter <em>feature/old</em></li>
  <li><code>cp fix</code> â†’ chip <kbd>Cherry-pick</kbd>, filter commits containing <em>fix</em></li>
  <li><code>apply WIP</code> â†’ chip <kbd>Apply Stash</kbd>, filter stashes containing <em>WIP</em></li>
  <li><code>tab:docs</code> â†’ chip <kbd>Switch Tab</kbd>, filter tabs whose name contains <em>docs</em></li>
  <li><code>wt feature/api</code> â†’ chip <kbd>Worktree Info</kbd>, filter worktrees whose branch contains <em>feature/api</em></li>
  <li><code>linkc bug-fix</code> â†’ chip <kbd>Copy arbor:// Link to Commit</kbd>, filter commits matching <em>bug-fix</em></li>
</ul>
...
<ul>
  <li><em>Delete Branch</em>, <em>Delete Tag</em>, <em>Drop Stash</em> â€” native <code>confirm()</code> prompt</li>
  <li><em>Reset Hard</em> â€” lists the target SHA in the prompt</li>
  <li><em>Discard All Changes</em> â€” same</li>
  <li><em>Undo Last Commit</em> â€” shows the parent SHA that HEAD will move to</li>
</ul>

<h2>Open With â€” launching an IDE</h2>
<p>The <strong>Open With</strong> section is populated from your IDE configuration in <em>Settings â†’ IDE Integration</em>:</p>
<ul>
  ...
  <li>The IDE is launched <strong>detached</strong> â€” closing Arbor does not close the IDE</li>
</ul>
...
<ul>
  <li>Exact match â†’ 100</li>
  <li>Prefix match â†’ 85</li>
  <li>Word-boundary match â†’ 70</li>
  <li>Substring match â†’ 55</li>
  <li>Fuzzy (all characters present in order) â†’ 30</li>
  <li>No match â†’ hidden</li>
</ul>
...
<h2>Plugin API â€” <code>arbor.command</code></h2>
...
<p>When the user selects a plugin command the palette fires <code>fire_plugin_action(plugin_name, "command:&lt;id&gt;", "&#123;&#125;")</code>. Register the handler with <code>arbor.events.on("command:&lt;id&gt;", fn)</code> â€” the same mechanism used for activity-bar actions and keybindings.</p>
