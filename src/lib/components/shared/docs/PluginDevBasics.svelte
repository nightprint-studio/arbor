...
<h1>Plugin Development â€” Basics</h1>
...
<dl class="meta-grid">
  <dt>Runtime</dt><dd>Lua 5.4 (vendored) â€” no system Lua needed</dd>
  <dt>Manifest</dt><dd><code>plugin.toml</code> â€” required</dd>
...
<p>Open the <strong>Plugin Manager</strong> (Activity Bar â†’ puzzle icon) â€” the top-right toolbar exposes two shortcuts that avoid hand-editing files on disk:</p>
<div class="feature-grid two-col">
  <div class="feature-card">
    <div class="fc-title">Import from <code>.zip</code></div>
    <div class="fc-desc">
      The <strong>Upload</strong> icon opens a file picker; pick a plugin archive and Arbor extracts it into <code>plugins/&lt;name&gt;/</code>, then reloads. The zip must contain a top-level <code>plugin.toml</code> (either at the archive root or inside a single wrapping folder). Existing folders with the same name are refused â€” delete the old copy first. Imported plugins land <strong>disabled by default</strong> â€” review the manifest's <code>[permissions]</code>, then click the <strong>Power</strong> icon on the plugin card to enable.
    </div>
  </div>
  <div class="feature-card">
    <div class="fc-title">Export Template wizard</div>
    <div class="fc-desc">
      The <strong>Wand</strong> icon opens a 4-step wizard (Identity â†’ Permissions â†’ Hooks â†’ Recipes) that scaffolds a starter plugin and saves it as a zip. Each toggled recipe (command palette entry, keybinding, settings panel, modal form, toolbar action, sidebar, notification, background job, scheduler, HTTP) injects a canonical Lua snippet into <code>main.lua</code>. The bundle ships with <code>sdk.d.lua</code> + <code>.luarc.json</code> so lua-language-server provides <code>arbor.*</code> autocomplete in any editor.
    </div>
  </div>
</div>
<div class="hint">Once exported, unzip into <code>plugins/&lt;name&gt;/</code> and click <strong>Reload</strong> in the Plugin Manager â€” or hand the zip to another user and have them <em>Import</em> it.</div>

<h2>Managing installed plugins</h2>
<p>The Plugin Manager lists every plugin discovered in <code>plugins/</code>. Each row exposes a fixed action column on the right (left â†’ right):</p>
<table>
  <thead><tr><th>Icon</th><th>Action</th><th>Behaviour</th></tr></thead>
  <tbody>
    <tr><td>âš™ï¸ <strong>Settings</strong></td><td>Open the plugin's settings panel</td><td>Visible only when the plugin registered a settings container via <code>arbor.ui.settings.panel(...)</code>. Disabled while the plugin is off.</td></tr>
    <tr><td>â„¹ï¸ <strong>Info</strong></td><td>Open the <em>Plugin Info</em> modal</td><td>Detailed read-out of identity, permissions, hooks, schedulers + maintenance actions (see below).</td></tr>
    <tr><td>ðŸ—‘ <strong>Uninstall</strong></td><td>Permanently remove the plugin</td><td>Deletes the <code>plugins/&lt;name&gt;/</code> folder, the global <code>plugin_data/&lt;name&gt;/</code> store, every per-repo <code>.arbor/plugins/&lt;name&gt;/</code>, and the persisted enable-state. Shows a cascade-warning modal if other enabled plugins still depend on it.</td></tr>
    <tr><td>â» <strong>Power</strong></td><td>Enable / Disable</td><td>Persisted across restarts. Disabling stops every scheduler, fires <code>on_plugin_unload</code>, and closes any sidebar / panel that the plugin owned. Re-enabling reloads the plugin and re-fires <code>on_plugin_load</code>.</td></tr>
  </tbody>
</table>

<h3>Master kill-switch</h3>
<p>The toggle at the top of the modal â€” <em>Abilita gestione plugin</em> â€” is the <strong>master kill-switch</strong>. While it is off, the runtime is empty: nothing is loaded at startup, no schedulers fire, the contribution registry is wiped, and the per-plugin list is hidden until the switch is flipped back on. Useful for diagnosing whether a misbehaving plugin is the cause of an issue.</p>
...
<div class="feature-grid two-col">
  ...
  <div class="feature-card">
    ...
    <div class="fc-desc">One row per <code>arbor.scheduler.register</code> call with the action name, trigger summary (<code>every 5m</code>, <code>cron(â€¦)</code>, â€¦) and a per-action <strong>toggle</strong>. The header row exposes <em>Enable all</em> / <em>Disable all</em> bulk buttons. Toggling a single schedule calls <code>start_plugin_scheduler</code> / <code>stop_plugin_scheduler</code> on the backend; the change is in-memory only â€” restarting Arbor re-applies the manifest defaults.</div>
  </div>
  <div class="feature-card">
    <div class="fc-title">Permissions</div>
    <div class="fc-desc">Coloured pills (<span class="badge">safe</span> / <span class="badge badge-opt">warn</span> / danger) for filesystem scope, network allow-list, git capability tier and terminal access â€” same chips shown when reviewing imported plugins.</div>
  </div>
  <div class="feature-card">
  ...
  <div class="feature-card">
    <div class="fc-title">Maintenance Â· Open settings</div>
  ...
  <div class="feature-card">
    <div class="fc-title">Maintenance Â· Clear settings cache</div>
    <div class="fc-desc">Two-step destructive button (first click arms it red, second click confirms). Wipes every persisted setting written by this plugin (global + per-repo) â€” the plugin's own folder and code stay untouched. Use after schema breaking changes or to reset a misbehaving config.</div>
  </div>
</div>
<div class="hint">The Info modal stays in sync with backend events â€” reloading plugins or toggling the master switch refreshes its content automatically.</div>
...
<p>Set <code>doc_file = "doc.html"</code> in your manifest to expose plugin-specific documentation under the <strong>Plugins</strong> group in the left nav. Plain HTML â€” styles from the host docs apply automatically.</p>
...
<table>
  ...
  <tbody>
    ...
    <tr><td><code>&lt;p&gt;</code> Â· <code>&lt;ul&gt;</code> Â· <code>&lt;ol&gt;</code></td><td>Body text and lists</td></tr>
    <tr><td><code>&lt;strong&gt;</code></td><td>Bold, primary text colour</td></tr>
    <tr><td><code>&lt;code&gt;</code> Â· <code>&lt;pre&gt;&lt;code&gt;</code></td><td>Inline / block monospace</td></tr>
...
<ul class="prop-list">
  ...
  <li><strong>Unmet dep</strong>Missing or version-mismatched dependency â†’ plugin skipped, red banner on the card.</li>
  <li><strong>Optional</strong><code>optional = true</code> downgrades the error to a log warning. Your plugin still loads â€” guard calls that depend on the other plugin's presence.</li>
</ul>
...
<div class="feature-grid two-col">
  ...
  <div class="feature-card">
    ...
    <div class="fc-desc">Plugins that currently depend on yours â€” follow arrows backward to see who'd break if you disabled it.</div>
  </div>
</div>
...
<p>Declared once in <code>[permissions]</code> of <code>plugin.toml</code>. Capability is enforced at Lua call-time â€” trying to use a disabled API raises a runtime error.</p>
...
<table>
  ...
  <tbody>
    ...
    <tr><td rowspan="3"><code>fs_scope</code></td><td><code>[]</code> <span class="badge badge-opt">default</span></td><td>Sandboxed to the active repo's directory. <strong>Use <code>["*"]</code> instead when the plugin writes to user-picked paths via <code>arbor.ui.pick_file&#123; mode = "save" &#125;</code></strong> â€” the sandbox would otherwise reject anything outside the repo (e.g. <code>~/Downloads/foo.md</code>).</td></tr>
    <tr><td><code>["*"]</code></td><td>Unrestricted â€” any path on disk</td></tr>
    <tr><td><code>["/abs/path", â€¦]</code></td><td>Allow these absolute paths in addition to the active repo</td></tr>
    ...
    <tr><td><code>"read"</code></td><td><code>arbor.mr.list</code>, <code>arbor.mr.current_user</code>, <code>arbor.ci.runs</code> â€” credential-blind: tokens stay in the OS keyring</td></tr>
    ...
    <tr><td><code>["PATH", "JAVA_HOME"]</code></td><td>Allowlist â€” only listed names return a value, others return <code>nil</code></td></tr>
    <tr><td><code>service_export</code></td><td>bool</td><td><code>arbor.service.export / unexport / list_own</code> â€” expose callable services</td></tr>
    <tr><td><code>service_call</code></td><td>bool</td><td><code>arbor.service.call / list</code> â€” invoke services from other plugins</td></tr>
    <tr><td><code>settings_read_others</code></td><td>bool</td><td><code>arbor.settings.read(plugin, key)</code> â€” read other plugins' globals (own settings always readable)</td></tr>
  </tbody>
</table>

<div class="callout warning">
  <strong>Sandbox hardening</strong>
  These Lua functions are removed from the sandbox: <code>os.execute</code>, <code>os.exit</code>, <code>os.remove</code>, <code>os.rename</code>, <code>io.*</code>, <code>load</code>, <code>loadfile</code>, <code>dofile</code>. The <code>terminal</code> permission is captured at plugin load time â€” it cannot be escalated by overwriting a Lua global.
</div>
