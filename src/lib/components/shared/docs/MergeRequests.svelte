...
<p>The minimize button in the header parks a "reopen" shortcut in the status-bar dock — useful when you need to check a commit or branch and come back to the MR later. The status-bar badge (next to notifications) shows how many dialogs are parked; click it to see the list — each entry has a colour accent and type icon — and pick which to restore (click, or arrow to it and press <kbd>Enter</kbd>; its Discard button drops it without restoring). Restoring switches back to the source tab and re-opens the dialog, so the chip survives tab and workspace changes; scroll position and unsubmitted comments do not — the modal re-fetches and re-renders.</p>
