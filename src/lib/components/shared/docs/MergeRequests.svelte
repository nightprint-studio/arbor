...
<p class="doc-lead">Browse, review, and merge GitHub Pull Requests and GitLab Merge Requests from the sidebar. Reuses the same OAuth tokens as the CI/CD panel â€” no separate setup.</p>

<div class="feature-grid two-col">
  <div class="feature-card accent">
    <div class="fc-eyebrow">GitHub</div>
    <div class="fc-title">Pull Requests</div>
    <div class="fc-desc">Merge Â· Squash Â· Rebase Â· CI checks panel Â· Reopen.</div>
  </div>
  <div class="feature-card accent">
    <div class="fc-eyebrow">GitLab</div>
    <div class="fc-title">Merge Requests</div>
    <div class="fc-desc">Default strategy Â· Self-hosted instances supported Â· Reopen.</div>
  </div>
</div>

<h2>Authentication</h2>
<p>Connect your accounts in <strong>Settings â†’ Git &amp; Integrations</strong>. The same tokens used for CI/CD are reused â€” no extra setup. Click the <strong>GitPullRequest</strong> icon in the Activity Bar to open the sidebar.</p>

<h2>Sidebar</h2>
<ul class="prop-list">
  <li><strong>Search bar</strong>Client-side fuzzy filter over the loaded list â€” matches title, <code>#number</code>, source/target branches, author display name &amp; login, and label names. Clear with the <strong>Ã—</strong> button. The query resets on tab switch.</li>
  <li><strong>Filter tabs</strong>Switch between <em>Open</em> and <em>Merged</em> PRs/MRs. Backend reload â€” the search bar then narrows whichever set is loaded.</li>
  <li><strong>Row content</strong>Status icon Â· title Â· number Â· source â†’ target Â· author Â· time-ago Â· comment count Â· labels.</li>
...
<div class="feature-grid">
  <div class="feature-card">
    ...
    <div class="fc-desc">State badge (Open / Merged / Closed) Â· Draft flag Â· title Â· branches Â· author Â· time-ago Â· labels Â· refresh Â· open in browser.</div>
  </div>
  <div class="feature-card">
    <div class="fc-title">Overview</div>
    <div class="fc-desc">Markdown description Â· CI Checks summary (when available) Â· Assignees Â· Reviewers Â· Activity timeline.</div>
  </div>
</div>
...
<ul class="prop-list">
  ...
  <li><strong>Fence language auto-detect</strong>fenced blocks without an explicit language (<code>```</code> without a tag) are sniffed (<em>Rust, TOML, JSON, YAML, bash, TS/JS, markup</em>) and highlighted deterministically â€” no more wall-of-grey for bot-generated diffs.</li>
...
<div class="feature-grid">
  <div class="feature-card">
    ...
    <div class="fc-desc">Pipeline runs targeting the source branch â€” status pill, duration, retrigger, click to open the stage/job graph.</div>
  </div>
  <div class="feature-card">
  ...
  <div class="feature-card">
    ...
    <div class="fc-desc">Merge (split button) Â· Reopen (merged) Â· Close (with confirmation dialog).</div>
  </div>
</div>

<h3>Activity timeline</h3>
<p>The Overview tab renders comments and timeline events on a GitLab-style vertical rail. Three filter chips at the top toggle each category â€” counts always reflect what's loaded, regardless of visibility:</p>
<dl class="meta-grid">
  <dt><span class="chip-doc chip-doc-blue">Comments</span></dt>
  <dd>Human-authored comments â€” large avatar nodes, accent-blue strip on the left edge of each card. Body rendered as Markdown (headings, lists, fenced code blocks with Prism syntax highlighting, blockquotes, links).</dd>
...
<ul class="prop-list">
  ...
  <li><strong>Emoji shortcodes</strong><code>:warning:</code> â†’ âš ï¸, <code>:white_check_mark:</code> â†’ âœ…, <code>:x:</code> â†’ âŒ, etc. ~90 shortcodes resolved (covers GitHub, GitLab and the common ecosystem aliases). Unknown shortcodes are left intact.</li>
  <li><strong>Activity body trimming</strong>GitLab system notes that ship with an HTML expansion ("added 83 commits<code>&lt;ul&gt;â€¦&lt;/ul&gt;</code>") are truncated at the first tag â€” the timeline shows just the human-readable lede.</li>
</ul>

<h4>Default visibility</h4>
<p>Configure which chips start visible from <strong>Settings â†’ Access â†’ Merge Requests</strong>. Defaults are stored in <code>~/.config/arbor/config.toml</code> under <code>[mr]</code>:</p>
...
<p>Toggling a chip inside an open modal is session-only â€” it never writes back to the config. Use Settings to change the global default.</p>

<h3>Closing a PR / MR</h3>
<p>The <strong>Close</strong> button (visible when the PR/MR is open) asks for explicit confirmation in a centred dialog before sending the close request â€” no more "I clicked it thinking I was closing the modal" mistakes. The dialog spells out which number is about to be closed.</p>
...
<dl class="meta-grid">
  ...
  <dt>Detail modal</dt>
  <dd>Click a card to open the full stage / job graph â€” same modal used from the Pipelines panel. The provider icon is brand-tinted (orange for GitLab); stages render left-to-right in execution order; <kbd>Esc</kbd> closes.</dd>
</dl>
<div class="hint">
  Authentication is shared with the CI/CD panel â€” connect your GitHub or GitLab
  account once in <strong>Settings â†’ Authentication</strong> and the CI tab
...
<dl class="meta-grid">
  ...
  <dd>
    <ul class="prop-list">
      <li><strong>Branch query</strong><code>GET /actions/runs?branch={'{source_branch}'}</code> â€” push and <code>pull_request</code> runs whose <code>head_branch</code> matches.</li>
      <li><strong>Head-SHA query</strong><code>GET /actions/runs?head_sha={'{head_sha}'}</code> â€” fork PRs, <code>pull_request_target</code> workflows, and <code>workflow_dispatch</code> runs pinned to the SHA. These don't always tag the source branch on the run.</li>
    </ul>
  </dd>
  <dt>GitLab</dt>
  <dd>
    <ul class="prop-list">
      <li><strong>Detached MR pipelines</strong><code>GET /merge_requests/:iid/pipelines</code> â€” required for pipelines whose <code>ref</code> is <code>refs/merge-requests/{'{iid}'}/head</code>. These are the ones GitLab shows at the top of the MR page as <em>"Merge request pipeline #..."</em> and would otherwise be invisible to a plain branch filter.</li>
      <li><strong>Branch pipelines</strong><code>GET /pipelines?ref={'{source_branch}'}</code> â€” regular pushes to the source branch.</li>
    </ul>
  </dd>
</dl>
...
<ul class="prop-list">
  <li><strong>Branch exists locally</strong>Nothing to do if you never had it â€” the step is a no-op.</li>
...
<dl class="meta-grid">
  ...
  <dd>
    ...
    merges once required checks pass â€” GitHub uses <em>auto-merge</em>
...
<table class="matrix">
  ...
  <tbody>
    <tr><td>List open / closed / merged</td><td class="yes">âœ“</td><td class="yes">âœ“</td></tr>
    <tr><td>Sidebar search (client-side)</td><td class="yes">âœ“</td><td class="yes">âœ“</td></tr>
    <tr><td>Markdown description &amp; comments</td><td class="yes">âœ“</td><td class="yes">âœ“</td></tr>
    <tr><td>Emoji shortcodes (<code>:warning:</code> â†’ âš ï¸)</td><td class="yes">âœ“</td><td class="yes">âœ“</td></tr>
    <tr><td>Activity timeline (state / labels / assigns / â€¦)</td><td class="yes">âœ“ <em>via /events</em></td><td class="yes">âœ“ <em>via system notes</em></td></tr>
    <tr><td>Bot detection (filterable)</td><td class="yes">âœ“ <em>[bot] suffix</em></td><td class="yes">âœ“ <em>name heuristic</em></td></tr>
    <tr><td>Create PR / MR</td><td class="yes">âœ“</td><td class="yes">âœ“</td></tr>
    <tr><td>Auto-merge on creation</td><td class="yes">âœ“ <em>branch protection req.</em></td><td class="yes">âœ“ merge-when-pipeline-succeeds</td></tr>
    <tr><td>Merge</td><td class="yes">âœ“ merge / squash / rebase</td><td class="yes">âœ“ merge / squash</td></tr>
    <tr><td>Delete source branch on merge</td><td class="yes">âœ“</td><td class="yes">âœ“</td></tr>
    <tr><td>Close (with confirmation) / Reopen</td><td class="yes">âœ“</td><td class="yes">âœ“</td></tr>
    <tr><td>Add comment</td><td class="yes">âœ“</td><td class="yes">âœ“</td></tr>
    <tr><td>Draft / WIP flag</td><td class="yes">âœ“</td><td class="yes">âœ“</td></tr>
    <tr><td>Labels</td><td class="yes">âœ“</td><td class="yes">âœ“</td></tr>
    <tr><td>Assignees / Reviewers</td><td class="yes">âœ“</td><td class="yes">âœ“</td></tr>
    <tr><td>CI checks summary (Overview)</td><td class="partial">when available</td><td class="no">â€”</td></tr>
    <tr><td>Pipeline runs tab (filtered by source branch)</td><td class="yes">âœ“</td><td class="yes">âœ“</td></tr>
    <tr><td>Re-trigger run from PR/MR</td><td class="yes">âœ“</td><td class="yes">âœ“</td></tr>
    <tr><td>Self-hosted instance</td><td class="no">â€”</td><td class="yes">âœ“</td></tr>
  </tbody>
</table>
