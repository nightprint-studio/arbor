...
<table class="shortcuts-table">
  ...
  <tbody>
    ...
    <tr><td><code>:alert(cfg)</code> · <code>:info_card(cfg)</code> · <code>:chip_bar(cfg)</code></td><td>Push the corresponding display node (see the node-types table). <code>alert</code> takes <code>&#123;text, variant?, style?&#125;</code>; <code>chip_bar</code> is the only one that's value-bearing (selection stored at <code>values[name]</code>).</td></tr>
    <tr><td><code>:copy_button(cfg)</code> · <code>:experimental_badge(cfg)</code> · <code>:section_header(cfg)</code></td><td>Three more display-only widgets — click-to-copy button with chrome, the amber "Experimental" pill, and the standalone section title bar (no body).</td></tr>
