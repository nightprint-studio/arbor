...
<table class="shortcuts-table">
  ...
  <tbody>
    ...
    <tr><td><code>reorder_list</code></td><td>name, label?, items[], readonly?</td><td>A list whose <strong>order is the answer</strong> — parts to concatenate, steps to run, files to merge. Its form value is the array of <code>id</code>s in the order shown, so a submit carries the sequence without you tracking moves. Items: <code>&#123; id, label, sublabel?, icon?, meta? &#125;</code>. Keyboard-first by design: <kbd>↑</kbd>/<kbd>↓</kbd> move the selection, <kbd>Alt</kbd>+<kbd>↑</kbd>/<kbd>↓</kbd> move the row, <kbd>Home</kbd>/<kbd>End</kbd> jump — with buttons for the same three verbs. Unlike <code>tree</code>&rsquo;s <code>reorderable</code>, it does not use HTML5 drag, which does not work on Windows&rsquo; WebView2.</td></tr>
