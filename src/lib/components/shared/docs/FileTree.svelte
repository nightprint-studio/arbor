...
<ul>
  ...
  <li><strong>Tables</strong> — GFM pipe tables render as a real HTML table (framed, padded cells, header row, column alignment from the <code>|:--:|--:|</code> markers, inline markdown rendered inside each cell). An all-empty header row (<code>| | |</code>) renders as a headerless grid. Clicking the rendered table puts the caret on its source so you can edit the raw pipes, and stepping the caret out of the block restores the rendered view</li>
  <li><strong>Images, video and audio</strong> — <code>![alt](url)</code> references render inline. URLs can be <code>http(s):</code>, <code>data:</code>, or filesystem paths next to the <code>.md</code> file (resolved relative to the document via Tauri's asset protocol — <code>./logo.png</code>, <code>assets/diagram.svg</code>, absolute paths all work). The media kind is picked from the file extension: <code>.mp4</code> / <code>.webm</code> / <code>.mov</code> / <code>.m4v</code> / <code>.mkv</code> render as <code>&lt;video controls&gt;</code>, <code>.mp3</code> / <code>.wav</code> / <code>.m4a</code> / <code>.flac</code> as <code>&lt;audio controls&gt;</code>, anything else as <code>&lt;img&gt;</code>. A bare URL on its own line gets the same treatment when it points to a media file or a known video CDN; URLs mid-sentence stay regular links. <strong>GitHub user-attachments videos</strong> (<code>https://github.com/user-attachments/assets/&lt;uuid&gt;</code>) render as a clickable "Play video in browser" card instead of an inline player — GitHub's CDN sets the session cookie required to follow the signed redirect with <code>SameSite=Lax</code>, which the isolated WebView origin can't store, so inline playback isn't possible. The card opens the URL in the system browser, where the cookie chain works normally. The asset protocol is scoped to known media extensions (png / jpg / gif / webp / svg / bmp / ico / avif for images; mp4 / webm / ogv / mov / m4v / mkv for video; mp3 / wav / m4a / flac / aac / opus / oga for audio) so non-media files in your repo stay invisible to the WebView. Caret on the source reveals the raw <code>![…](…)</code> for editing</li>
