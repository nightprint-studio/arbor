...
<Callout variant="info" title="Be the default file explorer">
  Turn on <strong>Open in the built-in explorer</strong> (Settings → File Explorer) and the app's “Open / Reveal in File Explorer” actions — worktree info, plugin folders, notification reveals, the project trees in Bennu and Merula, and a plugin's own “open in file manager” action — open here instead of the OS file manager, focusing an existing tab when that folder is already open. Off by default. The explorer's own <strong>Reveal in File Explorer</strong> item always uses the OS, as an explicit way out to the system shell.
</Callout>
