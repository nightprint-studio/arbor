...
<div class="feature-grid two-col">
  <div class="feature-card accent">
    ...
    <div class="fc-title">OAuth · Personal API Key</div>
  ...
  <div class="feature-card accent">
    ...
    <div class="fc-title">Cloud · Data Center · Server</div>
    <div class="fc-desc">Email + API token, PAT for DC/Server, OAuth 2.0 (3LO) for Cloud. Self-signed certs accepted.</div>
  </div>
</div>

<h2>Setup</h2>
<p>Open the <strong>Issues</strong> sidebar and pick a tracker — or configure credentials in <strong>Settings → Git &amp; Integrations → Issue Trackers</strong>. Each repository stores its own selection.</p>

<div class="divider">Linear</div>

<h3>OAuth <span class="badge badge-new">Recommended</span></h3>
<ol class="step-list">
  <li>Register a <strong>Public OAuth application</strong> at <code>linear.app → Settings → API → OAuth applications</code></li>
  <li>Add <code>http://127.0.0.1:7729/callback</code> as the redirect URI</li>
  <li>Click <strong>Connect → OAuth</strong> in settings and approve in the browser</li>
  <li>Arbor completes the PKCE flow and stores the token in the OS keychain</li>
</ol>

<h3>Personal API Key</h3>
<ol class="step-list">
  <li>Generate a key at <code>linear.app → Settings → API → Personal API keys</code></li>
  <li>Click <strong>Connect ▾ → Personal API Key</strong> and paste the <code>lin_api_…</code> token</li>
</ol>

<div class="divider">Jira</div>

<h3>API Token — Jira Cloud <span class="badge badge-new">Recommended</span></h3>
<p>Generate an API token at <code>id.atlassian.com → Security → API tokens</code>, then click <strong>Connect → API Token</strong> and fill in:</p>
<dl class="meta-grid">
...
<h3>Personal Access Token — Data Center / Server</h3>
<p>Generate a PAT at <code>Jira → Profile → Personal Access Tokens</code>. Use the <strong>API Token</strong> form with the full hostname as the subdomain (e.g. <code>jira.internal.example.com</code>) plus email and PAT.</p>
...
<h3>OAuth 2.0 (3LO) — Jira Cloud only</h3>
<p>Click <strong>Connect ▾ → OAuth 2.0</strong> and follow the browser prompt. Arbor auto-discovers your site and stores access + refresh tokens in the OS keychain. Token refresh is transparent.</p>

<h3>Jira compatibility matrix</h3>
<table class="matrix">
  <thead><tr><th>Edition</th><th>Auth</th><th>API</th><th>Notes</th></tr></thead>
  <tbody>
    <tr><td>Cloud <code>*.atlassian.net</code></td><td>Token · OAuth 2.0</td><td>v3</td><td>Full feature set</td></tr>
    <tr><td>Data Center ≥ 8.4</td><td>Email + PAT</td><td>v2</td><td>Self-signed certs OK</td></tr>
...
<ul class="prop-list">
  <li>
    ...
    <dl class="meta-grid">
      ...
      <dd><strong>Default</strong> — matches the ticket code <em>and</em> any text that mentions it. Free-form text (e.g. <code>login bug</code>) falls back to text matching across title / description / comments.</dd>
      <dt><code>~PROJ-42</code></dt>
      <dd><strong>Text-only</strong> — the <code>~</code> prefix bypasses the code lookup. Finds only descriptions / comments / titles that <em>mention</em> <code>PROJ-42</code>, never the ticket whose key is <code>PROJ-42</code>. Useful for tracing references without the noise of the ticket card on top.</dd>
    </dl>
    <div class="hint">
      On Jira the text side searches <code>summary + description + comments</code> (the JQL <code>text ~</code> operator). On Linear it searches <code>title</code> only — the GraphQL filter doesn't expose body / comment search.
    </div>
  </li>
  ...
  <li><strong>Issue Type</strong>Jira only. Multi-select by type (Bug, Story, Task, Epic, Sub-task…) with per-type colour indicators.</li>
...
<h3>Issue card</h3>
<p>Priority emoji · Identifier (<code>ARB-123</code>) · Title · Labels · Status badge · Assignee avatar · Time-ago · Comment count. <strong>Click</strong> opens the detail modal, <strong>right-click</strong> the context menu.</p>

<h3>Ticket picker</h3>
<p>Appears when creating a branch via GitFlow or the graph context menu. Uses the active repo's tracker automatically — no need to open the sidebar first. Same filters as the sidebar; selecting an issue populates the branch name.</p>
...
<ul class="prop-list">
  <li><strong>Click to download</strong>Opens Arbor's in-app save picker with the original filename pre-filled. The fetch only starts after you confirm a destination — cancelling the picker is a true no-op.</li>
  <li><strong>Authenticated &amp; streamed</strong>The download runs on the Tokio runtime off the UI thread, and the body is streamed chunk-by-chunk straight to disk — no whole-file buffering in RAM, no UI freeze.</li>
  ...
  <li><strong>Status feedback</strong>The card icon becomes a spinner while downloading and a green ✓ on success; failures show a red border and a toast.</li>
</ul>
...
<table>
  ...
  <tbody>
    ...
    <tr><td>Status</td><td>Status</td><td>Status category → type (unstarted / started / completed)</td></tr>
    <tr><td>Labels</td><td>Labels</td><td>Plain strings; colour deterministic</td></tr>
    <tr><td>Priority</td><td>Priority</td><td>Highest → Urgent, High, Medium, Low/Lowest</td></tr>
...
<h2>Create Issue</h2>
<p>Two-column form — title/description left, metadata right.</p>
<div class="feature-grid two-col">
  <div class="feature-card">
    <div class="fc-title">Linear fields</div>
    <div class="fc-desc">Team · Status · Priority · Project · Milestone · Assignee (self) · Labels · Due date · Estimate.</div>
  </div>
  <div class="feature-card">
    <div class="fc-title">Jira fields</div>
    <div class="fc-desc">Project <span class="badge badge-req">Req</span> · Issue Type <em>(default: Task)</em> · Priority · Labels · Assignee (self) · Fix Version · Due date · Story Points.</div>
  </div>
</div>

<h2>Plugin API — <code>arbor.issues</code></h2>
<p>Works identically for Linear and Jira — the active provider for each repo is resolved transparently.</p>
<dl class="meta-grid">
