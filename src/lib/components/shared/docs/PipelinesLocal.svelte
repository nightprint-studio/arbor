...
<h1>Pipelines â€” Plugin Pipelines</h1>
...
<ul>
  ...
  <li>
    <strong>Right column</strong> â€” a filter row with a multi-select dropdown
...
<p>Click the <strong>Run</strong> icon in the left toolbar to replay the most recently launched pipeline. To run a different one, right-click any of its existing run cards in the list and pick <strong>Run â€œâ€¦â€</strong> from the context menu â€” the menu's other entries (Open detail, Cancel, Resume, Discard) mirror the row's hover buttons. The orchestrator spawns a background thread that executes each step sequentially. The node graph updates in real time with status colours:</p>
<p>
  ...
  button, sequence runner, â€¦), so Play replays it directly via
...
<p>
  ...
  <code>running</code> run â€” cancel it first.
</p>
...
<p>
  ...
  <code>running</code> state at a time â€” a second attempt fails immediately with
...
<p>
  ...
  <code>arbor.pipeline.define</code> â€” useful when different pipelines compete
...
<p>
  ...
  configured <code>log_level</code> (default <code>info</code>) â€” set
...
<p>Two equivalent shapes â€” pick whichever reads better for your case:</p>
<ul>
  <li><code>arbor.pipeline.define(table)</code> â€” declarative table config (good when you build the pipeline programmatically from data).</li>
  <li><code>arbor.pipeline("id"):...:commit()</code> â€” chainable builder (good for static, hand-written pipelines). Compiles down to the same table on <code>:commit()</code>.</li>
</ul>
...
<pre class="language-lua"><code>{@html highlight(`arbor.pipeline("build")
  ...
  :commit()`, '.lua')}</code></pre>
<p>
  Builder methods: <code>:name</code> Â· <code>:description</code> Â·
  <code>:icon</code> Â· <code>:lock</code> (alias <code>:lock_key</code>) Â·
  <code>:log_level</code> Â· <code>:stage(name|cfg)</code> Â· <code>:mode</code> Â·
  <code>:max_parallel</code> Â· <code>:run(op, params)</code> Â·
  <code>:shell(cmd|cfg)</code> Â· <code>:step(cfg)</code> Â· <code>:commit()</code>.
...
<table class="shortcuts-table">
  ...
  <tbody>
    <tr><td><code>"on_pipeline_run_request"</code></td><td><code>on_pipeline_run_request</code></td><td><code>pipeline_id, tab_id?</code> â€” fired on the def's owning plugin when the user presses Play on a <em>stub</em> def (empty <code>stages</code>). Defs with non-empty stages are replayed directly without invoking this hook. The handler must compile stages and call <code>arbor.pipeline.run</code> itself.</td></tr>
...
<p>
  ...
  (precedence top-to-bottom): <code>if_block</code> â†’
  <code>builtin</code> â†’ <code>lua_op</code> â†’ <code>command</code>. The
...
<table class="shortcuts-table">
  ...
  <tbody>
    ...
    <tr><td><code>builtin</code></td><td>table?</td><td>Run a built-in op (file_exists / file_read / env / json_get / path_join / set_var / echo / match). See the dedicated section below. Resolved by the runtime â€” no shell, no Lua VM.</td></tr>
...
<p>
  ...
  reference its values via <code>$&#123;var&#125;</code> syntax in any string field â€”
...
<ul>
  <li><code>var</code> â€” name to store under (no <code>$</code> prefix).</li>
  <li><code>source</code> â€” what part of the step's outcome to capture:
    <code>"stdout"</code> (default), <code>"stderr"</code>,
    <code>"exit_code"</code>, <code>"success"</code> (boolean: exit_code == 0),
    or <code>"return_value"</code> (Lua/builtin's typed return â€” falls back
    to stdout for shell steps).</li>
  <li><code>transforms</code> â€” optional ordered list of <em>declarative
...
<table class="shortcuts-table">
  ...
  <tbody>
    ...
    <tr><td><code>&#123; kind="lower" &#125;</code> Â· <code>&#123; kind="upper" &#125;</code></td><td>ASCII case folding</td></tr>
    <tr><td><code>&#123; kind="lines" &#125;</code></td><td>Split a string on <code>\n</code> â†’ list (drops trailing empty lines)</td></tr>
    <tr><td><code>&#123; kind="split", sep="," &#125;</code></td><td>Split on a literal separator â†’ list</td></tr>
    <tr><td><code>&#123; kind="join", sep=", " &#125;</code></td><td>Join a list with <code>sep</code> â†’ string</td></tr>
    <tr><td><code>&#123; kind="first" &#125;</code> Â· <code>&#123; kind="last" &#125;</code> Â· <code>&#123; kind="nth", n=2 &#125;</code></td><td>Index a list (negative <code>n</code> counts from end)</td></tr>
    ...
    <tr><td><code>&#123; kind="json_parse" &#125;</code> Â· <code>&#123; kind="json_get", path="a.b.0" &#125;</code></td><td>Parse a JSON string; walk a dotted path</td></tr>
    <tr><td><code>&#123; kind="to_bool" &#125;</code> Â· <code>&#123; kind="to_number" &#125;</code></td><td>Coerce to boolean / number (<code>null</code> on failure)</td></tr>
...
<table class="shortcuts-table">
  ...
  <tbody>
    ...
    <tr><td><code>set_var</code></td><td><code>value</code> (any JSON)</td><td>the value verbatim â€” pair with <code>capture.var</code></td></tr>
...
<p>
  Conditions are <strong>structured values</strong> â€” there's no DSL or
...
<table class="shortcuts-table">
  ...
  <tbody>
    <tr><td><code>compare</code></td><td><code>left</code>, <code>op</code>, <code>right</code></td><td><code>op</code> âˆˆ <code>eq</code>, <code>ne</code>, <code>i_eq</code>, <code>contains</code>, <code>starts_with</code>, <code>ends_with</code>, <code>matches</code> (right = regex), <code>gt</code>/<code>lt</code>/<code>gte</code>/<code>lte</code> (numeric)</td></tr>
    ...
    <tr><td><code>all_of</code> Â· <code>any_of</code> Â· <code>not</code></td><td><code>conditions</code> / <code>condition</code></td><td>Logical combinators.</td></tr>
    <tr><td><code>always</code> Â· <code>never</code></td><td>â€”</td><td>Constants. <code>always</code> is the natural condition for the catch-all <code>else</code> branch (or just leave <code>else_steps</code> set).</td></tr>
  </tbody>
</table>
...
<p>
  ...
  work â€” drilling deep is supported, the run viewer shows the
...
<ul>
  <li><code>nil</code> / <code>true</code> â†’ exit_code = 0 (success)</li>
  <li><code>false</code> â†’ exit_code = 1</li>
  <li><code>&lt;number&gt;</code> â†’ that exit code</li>
  <li><code>&lt;string&gt;</code> â†’ stdout, exit_code = 0</li>
  <li><code>&#123; exit_code?, stdout?, stderr? &#125;</code> â†’ structured</li>
</ul>
...
<p>Two usage patterns â€” pick whichever fits:</p>
...
<table class="shortcuts-table">
  ...
  <tbody>
    ...
    <tr><td><code>arbor.fs.yaml_set&#123; path, ypath, value &#125;</code></td><td><code>serde_yaml</code> â†’ json walker</td><td>dotted path, same as JSON</td></tr>
...
<h2>Permissions</h2>
<p>No special permissions are required to define or trigger pipelines â€” any plugin can call <code>arbor.pipeline.define()</code> and <code>arbor.pipeline.run()</code>. The commands run under the same OS user as Arbor itself. Plugins do <em>not</em> need the <code>terminal</code> permission for pipeline steps (that applies only to <code>arbor.terminal.exec</code>).</p>
