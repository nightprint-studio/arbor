...
<div class="feature-grid two-col">
  <div class="feature-card accent">
    ...
    <div class="fc-desc">GraphQL: severity counts · time series · risk score · per-finding metadata. Ultimate-only fields degrade gracefully.</div>
  </div>
  <div class="feature-card accent">
    <div class="fc-eyebrow">GitHub</div>
    <div class="fc-title">GHAS · Dependabot · Secret Scanning</div>
...
<ul class="prop-list">
  <li><strong>Probing</strong><span>Spinner + "Checking provider…" while the support probe is in flight.</span></li>
  ...
  <li><strong>Loaded</strong><span>Filter bar · 6 severity counter cards · risk-score gauge + vulns-over-time chart · truncation note when the cap is hit.</span></li>
</ul>
...
<p>
  ...
  <code>(critical×10 + high×5 + medium×2 + low×0.5)</code> capped at 100.
</p>
<p>
...
<ul class="prop-list">
  <li><strong>Header</strong><span>Shield icon · risk pill · "Open in &lt;provider&gt;" external link.</span></li>
  <li><strong>Tabs</strong><span>Per-severity strip (<code>All | Critical | High | …</code>) — counts dynamic, zero tabs disabled.</span></li>
  ...
  <li><strong>Progress bar</strong><span>Indeterminate sliding bar at the top of the list region — shows during fetches AND during tab/scope swaps to mask the DOM-thrash on large severities.</span></li>
  <li><strong>Virtualized list</strong><span>Each row is a fixed 64px so the list can render 300+ findings as ~20 DOM nodes. Severity desc → age desc sort.</span></li>
...
<ul class="prop-list">
  <li><strong>Header</strong><span>Severity chip · title · CVE / report-type chips · "Open in &lt;provider&gt;".</span></li>
  <li><strong>Remediation</strong>
    <span>
      Prominent <em>"How to fix"</em> block when the provider exposes one.
      GitLab → <code>Vulnerability.solution</code> as-is. GitHub Dependabot →
...
<p>
  The toggle <em>only</em> affects the modal — the dashboard's counter
...
<ul class="prop-list">
  ...
  <li><strong>Type multiselect</strong><span>Auto-populates from the loaded findings — <code>sast</code>, <code>dependency_scanning</code>, <code>secret_detection</code>, … etc.</span></li>
  <li><strong>Clear</strong><span>Resets severity / type / search but <em>preserves the state scope</em> — the user's scope choice is treated as a view mode, not a narrowing filter.</span></li>
</ul>
...
<ul class="prop-list">
  ...
  <li><strong>In-flight dedup</strong><span><code>loadSummary</code> / <code>loadFindings</code> share a single Promise per tab — the AppShell pre-load and the panel mount fetch can fire concurrently without racing each other into a stuck loading state.</span></li>
...
<p>
  ...
  never leaves the host — provider permission gate is the same
  <code>provider = "read"</code> flag used by <code>arbor.mr.*</code> and
  <code>arbor.ci.*</code>.
</p>

<pre class="language-lua"><code>{@html highlight(`-- Cheap probe — false for tokenless repos / providers without a dashboard
...
<table class="shortcuts-table">
  ...
  <tbody>
    <tr><td>Dashboard probe</td><td>GraphQL (<code>vulnerabilitySeveritiesCount</code> + <code>vulnerabilities</code>)</td><td>REST x3 (code-scanning · dependabot · secret-scanning)</td></tr>
    <tr><td>Severity counts</td><td>Server-side, state-filtered</td><td>Computed host-side from open alerts</td></tr>
    <tr><td>Time series</td><td>Ultimate-only via <code>vulnerabilitiesCountByDay</code></td><td>Not exposed → chart hidden</td></tr>
