...
<h2>Tempo</h2>
<pre><code>cps(0.6)                          // constant clock: cycles per second
tempo(cycles(8, 0.5), cycles(16, 0.6))  // piecewise tempo map (wins over cps)</code></pre>
<p>
...
<pre><code>let bass = n(c2 g1).inst("synth.bass")   // a value (does not sound on its own)
fn echo(p) = stack(p, p.slow(2).gain(0.4))  // a one-shot function, no recursion</code></pre>
<p>
...
<h2>Imports</h2>
<pre><code>import &lbrace; kick, snare &rbrace; from "lib/drums.nemus"</code></pre>
<p>
