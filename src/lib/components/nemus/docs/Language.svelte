...
<h2>Tempo</h2>
<NemusCode code={`cps(0.6)                          // constant clock: cycles per second
...
<NemusCode code={`let bass = n(c2 g1).inst("synth.bass")   // a value (does not sound on its own)
...
<h2>Imports</h2>
<NemusCode code={`import { kick, snare } from "lib/drums.nemus"`} />
<p>
