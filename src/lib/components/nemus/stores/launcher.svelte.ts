 ...
 *
 * `scene(...)` declarations are exposed by the backend (`nemus_scenes`): the base
...
import { nemusScenes, nemusLaunch, type NemusScene, type NemusClipSelection } from '$lib/ipc/nemus';
import { transportStore, nemusEngine } from './engine.svelte';
...
function createLauncherStore() {
  ...
  function push(sel: Sel): void {
    const payload: NemusClipSelection[] = Object.entries(sel).map(([track, scene]) => ({
      track: Number(track),
      scene,
    }));
    void nemusLaunch(payload);
  }
  ...
  function arm(desired: Sel): void {
    ...
    if (!transportStore.playing) {
      queued = new Set();
      pushSel = null;
      if (Object.keys(desired).length) void nemusEngine.play();
      return;
    }
  ...
  return {
    ...
    /** Transport tick (driven by NemusShell): fire a deferred backend swap at the
