/**
 * nemus export/render status — drives the title-bar feedback for the offline WAV
 * bounce. The render runs as a background job, so `nemusRender` resolves with a
...
function createRenderStore() {
  ...
  return {
    ...
    /**
     * Track a render: `promise` is the `nemusRender` call (resolves to a job id),
