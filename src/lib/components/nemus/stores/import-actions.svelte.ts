/**
 * nemus import-action launcher — the single owner of the audio/MIDI import flow
 * (D3/D4/D5). Mirrors `projectActions`: the store holds which picker/dialog is
 * open and runs the IPC; `NemusImportActions.svelte` renders the modals once for
 ...
 *
 * Flow:
 *   start()  → pick a file. MIDI → import as .nemus directly (D5). Audio → ask
 *              "Convert to .mid file" (D4) or "Import as .nemus" (D3).
 *   startConvert() → pick a WAV, then a .mid output (D4 only) — the palette verb.
 *
 * Results that produce `.nemus` text (D3/D5) open in a new tab via the project
...
import {
  nemusImportAudioAsNemus, nemusImportMidiAsNemus, nemusConvertWavToMidi,
} from '$lib/ipc/nemus';
...
function createImportActions() {
  ...
  /** Open generated `.nemus` text as a new tab (cached, not yet on disk). */
  async function openResult(srcPath: string, text: string) {
    const base = `imported-${stem(srcPath)}.nemus`;
  ...
  async function runImport(path: string, audio: boolean) {
    ...
    try {
      ...
      await track(opId, filename(path), audio ? 'Transcribing…' : 'Reading MIDI…', undefined, async () => {
        text = audio ? await nemusImportAudioAsNemus(path, opId) : await nemusImportMidiAsNemus(path);
      }, true);
  ...
  function runConvert(wav: string, out: string) {
    ...
    void track(opId, filename(out), 'Transcribing…', out, async () => {
      await nemusConvertWavToMidi(wav, out, opId);
