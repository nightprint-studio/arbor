/**
 * Lazy per-file metadata for the Files panel. Reading every `.nemus` file off
...
export interface NemusFileMeta {
...
function parseMeta(src: string): NemusFileMeta {
...
function createFileMetaStore() {
  const cache = new SvelteMap<string, NemusFileMeta>();
  const loading = new Set<string>();

  return {
    /** Cached summary for `path`, or null until it's been parsed. */
    get(path: string): NemusFileMeta | null { return cache.get(path) ?? null; },
