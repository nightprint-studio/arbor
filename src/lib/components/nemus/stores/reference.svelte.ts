/**
 * nemus DSL reference store — the canonical language catalogue (`nemus_lang_reference`).
 *
 ...
 * Rust (`arbor-nemus-lang`). This single source feeds the editor's autocomplete,
 * hover docs, and the Docs panel — so language intelligence never drifts from the
 * evaluator. Imports only the IPC seam; nemus stays extractable.
 */

import { nemusLangReference, type NemusDslEntry } from '$lib/ipc/nemus';

function createReferenceStore() {
  let entries = $state<NemusDslEntry[]>([]);
  let loaded = $state(false);
  let loading = $state(false);
  /** Name → entry, for hover / completion lookups. Built lazily from `entries`. */
  let byNameMap = $state<Map<string, NemusDslEntry>>(new Map());
