/**
 * nemus sound-alias store — the global `alias → target` name map (e.g.
 * `kick → "RolandTR808_bd"`, `timp → "perc.timpani"`). Resolved by the audio
 * registry so `s("alias")` / `.inst("alias")` play the target voice. **Global**,
 * not per-project / per-file: a dedicated nemus-data file (`aliases.json`), NOT
...
import { getNemusAliases, setNemusAliases } from '$lib/ipc/nemus';
...
function createAliasesStore() {
  ...
  function persist() { void setNemusAliases({ ...map }).catch(() => {}); }
  ...
  return {
    ...
    async load() {
      try { map = await getNemusAliases(); loaded = true; }
