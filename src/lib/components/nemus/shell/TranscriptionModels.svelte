<script lang="ts">
  ...
  import type { NemusModelStatus } from '$lib/ipc/nemus';

  let confirmDelete = $state<NemusModelStatus | null>(null);
  ...
  function askDelete(model: NemusModelStatus) { deleteError = null; confirmDelete = model; }
