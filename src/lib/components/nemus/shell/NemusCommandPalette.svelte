<script lang="ts">
  ...
  const commands = $derived<Cmd[]>([
    ...
    // Project / file
    { id: 'new_project',  label: 'New project…',     group: 'Project', icon: 'FolderPlus', keys: 'Ctrl+Shift+N', desc: 'Scaffold a new nemus project folder with a starter file.', run: () => projectActions.newProject() },
    { id: 'open_project', label: 'Open project…',    group: 'Project', icon: 'FolderOpen', keys: 'Ctrl+O',       desc: 'Open an existing nemus project folder.', run: () => projectActions.openProject() },
    { id: 'open_file',    label: 'Open file…',       group: 'Project', icon: 'FolderOpen', keys: 'Ctrl+Shift+O', desc: 'Open a single .nemus file as an editor tab.', run: () => projectActions.openFile() },
    { id: 'new_file',     label: 'New .nemus file…', group: 'Project', icon: 'FilePlus2',  keys: 'Ctrl+N',       desc: 'Create a new .nemus file in the open project.', run: () => projectActions.newFile() },
    ...
    { id: 'rename_project', label: 'Rename project…', group: 'Project', icon: 'FolderPen', desc: 'Change the project name stored in nemus.toml.', run: () => { if (projectStore.project) nemusStore.openRenameProject(); } },
    { id: 'workspaces', label: 'Manage workspaces…', group: 'Project', icon: 'LayoutGrid', desc: 'Create, switch and colour named groups of projects.', run: () => nemusStore.openWorkspaces() },
    { id: 'rename_file', label: 'Rename active file…', group: 'Project', icon: 'FilePen', desc: 'Rename the .nemus file open in the editor.',
      run: () => { const p = projectStore.activeFilePath; if (p) nemusStore.openRenameFile(p); } },
    { id: 'delete_file', label: 'Delete active file…', group: 'Project', icon: 'Trash2', desc: 'Move the active .nemus file to the Recycle Bin.',
      run: () => { const p = projectStore.activeFilePath; if (p) nemusStore.openDeleteFile(p); } },
    ...
    { id: 'import',       label: 'Import audio / MIDI…', group: 'Project', icon: 'FileInput', keys: 'Alt+Shift+I', desc: 'Transcribe audio or convert MIDI into a .nemus file.', run: () => importActions.start() },
    ...
    { id: 'models', label: 'Manage transcription models…', group: 'Project', icon: 'Boxes', desc: 'Open Settings to manage downloaded transcription models.', run: () => nemusStore.openSettings() },
    ...
    // Panels
    { id: 'p_files',     label: 'Toggle Files',      group: 'View', icon: 'Files',            desc: 'Show or hide the project files sidebar.', run: () => nemusStore.toggleLeft('files') },
    { id: 'p_outline',   label: 'Toggle Outline',    group: 'View', icon: 'ListTree',         desc: 'Show or hide the symbol outline of the active file.', run: () => nemusStore.toggleLeft('outline') },
    { id: 'p_sounds',    label: 'Toggle Sound bank', group: 'View', icon: 'Music4',           desc: 'Browse and preview the resolvable instruments.', run: () => nemusStore.toggleLeft('soundbank') },
    { id: 'p_mixer',     label: 'Toggle Mixer',      group: 'View', icon: 'SlidersHorizontal', desc: 'Show or hide the per-track mixer strips.', run: () => nemusStore.toggleBottom('mixer') },
    { id: 'commit_overrides', label: 'Commit mixer overrides to source', group: 'Mixer', icon: 'ArrowDownToLine', keys: 'Alt+Shift+C', desc: 'Write the live mixer knob values back into the .nemus source.', run: () => mixerStore.commitAll() },
    { id: 'p_console',   label: 'Toggle Console',    group: 'View', icon: 'Terminal',         desc: 'Show or hide the script log console.', run: () => nemusStore.toggleBottom('console') },
    { id: 'p_problems',  label: 'Toggle Problems',   group: 'View', icon: 'AlertTriangle',    desc: 'Show or hide the diagnostics (errors / warnings) list.', run: () => nemusStore.toggleBottom('problems') },
    { id: 'p_jobs',      label: 'Toggle Jobs',       group: 'View', icon: 'Boxes',            desc: 'Show or hide background job output (renders, downloads).', run: () => nemusStore.toggleBottom('jobs') },
    { id: 'p_scratch',   label: 'Toggle Scratch (expression evaluator)', group: 'View', icon: 'FlaskConical', keys: 'Ctrl+Shift+S', desc: 'Evaluate / audition arbitrary .nemus expressions in isolation.', run: () => nemusStore.toggleBottom('scratch') },
    { id: 'p_keyboard',  label: 'Toggle Keyboard (live notes)', group: 'View', icon: 'Piano', desc: 'Piano that lights the notes sounding at the playhead.', run: () => nemusStore.toggleBottom('keyboard') },
    { id: 'p_launcher',  label: 'Toggle Clip launcher', group: 'View', icon: 'LayoutGrid', keys: 'Ctrl+Shift+G', desc: 'Show or hide the scene / clip launcher grid.', run: () => nemusStore.toggleBottom('launcher') },
    ...
    { id: 'p_inspector', label: 'Toggle Inspector',  group: 'View', icon: 'Crosshair',        desc: 'Show or hide the selected track inspector.', run: () => nemusStore.toggleRight('inspector') },
    { id: 'p_docs',      label: 'Toggle Language reference', group: 'View', icon: 'Braces',    desc: 'Show or hide the .nemus language reference panel.', run: () => nemusStore.toggleRight('docs') },
    { id: 'c_viz',       label: 'Toggle Arrangement', group: 'View', icon: 'PanelLeft',       desc: 'Collapse or restore the arrangement pane.', run: () => nemusStore.toggleCollapseUi() },
    { id: 'c_editor',    label: 'Toggle Editor',      group: 'View', icon: 'PanelRight',      desc: 'Collapse or restore the editor pane.', run: () => nemusStore.toggleCollapseTabpane() },
    { id: 'zen',         label: 'Toggle Zen mode',   group: 'View', icon: 'Minimize2', keys: 'Ctrl+Shift+Z', desc: 'Hide the rails, footer and bottom panel for focus.', run: () => nemusStore.toggleZen() },
    { id: 'performance', label: nemusStore.performance ? 'Exit performance mode' : 'Performance mode (full-screen stage)', group: 'View', icon: 'Maximize2', keys: 'F11', desc: 'Full-screen distraction-free stage for live play.', run: () => nemusStore.togglePerformance() },
    { id: 'find',        label: 'Search Console / Problems', group: 'View', icon: 'Search', keys: 'Ctrl+F', desc: 'Filter the active bottom panel by text.', run: () => nemusStore.requestFind() },
    { id: 'find_usages', label: 'Find usages of symbol at caret', group: 'View', icon: 'SearchCode', keys: 'Alt+F7', desc: 'List every reference to the symbol under the caret.', run: () => nemusStore.requestFindUsages() },
    { id: 'structure',   label: 'File structure (find method / variable)', group: 'View', icon: 'ListTree', keys: 'Ctrl+F12', desc: 'Jump to a declaration via the file-structure picker.', run: () => nemusStore.requestStructure() },
    // Edit
    { id: 'format', label: 'Format document', group: 'Edit', icon: 'AlignLeft', keys: 'Alt+Shift+L', desc: 'Reformat the active file to canonical style.', run: () => nemusStore.requestFormat() },
    { id: 'rename', label: 'Rename symbol…', group: 'Edit', icon: 'PenLine', keys: 'Shift+F6', desc: 'Rename the symbol at the caret across the file.', run: () => nemusStore.requestRename() },
    { id: 'extract', label: 'Extract selection to let…', group: 'Edit', icon: 'FileOutput', keys: 'Alt+Shift+V', desc: 'Lift the selected expression into a named let binding.', run: () => nemusStore.requestExtract() },
    { id: 'inline', label: 'Inline let', group: 'Edit', icon: 'FileSymlink', keys: 'Alt+Shift+N', desc: 'Replace a let binding with its value at each use.', run: () => nemusStore.requestInline() },
    { id: 'freeze', label: 'Freeze pattern to notes', group: 'Edit', icon: 'Snowflake', desc: 'Materialize a generated pattern into literal notes.', run: () => nemusStore.requestFreeze() },
    { id: 'euclid', label: 'Insert euclidean rhythm…', group: 'Edit', icon: 'Grid3x3', desc: 'Insert a euclid(...) rhythm via a small builder.', run: () => nemusStore.requestEuclid() },
    { id: 'chordprog', label: 'Insert chord progression…', group: 'Edit', icon: 'Music4', desc: 'Insert a chord progression via a small builder.', run: () => nemusStore.requestChord() },
    { id: 'intentions', label: 'Show context actions / quick-fixes', group: 'Edit', icon: 'Lightbulb', keys: 'Alt+Enter', desc: 'Open the quick-fix / intentions popup at the caret.', run: () => nemusStore.requestIntentions() },
    // Window
    { id: 'docs',      label: 'Documentation',      group: 'Window', icon: 'BookOpen', keys: 'F1',       desc: 'Open the in-app nemus documentation.', run: () => nemusStore.openDocs() },
    { id: 'settings',  label: 'Settings…',          group: 'Window', icon: 'Settings', keys: 'Ctrl+,',   desc: 'Open the nemus settings (audio, render, editor).', run: () => nemusStore.openSettings() },
    { id: 'shortcuts', label: 'Keyboard Shortcuts', group: 'Window', icon: 'Keyboard', keys: 'Shift+F1', desc: 'View the full keyboard shortcut reference.', run: () => nemusStore.openShortcuts() },
  ]);
