<script lang="ts">
  /**
   * Nemus keyboard-shortcuts cheat-sheet. Read-only reference for the nemus
   * window's bindings (the canonical set is `nemus-keybindings.ts`). Nemus keeps
...
<Modal {onClose} width="560px" height="560px" ariaLabel="Keyboard Shortcuts">
  ...
  <p class="sc-lead">Nemus is fully keyboard-navigable. Editor-scoped keys fire only when the tab pane has focus.</p>

  <table class="sc-table">
    <tbody>
      {#each NEMUS_BINDINGS as b (b.id)}
        <tr>
          <td class="sc-keys">
