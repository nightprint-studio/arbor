<script lang="ts">
  /**
   * Rename a `.nemus` file in place (same folder). Keyboard-first: the field
   * auto-focuses with the base name pre-selected, Enter / Ctrl+Enter submits, Esc
   * cancels (handled by Modal). The `.nemus` extension is optional in the input —
...
<Modal {onClose} width="520px" height="240px" ariaLabel="Rename file">
  ...
  <div class="rf">
    <FormField label="File name" hint="The .nemus extension is added automatically.">
      <Input bind:value={name} autofocus placeholder="song.nemus" onkeydown={onKeydown} ariaLabel="File name" />
    </FormField>
  </div>
