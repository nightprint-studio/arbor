<script lang="ts">
   ...
   *
   * Selection is shared via the NemusShell store (index-keyed), so clicking a
   * mixer strip (and, once Step 3a wires it, an arrangement header) drives this.
   *
   * Imports only shared/ui + nemus-local.
   */
  ...
  {:else}
    ...
    <div class="insp" style="--c: {track.color}">
      ...
      <p class="insp-hint">
        ...
        <strong>code-first</strong>: their knobs edit the <code>.nemus</code> source
        literal directly.
      </p>
    </div>
  {/if}
