<script lang="ts">
  /**
   * Docs — the navigable nemus language reference, rendered from the canonical
   * DSL catalogue (`referenceStore`, fed by `nemus_lang_reference`). Entries are
  ...
  import type { NemusDslEntry, NemusDslKind } from '$lib/ipc/nemus';
  import DocsEntryRow from './DocsEntryRow.svelte';

  // Idempotent: NemusShell loads this on mount, but a standalone mount still works.
  void referenceStore.load();

  let query = $state('');

  // Display order + label / icon / accent per kind. Drives the section grouping.
  const GROUPS: { kind: NemusDslKind; label: string; icon: typeof Braces; color: string }[] = [
  ...
  function matches(e: NemusDslEntry, q: string): boolean {
  ...
  const sections = $derived.by(() => {
    ...
    const byKind = new Map<NemusDslKind, NemusDslEntry[]>();
