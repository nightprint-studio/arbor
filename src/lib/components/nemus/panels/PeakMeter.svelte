<script lang="ts">
   ...
   * (`nemus:meters`). The level gradient is green → amber → red along the full
   ...
   *
   * Nemus-local presentational widget shared by the mixer strips + master + the
   * Inspector (a candidate for `shared/ui/` if it gets reused outside nemus).
   */
  import type { NemusStereoPeak } from '$lib/ipc/nemus';
