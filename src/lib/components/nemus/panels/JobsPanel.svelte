<script lang="ts">
  /**
   * Jobs — the nemus window's background-job output panel. Renders the same job
   ...
   * `jobsStore` (already mounted + target-routed by `<FeedbackHost id="nemus">`
   * in NemusWindow) is the single source of truth, and the output drill-in
   * composes the shared `LogStream`. Only the panel chrome is nemus-local
   * because the bottom-panel routing is nemus-specific (nemusStore). If a second
  ...
  // nemus jobs are never hidden, but keep the showHidden filter honest in case
  // a future nemus job opts in — mirror the overlay's listing semantics.
...
<div class="jobs">
  {#if activeJob}
    <!-- ── Output drill-in ─────────────────────────────────────────────────── -->
    <BottomPanelHeader title={activeJob.name} onClose={() => nemusStore.toggleBottom('jobs')}>
      {#snippet icon()}
  ...
  {:else}
    <!-- ── Job listing ─────────────────────────────────────────────────────── -->
    <BottomPanelHeader title="Jobs" onClose={() => nemusStore.toggleBottom('jobs')}>
