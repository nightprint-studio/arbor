<script lang="ts">
   ...
   * (`mixerStore.tracks`, index-keyed), meters from `nemus:meters`, and the
   * gain/pan knobs push **live ephemeral overrides** (`nemus_set_track`, gate 2)
   ...
   * commit button, the value lands in the `.nemus` on its own once the drag rests.
   *
...
<div class="mixer-root">
  <BottomPanelHeader title="Mixer" count={tracks.length} onClose={() => nemusStore.toggleBottom('mixer')}>
    {#snippet icon()}<SlidersHorizontal size={13} />{/snippet}
  </BottomPanelHeader>

  <div class="mixer-body">
  {#if !tracks.length}
    <EmptyState message="No arrangement yet — Run a .nemus file to see its mixer." />
  {:else}
    <div class="mix">
