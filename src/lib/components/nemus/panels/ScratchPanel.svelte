<script lang="ts">
   ...
   * Errors come back inline from `nemus_eval_snippet`.
   */
  ...
  import { createNemusExtensions } from '../editor/nemus-cm';
  import { withFileDeps } from '../editor/nemus-lang';
  import type { NemusIntelSource } from '../editor/nemus-intel';
  import { nemusEvalSnippet, type NemusQueryHap } from '$lib/ipc/nemus';
  ...
  function scheduleEval(tabId: string, source: string) {
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        ...
        const r = await nemusEvalSnippet(full, projectStore.project?.path);
  ...
  function mount(target: HTMLDivElement) {
    ...
    const state = EditorState.create({
      doc: active.source,
      extensions: [createNemusExtensions({ intel }), updateListener],
    });
  ...
  async function play() {
    ...
    void nemusEngine.playSnippet(full, projectStore.project?.path);
  }
  function stop() { void nemusEngine.stopSnippet(); }
  ...
  function rowLabel(h: NemusQueryHap): string {
...
<div class="scr" onkeydown={onKeydown} role="group">
  <BottomPanelHeader title="Scratch" onClose={() => nemusStore.toggleBottom('scratch')}>
