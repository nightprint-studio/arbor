<script lang="ts">
   ...
   * packs. Driven by the **real registry** (`soundsStore` ← `nemus_sounds`), not
  ...
  function match(list: NemusInstrument[]): NemusInstrument[] {
  ...
  interface SamplerGroup { id: string; name: string; items: NemusInstrument[]; }
  ...
  function askDelete(pack: NemusPack) { deleteError = null; confirmDelete = pack; }
  ...
  // The pack subscription is owned by the NemusShell; here we just (re)read the
  ...
  // (nemus_sounds is a fast registry read — usually no spinner at all).
...
{#snippet packCard(pack: NemusPack)}
