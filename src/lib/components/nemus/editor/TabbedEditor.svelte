<script lang="ts">
  /**
   * Editor pane: a JetBrains-style tab strip over the open `.nemus` files, a
   * code-block toolbar (breadcrumb · goto-line · copy), and the CodeMirror 6
   * editor (`NemusEditor`) with Tree-sitter highlight + lint + active-hap. Drives
   * off the real `projectStore` (path-keyed source model from Step 1).
   *
   * Edits flow `NemusEditor → projectStore.setSource` and trigger a debounced
   * re-eval (`nemusEngine.eval`) whose diagnostics come back through the store;
   * switching tabs re-evals immediately so lint matches the visible file. Exposes
   * `openGoto()` + `newFile()` for the NemusShell keybindings (Ctrl+G / Ctrl+N).
   *
   * Imports only shared/ui (Tabs, EmptyState) + nemus-local code + the tooltip
   * action — the domain editor lives entirely under nemus/.
   */
  ...
  function evalActive() {
    void nemusEngine.eval(projectStore.activeSource, projectStore.project?.path);
  }
  ...
  // ── Outline / Problems → editor jump (one-shot relay from the NemusShell store) ─
  ...
  let lastGotoSeq = 0;
  $effect(() => {
    const req = nemusStore.gotoRequest;
  ...
  let lastCommitSeq = 0;
  $effect(() => {
    const req = nemusStore.commitRequest;
    const comp = editorComp;
  ...
  /** New `.nemus` — delegated to the centralised project-action picker (writes a
...
<div class="ed">
  ...
  {#if activePath}
    {#key activePath}
      <NemusEditor
        ...
        oninput={onInput}
        oncaret={(line, col) => nemusStore.setCaret(line, col)}
  ...
  {:else}
    <div class="ed-empty">
      <EmptyState
        message="No file open. Open a project (Ctrl+O) or create a new .nemus (Ctrl+Shift+N)."
      />
    </div>
  {/if}
  ...
  {#if activePath}
    <div class="ed-footer">
      <span class="ed-pos"><MapPin size={11} /> Ln {nemusStore.caretLine}, Col {nemusStore.caretCol}</span>
    </div>
  {/if}
