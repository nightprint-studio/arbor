/**
 * nemus structural refactors over the Tree-sitter CST (pure, CodeMirror-free).
 *
 * The on-demand counterpart of `nemus-edit.ts` (which rewrites control *literals*):
 ...
 *
 * The `.nemus` language is **flat-scoped** — a top-level name has no shadowing, so
...
import { identifierUsages, extractSymbols, type Tree, type Node } from './nemus-lang';
import type { EditChange } from './nemus-edit';

/** A legal `.nemus` identifier (the binding-name grammar). */
 ...
 *  expression so binding doesn't change (same set as `nemus-edit`'s LOW_PREC). */
