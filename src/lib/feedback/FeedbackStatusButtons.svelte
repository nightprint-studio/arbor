<script lang="ts">
   ...
   * StatusBar and the Merula footer: a jobs badge (running / finished count), an
   ...
   * mounted in (main shows untagged + main-targeted items; merula shows
   * merula-targeted ones). Renders three bare buttons so it drops directly into
  ...
  let {
    /** Show the minimized-dialogs badge (main window only — merula has no parked
     *  modals). */
    parked = false,
    /** Always show the transfers (downloads/exports) badge, even when idle. Merula
