...
- **Bennu** (Java editor): the **Structure** tool window is now a proper tree — a Java type shows its fields, methods and nested types grouped and nested, each member tagged with a visibility dot (public/protected/private/package) and fields keeping the getter/setter/wither markers. It also works on **XML/JSP** files (`.jsp/.jspf/.tag/.xml/.xsd/.wsdl/.tld/.pom`), showing a nested tag outline with a key attribute (id/name/var/…); click any entry to jump to its line. Its header has **Collapse all** / **Expand all** chevrons like the Project panel.
