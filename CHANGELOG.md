...
- File Explorer: a global **Ctrl+Shift+E** shortcut (registered system-wide, so it fires even when Arbor isn't focused) opens the built-in explorer in its own dedicated window — a standalone, frameless explorer rather than the whole app. Also available from the Command Palette ("Open File Explorer in New Window"); re-pressing focuses the existing window instead of opening a second one.
