...
- Context menus are now fully keyboard-operable: open one on the cursor row with the **Menu** key or **Shift+F10**, then ↑/↓ to move, type-ahead to jump, →/← to open/close a submenu, Enter to activate, Esc to close — with focus trapped inside the menu so it never escapes to the window chrome.
