...
Stato: **✅ atterrato** (PR #4 completo in 8 sessioni).
...
| 5 | 6 | Migrazione ns host-pure (~22 file) | ✅ | Migrati in `arbor_plugin_core::lua_api::ns::*`: `log`, `events`, `json`, `text`, `meta`, `notify`, `hooks`, `command`, `keybinding`, `service`, `timer`, `scheduler`, `contribution`, `fs`, `http`, `settings`, `ui/*` (13 sub-file, **escluso** `branding`), studios (5). `register(...)` ora hardcoda le 22 `ns::*::install` prima degli `extra_installers`; `shell_installers()` ridotto a 16 wrapper (i ns "shell"). **Nuova capability `AppCtx`**: `active_repo_path()` (per `settings.read_project`) + `open_path()` (per `ui.open_path`), entrambe con default no-op. **Nuovo plumbing**: `host_weak: Option<Weak<Mutex<PluginHost>>>` su `ApiInstallParams` + `ApiCtx` (popolato da `PluginHost::self_arc` in `load_plugin`) — usato da `events.emit`, `timer.*`, `service.call/list`, `http.get`, `meta.plugin_loaded` per rientrare nell'host senza `AppState`. `http.get` non usa più `tauri::async_runtime::spawn`: gira su un current-thread Tokio runtime in un `std::thread`. `scheduler` valida il cron via nuovo `arbor_scheduler::prelude::validate_cron` (nessuna nuova lib — `cron` era già in `arbor-scheduler`). `ui.branding` resta nel guscio come `UiBrandingInstaller` che fa `arbor.get("ui")` e ci attacca le funzioni (gira dopo `ns::ui::install`). |
| 6 | 7 | ns "shell" che restano in src-tauri + wiring `LuaNamespaceInstaller` | ✅ | Dir rinominata: `src-tauri/src/plugin/api/ns/` → `src-tauri/src/plugin/ns_shell/` (16 ns + `ui/branding`). I 16 wrapper `XxxInstaller` + `shell_installers()` migrati da `api/mod.rs` a `ns_shell/mod.rs` (path macro `ns::repo::install` → `repo::install`, ora moduli fratelli). `plugin/mod.rs` ora dichiara `pub mod ns_shell;`; `api/mod.rs` ridotto a `ctx` + `helpers` (shim). `api_installer.rs` chiama `crate::plugin::ns_shell::shell_installers()`. I ns importano via path assoluti `crate::plugin::api::{ctx,helpers}` (invariati). `ui/branding` resta coerente: `UiBrandingInstaller → ui::install` fa `arbor.get("ui")` e gira dopo il `ns::ui` di plugin-core (ordine in `shell_installers()` immutato). |
| 7 | 8 | `HookDispatcher` + `LuaHookListener` + migrazione ~30 call site `fire_hook` | ✅ | `hook_registry.rs` rimosso (core + shim src-tauri); nuovo `arbor-plugin-core::hook_router` (`matches_pattern`, `fire`/`fire_collecting`, free fn `fire_broadcast`/`fire_on`/`fire_vetoable` su `&PluginHost`, e `LuaHookListener` impl `HookListener`). `PluginHost` perde `fire_hook`/`fire_hook_on`/`collect_veto` (restano `plugin_has_handler`/`remove_hook`). `HookDispatcher::fire_blocking`/`fire_vetoable_blocking` (wrap `futures_executor::block_on`, nuova dep `futures-executor`). `Arc<HookDispatcher>` in `AppState`, costruito in `new()` via `build_hook_dispatcher` (registra `HOOK_CATALOG` con `on_pre_commit`→`Vetoable` + `LuaHookListener` su `Weak<plugin_host>`). Helper `AppState::fire_hook(hook, Value)` come unico bridge sync per i broadcast. Call site: broadcast→`state.fire_hook`/`dispatcher`; veto→`fire_vetoable_blocking` (ora **short-circuit al primo plugin**, msg singolo invece di lista); targeted (`fire_hook_on`)→`hook_router::fire_on(&host,…)` (mantiene il guard già detenuto). Callers interni core (events.emit, timer, http, scheduler lua_action, lifecycle, unload_all) migrati alle free fn. |
| 8 | 9 + 10 + 11 | Cleanup shim + prelude finale + README + docs + sanity check | ✅ | Rimossi tutti gli shim `pub use` (`sandbox`, `runtime`, `event_bus`, `lua_ctx`, `settings_store`, `toolchain`, `contribution`, `tree`) + dir `src-tauri/src/plugin/api/` (ctx + helpers, file orfani inclusi). `ApiCtxExt` (downcast Tauri-only su `ApiCtx`) spostato in `src-tauri/src/plugin/ns_shell/ctx_ext.rs`. Tutti i call site rimasti migrati a `arbor_plugin_core::prelude::*`; le import multiple di prelude nei file `ns_shell/*` accorpate in una sola. `plugin/mod.rs` ridotto a `ns_shell` + `api_installer`. Prelude esteso con `load_plugin_states`/`save_plugin_states` (+ re-export in `runtime/mod.rs`) e con gli helper consumati dal guscio (`json_to_lua`, `LuaTuple`, `ok2`/`err2`/`boolerr2`). README di plugin-core riscritto "planned"→"atterrato"; sezione PR #4 di `plugin-api-architecture.md` aggiornata. Nessuna entry CHANGELOG (refactor interno). |
