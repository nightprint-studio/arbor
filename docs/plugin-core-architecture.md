...
| 3 | 4 | Sandbox + runtime (`consts`, `loaded`, `manifest`, `scheduler`, `host`) | ⏳ | Step pesante. 8 file Lua embedded da spostare. `host::hooks` viene riscritto in sessione 7. |
| 4 | 5 | api/ctx + api/helpers + register orchestrator | ⏳ | Introduce trait `LuaNamespaceInstaller` per ns che restano nel guscio. |
