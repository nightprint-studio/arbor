...
| 6 | 7 | ns "shell" che restano in src-tauri + wiring `LuaNamespaceInstaller` | 🚧 | Già wired in `shell_installers()` come wrapper: `repo`, `mr`, `ci`, `issues`, `notes`, `pipeline`, `cloud`, `brp`, `security`, `toolchain`, `terminal`, `tabs`, `workspace`, `linked_worktrees`, `job`, `ui/branding`. Rimane (Step 7 vero): rinominare la dir `src-tauri/src/plugin/api/ns/` → `ns_shell/` e migrare `HookDispatcher`/`fire_hook`. |
