...
**Stato**: 🚧 **in corso**. **Ordine adottato per rischio** (no-compile in sessione): si parte dai pezzi leaf/isolati per stabilire il pattern, poi i grossi. Fatti finora:
...
- **`corvus-pipeline-api`** (leaf): il **motore espressioni puro** di pipeline — `vars` (RunContext/VarValue/interpolazione/transform chain) + `condition` (Condition/CompareOp/`evaluate`) + `condition_parser` — estratto in `crates/corvus/pipeline/api` (serde/serde_json/regex/tracing), con i **unit test già presenti** (parser, evaluator, vars/transform). `pipeline/mod.rs` re-exporta `vars` dal crate; `pipeline/condition.rs` è ora uno shim che re-exporta l'API condition + tiene `IfBlock`/`IfBranch`/`BranchSelection` (accoppiati a `StepDef`). Stub `crates/pipeline/api` cancellato.

**Rimanenti M2**: impl jira/linear → `corvus-issue-tracker-{jira,linear}` (⚠️ host-coupled: `crate::error::AppError`, `credential_store` keyring service `"arbor-git-client"`, **OAuth refresh** `oauth_linear::try_refresh`); git_provider ~9.2k/47file — ⚠️ i **DTO sono incastrati dentro i file impl** (`mr_impl` 2k righe ecc.), `types/` li ri-aliasa → l'`api` NON è un leaf pulito, serve prima sbrogliare le impl; `corvus-pipeline-core` (orchestratore, da `pipeline/mod.rs`); poi marketplace. **Nodo ricorrente**: le impl credential-coupled (jira/linear, git_provider) hanno bisogno di una strategia credenziali — è il dominio del **credential broker** di Modello D (§D.5/M1c→flip M3). Conviene estrarle **con/dopo M3** (token dal broker, backend keyring-free) oppure dietro un'astrazione token interim. Lo **step 6** (contribution namespace/hook runtime-agnostic) NON fatto: i namespace `arbor.*` vivono ancora in `ns_shell/` e importano i crate; arriva con M6.
